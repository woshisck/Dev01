...
try {
  try {
    ...
    Write-Status -Result 'skipped' -ProjectCL $projectChange -CodeCL $codeChange -Message 'Another Dev01 UGS build is already running.'
    exit 0
  }
  ...
  $env:P4TICKETS = Join-Path $agentRoot 'secrets\Dev01BuildAgent.ticket'
  $env:P4TRUST = Join-Path $agentRoot 'secrets\p4trust.txt'
  ...
  Write-Status -Result 'failed' -ProjectCL $projectChange -CodeCL $codeChange -Message $message
