...
namespace
{
	...
	const FLocalizedPropertyText SettingsProperties[] =
	{
		...
		{ GET_MEMBER_NAME_CHECKED(UStylizedLightingSettings, bEnableDarkColorFloor), TEXT("Enable Dark Character Readability"), TEXT("启用角色暗处可读性"), TEXT("Weak view-upper-right two-tone shading, faded out by real lighting. No extra LightActor; metallic surfaces remain reflection-driven."), TEXT("仅在暗处提供来自镜头右上方的弱明暗面；不随角色旋转，不创建补光灯，真实光照足够时自动退出。金属仍依赖真实反射。") },
	...
	const FLocalizedPropertyText ProfileProperties[] =
	{
		...
		{ GET_MEMBER_NAME_CHECKED(FStylizedCharacterLightingProfile, CharacterBaseFill), TEXT("Dark Readability Strength"), TEXT("暗处可读强度"), TEXT("Strength of the subtle two-tone fallback. Zero disables it. Real lighting smoothly takes over; normal camera exposure still applies."), TEXT("控制镜头右上方弱明暗面的整体强度。0 为关闭；真实光照会平滑接管。它仍接受相机曝光，不会把角色锁成恒亮。") },
		{ GET_MEMBER_NAME_CHECKED(FStylizedCharacterLightingProfile, CharacterBaseFillSoftness), TEXT("Dark Readability Boundary Softness"), TEXT("弱明暗交界柔度"), TEXT("Softens only the fallback partition, not the real scene-light partition."), TEXT("只调整暗处弱明暗面的交界柔度，不改变真实灯光的二分逻辑。") },
		{ GET_MEMBER_NAME_CHECKED(FStylizedCharacterLightingProfile, CharacterBaseFillOcclusionInfluence), TEXT("Dark Readability AO Influence"), TEXT("暗处可读性受材质 AO 影响"), TEXT("How strongly material AO can darken the fallback. Zero ignores AO; one applies full material AO."), TEXT("材质 AO 对弱明暗面的压暗程度。0 不受 AO 影响，1 完全接受材质 AO。") },
