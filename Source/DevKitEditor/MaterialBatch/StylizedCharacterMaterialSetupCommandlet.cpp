...
void BuildMaterialGraph(UMaterial* Material, UTexture* DefaultColor, UTexture* DefaultNormalBias, UTexture* DefaultMixMap)
{
	...
	UMaterialExpressionComponentMask* SpecControl = AddMask(Material, MixMap, true, false, false, -820, 340);
	UMaterialExpressionComponentMask* Roughness = AddMask(Material, MixMap, false, true, false, -820, 470);
	UMaterialExpressionComponentMask* Metallic = AddMask(Material, MixMap, false, false, true, -820, 600);
	...
	// R=0.5 is neutral. SpecIntensity scales the signed deviation without changing
	// the neutral point, so zero disables the texture-authored adjustment.
	UMaterialExpressionSubtract* SignedSpecControl = AddExpression<UMaterialExpressionSubtract>(Material, -480, 500);
	SignedSpecControl->A.Connect(0, SpecControl);
	SignedSpecControl->B.Connect(0, AddConstant(Material, 0.5f, -680, 520));
	UMaterialExpressionMultiply* SpecControlStrength = AddExpression<UMaterialExpressionMultiply>(Material, -260, 550);
	SpecControlStrength->A.Connect(0, SignedSpecControl);
	SpecControlStrength->B.Connect(0, SpecIntensity);
	UMaterialExpressionAdd* NeutralSpecControl = AddExpression<UMaterialExpressionAdd>(Material, -60, 550);
	NeutralSpecControl->A.Connect(0, SpecControlStrength);
	NeutralSpecControl->B.Connect(0, AddConstant(Material, 0.5f, -240, 680));
	UMaterialExpressionSaturate* Specular = AddExpression<UMaterialExpressionSaturate>(Material, 0, 550);
	Specular->Input.Connect(0, NeutralSpecControl);
	...
	AddComment(Material, TEXT("T_MixMap: R = signed GGX highlight control (0.5 neutral); G = Roughness; B = Metallic."), -1260, 210, 1080, 130);
	AddComment(Material, TEXT("Specular = saturate(0.5 + (T_MixMap.R - 0.5) * SpecIntensity). Below 0.5 suppresses GGX; above 0.5 strengthens and broadens the energy-conserving GGX lobe."), -560, 500, 980, 120);
