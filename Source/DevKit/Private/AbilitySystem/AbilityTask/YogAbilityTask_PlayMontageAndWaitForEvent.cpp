...
void UYogAbilityTask_PlayMontageAndWaitForEvent::TickTask(float DeltaTime)
{
	...
	const float CurveValue = MontageToPlay->EvaluateCurveData(
		YogAbilityTaskPlayMontageAndWaitForEvent_PlayRateCurveName, FAnimExtractContext(static_cast<double>(Position)), false);
	AnimInstance->Montage_SetPlayRate(MontageToPlay, FMath::Max(Rate * CurveValue, 0.01f));
}

UYogAbilitySystemComponent* UYogAbilityTask_PlayMontageAndWaitForEvent::GetTargetASC()
{
...
void UYogAbilityTask_PlayMontageAndWaitForEvent::OnAbilityCancelled()
{
...
void UYogAbilityTask_PlayMontageAndWaitForEvent::OnMontageEnded(UAnimMontage* Montage, bool bInterrupted)
{
	if (!bInterrupted)
	{
		if (ShouldBroadcastAbilityTaskDelegates())
...
void UYogAbilityTask_PlayMontageAndWaitForEvent::OnGameplayEvent(FGameplayTag EventTag, const FGameplayEventData* Payload)
{
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
UYogAbilityTask_PlayMontageAndWaitForEvent* UYogAbilityTask_PlayMontageAndWaitForEvent::PlayMontageAndWaitForEvent(UGameplayAbility* OwningAbility,
	FName TaskInstanceName, UAnimMontage* MontageToPlay, FGameplayTagContainer EventTags, float Rate, FName StartSection, bool bStopWhenAbilityEnds, float AnimRootMotionTranslationScale)
{
	...
	UYogAbilityTask_PlayMontageAndWaitForEvent* MyObj = NewAbilityTask<UYogAbilityTask_PlayMontageAndWaitForEvent>(OwningAbility, TaskInstanceName);
...
void UYogAbilityTask_PlayMontageAndWaitForEvent::Activate()
{
	if (Ability == nullptr)
	{
		return;
	...
	if (YogAbilitySystemComponent)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			if (YogAbilitySystemComponent->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection) > 0.f)
			{
				...
				CancelledHandle = Ability->OnGameplayAbilityCancelled.AddUObject(this, &UYogAbilityTask_PlayMontageAndWaitForEvent::OnAbilityCancelled);

				BlendingOutDelegate.BindUObject(this, &UYogAbilityTask_PlayMontageAndWaitForEvent::OnMontageBlendingOut);
				AnimInstance->Montage_SetBlendingOutDelegate(BlendingOutDelegate, MontageToPlay);

				MontageEndedDelegate.BindUObject(this, &UYogAbilityTask_PlayMontageAndWaitForEvent::OnMontageEnded);
		...
		{
			ABILITY_LOG(Warning, TEXT("UYogAbilityTask_PlayMontageAndWaitForEvent call to PlayMontage failed!"));
		}
	}
	else
	{
		ABILITY_LOG(Warning, TEXT("UYogAbilityTask_PlayMontageAndWaitForEvent called on invalid AbilitySystemComponent"));
	}

	if (!bPlayedMontage)
	{
		ABILITY_LOG(Warning, TEXT("UYogAbilityTask_PlayMontageAndWaitForEvent called in Ability %s failed to play montage %s; Task Instance Name %s."), *Ability->GetName(), *GetNameSafe(MontageToPlay),*InstanceName.ToString());
		if (ShouldBroadcastAbilityTaskDelegates())
		{
			OnCancelled.Broadcast(FGameplayTag(), FGameplayEventData());
		}
...
void UYogAbilityTask_PlayMontageAndWaitForEvent::ExternalCancel()
{
...
bool UYogAbilityTask_PlayMontageAndWaitForEvent::StopPlayingMontage()
{
	const FGameplayAbilityActorInfo* ActorInfo = Ability->GetCurrentActorInfo();
	if (!ActorInfo)
	{
...
FString UYogAbilityTask_PlayMontageAndWaitForEvent::GetDebugString() const
{
