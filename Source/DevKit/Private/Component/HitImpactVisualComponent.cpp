...
FGameplayTagContainer UHitImpactVisualComponent::GatherVictimTags() const
{
	FGameplayTagContainer Tags;
	Tags.AddTag(HitReactMaterial.IsValid() ? HitReactMaterial : HitImpactVisual_DefaultMaterialTag());
	...
	Tags.AppendTags(OwnedTags);

	// Armor is tracked as an attribute, not a granted tag, so bridge it here.
	if (ASC->GetNumericAttribute(UBaseAttributeSet::GetArmorHPAttribute()) > 0.f)
	{
		Tags.AddTag(HitImpactVisual_ArmoredTag());
	}
...
int32 UHitImpactVisualComponent::PlayHitFeedback(const FVector& HitLocation, int32 Level)
{
	...
	if (FX.VFX)
	{
		UNiagaraFunctionLibrary::SpawnSystemAtLocation(
			World, FX.VFX, HitLocation, FX.VFXRotationOffset, FX.VFXScale);
	}

	if (FX.Sound)
	{
		UGameplayStatics::SpawnSoundAtLocation(
			World, FX.Sound, HitLocation, FRotator::ZeroRotator, FX.SoundVolume, FX.SoundPitch);
	}

	return FX.CameraShakeLevel;
}
