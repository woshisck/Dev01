 ...
 *
 * 工作流：
 *   ActivationRequiredTags = Buff.DashInvincible（必须在冲刺状态）
 *   ClearAllAmmo() ExecuteReloadCue（弹夹掉落音效）
 *   等待 RefillDelay 秒（Timer
 *   SetAmmoToMax() ExecuteReloadCue（补满音效）
 *   结束（冲GA 继续运行
 *
 * GA AbilityTags = Ability.Musket.SprintReload
 * 不包Character.State，冲GA CancelAbilitiesWithTag 不会取消它 *
 * Blueprint 子类 Class Defaults 需填写
 *   - RefillDelay   清空到补满之间的延迟（默0.3s，匹配冲刺动画中点）
 */
