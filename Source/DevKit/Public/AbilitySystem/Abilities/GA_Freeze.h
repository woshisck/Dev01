 ...
 * 冻结 GA（施加在目标身上
 *
 * - 激活时立即施加 ChillEffect（减GE
 * - 记录触发位置 OriginLocation
 * - 3 秒后检查：若距 OriginLocation < RequiredDistance(800)
 *   移除 ChillEffect，施FrozenStunEffect（眩+ 惩罚伤害
 * - 若成功逃脱= 800 单位）→ 仅移ChillEffect，GA 结束
 *
 * Blueprint 子类需配置
 *   ChillEffect      = GE_Chill（减速，Infinite，结束时自动移除
 *   FrozenStunEffect = GE_FrozenStun（眩+ DamageBuff，HasDuration
...
public:
	...
	/** 惩罚伤害量（TriggerEventData.EventMagnitude 读取*/
	UPROPERTY(EditDefaultsOnly, BlueprintReadOnly, Category = "Freeze")
