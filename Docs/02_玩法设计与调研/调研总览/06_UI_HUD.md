...
|---|---|---|---|
| 背包 UI（CommonUI + StyleDA + RuneInfoCard）(BACKPACK-002 / UI-COMMONUI) | 鼠标 + 手柄双操作 + 样式 DA 化 | ✅ 编译通过 | ✅ DA_BackpackStyle 已配 |
| 背包交互重构（单击抓取 / 长按 3s 退回 / 换格自动 / 悬停绿框）(UI-021) | 改抓取式交互 | ✅ 编译通过 | — |
| 符文边框 NativePaint (UI-022) | Slate 直接画包围框 | ✅ 编译通过 | — |
| 背包热度阶段点按钮 HeatPhaseDot (UI-005) | 1 / 2 / 3 切阶段预览 | ✅ 编译通过 | ✅ WBP_BackpackGrid 已配 |
| 热度区三色叠加 / 单阶切换显示 (UI-007a) | Zone0 / 1 / 2 叠加显示 | ✅ 编译通过 | ✅ DA 颜色已配 |
| 背包格默认半透 InactiveZoneOpacity (UI-010) | 空格不抢眼 | ✅ 编译通过 | ✅ DA 字段已配 |
| 三选一 UI（CommonUI 隔离）(COMBAT-003a / FIX-022 / 23) | 拾取驱动 + 独立触发 + 隔离 NativeOnFocusLost | ✅ 编译通过 | ⚙ HB_PlayerMain 赋 LootSelectionWidgetClass |
| 三选一结束自动开背包 (UI-014) | 选完立即可放置 | ✅ 编译通过 | — |
| HUD 主容器 YogHUDRootWidget (UI-024) | 一个 WBP 摆所有控件 | ✅ 编译通过 | ⚙ WBP_HUDRoot 待建（HUD-001）|
| 液态血条（材质驱动）(UI-023) | 血量晃动反馈 + 指数阻尼 | ✅ 编译通过 | ⚙ WBP_PlayerHealthBar 待建（HUD-001）|
| 武器拾取浮窗 WeaponFloatWidget (UI-008) | 走近显示武器信息 + 自动避开屏幕中心 | ✅ 编译通过 | ✅ |
| 武器拾取动画三阶段（Float → Trail → GlassIcon）(FEAT-015 / UI-013) | 浮窗 → 缩小 → 飞图标 + 流光拖尾 | ✅ 编译通过 | ⚙ WBP_WeaponTrail / GlassIcon 待建（UI-001 / 002）|
| 武器玻璃图标热度颜色 (UI-007b) | 图标随热度变色 | ✅ 编译通过 | ✅ B_PlayerOne 已配 HeatStyleDA |
| 液态玻璃框 GlassFrameWidget (UI-015 / FEAT-016) | Apple 液态玻璃感框 | ✅ 编译通过 | ✅ M_GlassFrame + WBP 已建 |
| 新手引导 TutorialManager + GameDialogWidget (TUT-001 / UI-006) | 武器 / 战斗后弹引导 + 状态机持久化 | ✅ 编译通过 | ✅ DA 已配 |
| 敌人方向箭头 (UI-009 / FIX-028) | 敌人离屏时屏幕边缘指引 | ✅ 编译通过 | ⚙ HB_PlayerMain 赋 EnemyArrowWidgetClass |
| 暂停弹窗屏幕变暗 PauseEffect (UI-012) | 弹窗激活变暗 + 多弹窗叠加计数 | ✅ 编译通过 | — 参数在 BP 调 |
| 战斗日志 / 伤害构成显示 (UI-001 / COMBAT-003 衍生) | DamageBreakdown / CombatLog 订阅 | ✅ 编译通过 | ✅ |
...
- **设计文档**：[BackpackSystem_Guide](../../99_归档/旧方案/2D背包方案/BackpackSystem_Guide.md) · memory: `project_backpack_ui_arch.md`（旧 2D 背包方案已归档；如需重写，请放入 `Docs/04_开发实现与系统文档/系统/Rune/`）
- **验收方式**：
...
- **设计文档**：[LootSelection_Technical](../../04_开发实现与系统文档/系统/UI/LootSelection_Technical.md)
...
- **设计文档**：[WeaponPickupAnim_Technical](../../04_开发实现与系统文档/系统/UI/WeaponPickupAnim_Technical.md)
...
- **设计文档**：[GlassFrame_Technical](../../04_开发实现与系统文档/系统/UI/GlassFrame_Technical.md)
...
- **设计文档**：[TutorialPopup_Copy](../../04_开发实现与系统文档/系统/UI/TutorialPopup_Copy.md)（文案）
- **验收方式**：
...
| ID | 内容 | 文档 |
|---|---|---|
| HUD-001 | 创建 `WBP_HUDRoot`（Parent = `YogHUDRootWidget`）+ `WBP_PlayerHealthBar`（Parent = `LiquidHealthBarWidget`），配 BP_YogHUD → `MainHUDClass` | [TASKS](../../05_完成记录与任务看板/PM/TASKS.md) |
| UI-001 | `WBP_WeaponGlassIcon` + `WeaponGlassAnimDA` 编辑器配置 | [WeaponPickupAnim_Technical](../../04_开发实现与系统文档/系统/UI/WeaponPickupAnim_Technical.md) |
| UI-002 | `WBP_WeaponTrail` 创建（根全屏 Canvas + Image 命名 TrailLine + M_WeaponTrail） | 同上 |
| FIX-028 配置 | HB_PlayerMain Details → Enemy Arrow → `EnemyArrowWidgetClass` 赋 WBP_EnemyArrow | [FeatureLog FIX-028](../../05_完成记录与任务看板/FeatureLog.md) |
| FIX-010 | `WBP_WeaponFloat` 白屏 Bug：将背景移入 InfoContainer | [FeatureLog FEAT-015](../../05_完成记录与任务看板/FeatureLog.md) |
