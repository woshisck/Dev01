...
## Death poison Flow asset
- Configured `/Game/Docs/BuffDocs/Playtest_GA/DeathPoison/FA_Rune_DeathPoison`: Wait DeathAnimComplete on BuffOwner -> ApplyGEInRadius GE_PoisonSplash radius=300 around BuffOwner.
...
- Configured `FA_Rune512_Heavy_Base`: WeaponSkill/Finisher card adds +8 damage after base knockback; finisher-role branch adds +20 damage and 520cm knockback.
- Found existing asset `/Game/Docs/BuffDocs/V2-RuneCard/512Generated/DA_Rune512_Heavy`.
- Updated RuneInfo -> CombatCard.
- Manual check: RequiredAction remains Any; the card is routed to the WeaponSkill slot and resolves as a Finisher. FA_Rune512_Heavy_Base adds a finisher-role branch for much higher bonus damage and knockback distance.
...
- Deprecated Combo Scaling remains disabled for AttackUp; card order and Link recipes drive build scaling.
- Updated RuneInfo -> CombatCard.
...
- No Spawn Slash Wave Projectile node found in `FA_Rune512_Moonlight_Base`.
- Found existing EffectProfile `/Game/Docs/BuffDocs/V2-RuneCard/512Generated/Profile/EP_Rune512_Moonlight_Base`.
- Updated EffectProfile `EP_Rune512_Moonlight_Base` from `FA_Rune512_Moonlight_Base` (no matching legacy node found).
...
- Configured `FA_Rune512_Moonlight_Forward_Burn`: SpawnSlashWave -> Wait `Action.Rune.MoonlightBurnHit` -> Play Niagara `/Game/Art/EnvironmentAsset/VFX/Niagara/Fire/NS_Fire_Floor` -> persistent UGE_RuneBurn DOT; cleared Flipbook nodes=0.
...
- No Spawn Slash Wave Projectile node found in `FA_Rune512_Moonlight_Forward_Poison`.
- Configured `FA_Rune512_Moonlight_Forward_Poison`: projectile event source -> Wait `Action.Rune.MoonlightPoisonHit` -> Play Niagara hit -> Apply GE_Poison x3 -> Play Niagara spread -> ApplyGEInRadius radius=300 max=3; normalized wait nodes=2; cleared Flipbook nodes=1.
- Found existing EffectProfile `/Game/Docs/BuffDocs/V2-RuneCard/512Generated/Profile/EP_Rune512_Moonlight_Forward_Poison`.
- Updated EffectProfile `EP_Rune512_Moonlight_Forward_Poison` from `FA_Rune512_Moonlight_Forward_Poison` (no matching legacy node found).
...
- Bleed, Rend, Wound, Knockback, Heavy, Fear, Freeze, Stun, and Curse are generated as Normal combat cards.
...
- Moonlight base and forward LinkFlows no longer use combo projectile scaling; Link recipes and card order drive build changes.
