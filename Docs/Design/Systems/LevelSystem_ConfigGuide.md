...
> 最后更新：2026-04-12（与代码对齐；修正 FDifficultyConfig 描述；难度档位仍在 DA_Room）
...
- **DA_Campaign（局内序列）** — 描述"这一局按什么顺序打、每关总难度分是多少、发多少金币、房间类型概率是多少"
...
程序根据 DA_Campaign 当前关的 **TotalDifficultyScore** 自动选取 Low / Medium / High 三档之一，再从对应档位读取 **MaxWaveCount**（最大波次数）。

在 **Difficulty** 分类下，填写三个档位的上限：

| 字段 | 说明 | 建议值 |
|------|------|--------|
| **Low Difficulty → Max Wave Count** | 低分段的最大波次数 | `2` |
| **Medium Difficulty → Max Wave Count** | 中分段的最大波次数 | `3` |
| **High Difficulty → Max Wave Count** | 高分段的最大波次数 | `4` |

> **分段阈值**在 GameMode 蓝图中配置（`LowDifficultyScoreMax` 和 `HighDifficultyScoreMin`），策划在 DA_Room 里只需填各档的上限波次数。
...
进入关卡时，系统从 Buff Pool 中随机选 N 个符文施加给所有刷出的怪。N 由 DA_Campaign 当前关的 **Buff Count** 决定。

点 **Buff Pool** 右侧的 **+**，将 `DA_Rune_XXX`（RuneDataAsset）拖入槽位。

> Buff Pool 使用 **RuneDataAsset**（符文）而非单独的 BuffDataAsset。选中的符文的 Flow Graph 会在每只新刷出的怪身上激活。
...
**Buff Pool**：DA_Rune_Buff_ArmorBreak、DA_Rune_Buff_Haste
...
| Floor | EliteChance | TotalDifficultyScore | GoldMin/Max | BuffCount |
|-------|-------------|----------------------|-------------|-----------|
| 1 | 0 | 20 | 10/15 | 0 |
| 2 | 0 | 28 | 12/18 | 1 |
| 3 | 0.2 | 36 | 15/22 | 1 |
...
**Q：金币没有增加**
- 确认 DA_Campaign 当前关的 `Gold Max > 0`
- 确认 HUD 绑定了 `PlayerCharacterBase` 的 `OnGoldChanged` 委托

**Q：Buff 没有施加给怪**
- 确认 DA_Campaign 当前关的 `Buff Count > 0`
- 确认 DA_Room 的 `Buff Pool` 中填了 RuneDataAsset
