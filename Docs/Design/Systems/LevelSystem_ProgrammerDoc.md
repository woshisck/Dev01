...
> 最后更新：2026-04-12（与代码对齐；修正 FDifficultyConfig 描述，实际不存在此结构体；难度档位仍在 DA_Room；FPortalDestConfig.RoomPool 类型修正）  
...
数据层（Data Assets）
  ...
  └── UCampaignDataAsset   局内关卡序列（FloorTable，每关含总难度分 + 奖励参数）
...
| `BuffPool` | `TArray<TObjectPtr<URuneDataAsset>>` | 进关时随机选 N 个 RuneDA，在每只刷出的怪身上激活其 Flow Graph |
...
> 这是 DA_Room 中唯一需要配置的难度参数。波次预算、刷怪方式等均由程序自动决定。
...
| `CommonWeight/RareWeight/EpicWeight` | `float` | 1/0.3/0.1 | 符文稀有度权重 |
| `GoldMin/GoldMax` | `int32` | 10/20 | 关卡结算金币范围 |
| `BuffCount` | `int32` | 1 | 从 DA_Room.BuffPool 中随机选几个施加给怪 |
...
StartPlay()
  └─ CampaignData != null → StartLevelSpawning()
       ...
       ├─ 缓存 ActiveRoomData、ActiveGoldMin/Max、ActiveBuffCount
       ├─ 根据 Config.TotalDifficultyScore 选档（Low/Medium/High）→ 取对应 MaxWaveCount
       ├─ SelectRoomBuffs() → ActiveRoomBuffs（进关时确定，刷怪时施加）
       ├─ GenerateWavePlans(TotalDifficultyScore, MaxWaveCount, ActiveRoomData)
...
```
GenerateWavePlans(TotalScore, MaxWaveCount, Room):
  ...
  for i in [0, WaveCount):
    ...
    WavePlans[i] = BuildWavePlan(Budget, Room)
```

```
BuildWavePlan(Budget, Room):
  ...
  Step 3: 用预算填充敌人，遵守以下限制：
    - 第一只：不受预算限制（保证至少 1 只）
    - 后续：敌人 DifficultyScore ≤ RemainingBudget
    - MaxCountPerLevel：跨波次累计上限（-1 = 不限）
    - 循环直到预算耗尽或无合法候选

  Step 4: 剩余预算 → 构建按需补刷池（DemandEnemyPool）
    - 收集分数 ≤ 剩余预算的敌人类型（不受 MaxCountPerLevel 限制）
...
SpawnNextOneByOne()
  ...
  │    └─ 对刷出的敌人：激活 ActiveRoomBuffs 中每个符文的 Flow Graph
...
EnterArrangementPhase()
  ...
  ├─ 发放金币 RandRange(ActiveGoldMin, ActiveGoldMax)
  ├─ SpawnActor<ARewardPickup>(LastEnemyKillLocation)
  └─ ActivatePortals()

ActivatePortals()（传送门目标在 ActiveRoomData.PortalDestinations）
...
| 函数 | 文件 | 说明 |
|------|------|------|
| `StartLevelSpawning()` | `YogGameMode.cpp` | 关卡启动入口，选 RoomData + 生成波次计划 |
| `GenerateWavePlans()` | `YogGameMode.cpp` | 将总难度分分配到各波 |
| `BuildWavePlan()` | `YogGameMode.cpp` | 按预算从 EnemyPool 填充单波敌人 |
...
| `ActivatePortals()` | `YogGameMode.cpp` | 随机开门并分配下一关 RoomData |
...
| `SpawnEnemyFromPool()` | `YogGameMode.cpp` | 白名单过滤 Spawner + 生成敌人 + 施加 Buff |
| `SelectRoomBuffs()` | `YogGameMode.cpp` | Fisher-Yates 洗牌 BuffPool，取前 N 个 |
