...
**职责：** 标识 `URoomDataAsset` 的房间类型，供 `SelectRoomByTag` 在关卡结算时匹配目标房间。  
**挂载位置：** 只填写在 `RoomDataAsset.RoomTypeTag` 字段，**不挂 ASC**，与 GAS 无关。  
**配置文件：** `Config/Tags/RoomTag.ini`

| Tag | 含义 |
|-----|------|
| `Room.Type.Normal` | 普通战斗房（最常见，标准敌人池） |
| `Room.Type.Elite` | 精英战斗房（精英专属敌人 `bEliteOnly` 可出现） |
| `Room.Type.Shop` | 商店房（功能待实现） |
| `Room.Type.Event` | 事件房（功能待实现） |

**注意：** Shop / Event 只在 `CampaignDataAsset.RoomPool`（或传送门 `RoomPool`）中存在对应资产时才会被骰出。若无匹配资产，自动退化为 Normal。
