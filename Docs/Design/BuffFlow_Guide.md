...
· Effects[]（属性修改/Tag）      · 触发后做什么（GrantGA/特效/音效）
· 背包格子形状                  · 复杂的状态机/计时器逻辑
─────────────────────────────   ─────────────────────────────
                策划配置                    策划用节点搭建
```
...
> **GA 授予已移至 FA 层。** 在 FA 里使用 `Grant GA` 节点授予被动 GA，无需在 DA 的 Effects[] 中配置。
...
#### `Get Rune Info`  ⭐
查询目标 ASC 上匹配 BuffTag 的符文（GE）的运行时状态，是 FA 中条件分支的核心节点。

| 配置项 | 说明 |
|---|---|
| `Target` | 查询目标（BuffOwner / Instigator / Target） |
| `BuffTag` | DA 中 RuneConfig.BuffTag 填写的 Tag |
...
[OnDamageDealt]
  ...
  → [GetRuneInfo](BuffTag=Buff.Berserk)
       ├─ Found
...
[OnPeriodic](Interval=0.5s)
  → [GetRuneInfo](BuffTag=Buff.ShieldBash)
       ├─ Found
...
#### `Grant GA`  ⭐
向目标授予 GameplayAbility，**FA 停止时自动撤销**（符文卸下时无需手动连接撤销逻辑）。

| 配置项 | 说明 |
|---|---|
| `AbilityClass` | 要授予的 GA 类（继承自 YogGameplayAbility） |
...
[Start]
  → [GrantGA](GA_Knockback, Target=BuffOwner)
       └─ Out → [OnDamageDealt]
                    → [PlayNiagara](NS_KnockbackImpact)
                    → [结束]
...
Effects:
  [0] Add Attribute Modifier
      ...
      ModOp     = Additive
      Value     = 600.0
      ← 不在 DA 配置 GA，GA 授予在 FA 里用 GrantGA 节点完成
```
...
```
EventData.Instigator → Get ASC → Get Numeric Attribute (KnockbackForce) → AttackerForce
...
> GA 直接读 AttributeSet 里的 `KnockbackForce`——GE 已经把 +600 写进去了，无需从 DA 传参。
...
| TriggerGA Fragment | 在 DA Effects[] 中配置 GA | 移至 FA，使用 `Grant GA` 节点 |
| 新增节点 | — | `Get Rune Info`（查询符文 GE 运行时信息） |
| 新增节点 | — | `Grant GA`（授予 GA，FA 停止时自动撤销） |
| 删除节点 | `Send Gameplay Event` | 由 `Grant GA` + AbilityTrigger 模式取代 |
