...
| 层 | 职责 | 关键类 |
|---|---|---|
| 写入层 | 产生伤害事件和卡牌结算事件，推入静态桥 | `UYogAbilitySystemComponent`、`UCombatDeckComponent` |
...
| `FinalDamage` | float | 最终伤害（含暴击；纯卡牌结算行为 0） |
| `bIsCrit` | bool | 是否暴击 |
...
| `bConsumedCard` | bool | 兼容字段：卡牌已结算（新逻辑等价于 `bResolvedCard`） |
...
| `bStartedShuffle` | bool | 本次结算后进入洗牌（卡牌结算行有效） |
| `bIsCardEventOnly` | bool | 纯卡牌结算行（FinalDamage=0，无目标） |
| `CardDisplayName` | FName | 卡牌显示名称 |
| `CardConsumeTiming` | FName | 兼容字段：结算时机（"OnCommit" / "OnHit"） |
...
| `"Card_Consume"` | 兼容名：卡牌结算（无伤害） | 淡蓝 (0.4,0.7,1) |
| `"Card_Shuffle"` | 卡牌结算并触发洗牌 | 灰蓝 (0.5,0.7,1) |
...
**注意**：核心方案新增的 4 个值（Card / Finisher / Link / Shuffle）追加在末尾，蓝图枚举值不能插入中间否则错位。
...
`UCombatDeckComponent::ResolveAttackCard` 在结算成功后，直接调用 `UCombatLogStatics::PushEntry(Resolve)` 推入一条 `bIsCardEventOnly=true` 的结算行（DamageType="Card_Consume" 或 "Card_Shuffle"；`Card_Consume` 为兼容枚举名）。
