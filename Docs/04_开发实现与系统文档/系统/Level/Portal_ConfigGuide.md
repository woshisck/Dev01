...
> 适用人群：策划  
> 配套文档：[传送门系统设计](../../../00_入口与规范/缺失引用记录.md)  
> 最后更新：2026-09-21（交互改为按住 E 蓄力，时长由 IA_Interact 的 Hold Trigger 决定）
...
关卡结算后生成的 `ARewardPickup`（奖励拾取物）不会自动触发，玩家需要**走近后按住 E 键**才能触发符文选择界面。

| 步骤 | 行为 |
|---|---|
| 玩家进入碰撞范围 | 系统自动登记（可在此时显示"按住 E 拾取"提示，UI 待实现）|
| 玩家按住 E 键 | 触发 `TryPickup()`，弹出符文三选一界面，拾取物消失 |
| 玩家离开范围（未按住 E）| 登记清除，再次进入才能重新拾取 |
