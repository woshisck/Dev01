...
| 贴图命名规则表 | Level Editor 顶部 `贴图规则` / `Tools -> DevKit 工具 -> 美术资产工具 -> 贴图命名规则与检查` | `/Game/Generated/Performance/DA_MaterialTextureNamingRules_Default`，类型 `UYogMaterialTextureNamingConvention` | 记录 BaseColor/Normal/ORM/Height/Mask/LightInfo/Emissive 的可接受后缀，后续 bake/VT 工具读取；后缀新增直接编辑该资产 |
...
- 贴图命名规则与检查：写出贴图后缀表资产，供后续材质 bake/VT 合并工具扩展。
...
- `YogPerformanceSettingsLibrary` 已写入 `r.Yog.MaterialQuality`、`r.Yog.DynamicOverlayQuality`、`r.Yog.BatchProxyPreference`、`r.Yog.VTAtlasQuality`、`r.Yog.MaterialLightQuality` 和 `r.Yog.MaterialLight.MaxLightInfoCount`。
