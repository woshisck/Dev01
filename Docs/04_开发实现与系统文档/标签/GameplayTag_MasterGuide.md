...
| 分类 | 示例 | 含义 |
|-----|------|------|
| `PlayerState.AbilityCast.*` | `AbilityCast.Attack` / `AbilityCast.WeaponSkill` | 玩家正在执行某个动作 |
...
**与其他层的关系：**
```
PlayerState.AbilityCast.Attack    ← GA 身份标签（具体是哪个动作槽）
...
|----------------|-------|------------|
| `PlayerState.AbilityCast.Attack` | 玩家状态层 | 玩家正在普攻时 |
...
|-----------|-----------|------------|----------|------|
| `PlayerState.AbilityCast.Attack` | `PlayerState.AbilityCast.Dash` | — | 10 | 普攻中无法闪避 |
| `PlayerState.AbilityCast.Dash` | `PlayerState.AbilityCast.Attack` | `PlayerState.AbilityCast.Attack` | 20 | 闪避取消普攻 |
