...
> 2026-06-23 更新：当前项目 Tag 重整结论以 [GameplayTag_ReorgPlan_LOTF.md](GameplayTag_ReorgPlan_LOTF.md) 为准。
> 本文下方保留早期规范细节，仍可作为迁移参考；其中 `PlayerState.*`、`Card.*`、`Enemy_Behavior.*`、`Action.*` 作为新资产正式入口的说法已被新方案替代。
> 新方案核心：`Buff.*` 独立为大系统；玩家/敌人通用动作状态统一到 `Character.State.*`；旧卡牌系统统一归入 `Rune.*`；剧情/教程提升为 `Director.*` 调度系统；UI Tag 只做 UI 路由，不承载业务状态。
...
| 子层 | 职责 |
|-----|------|
| `Rune.Type.*` / `Rune.Rarity.*` | 符文身份（类型/稀有度），贴在 RuneDataAsset |
...
### 2.4 `Character.State.*` — 角色运行时状态层

**职责：** 玩家和敌人共用的动作/窗口/装备切换等运行时状态，挂在角色 ASC。
**作用：** GA AbilityTags（用于查找/取消）+ ActivationOwnedTags（用于状态冲突系统 Block/Cancel）。

| 分类 | 示例 | 含义 |
|-----|------|------|
| `Character.State.Skill.*` | `Character.State.Skill.Attack` / `Character.State.Skill.WeaponSkill` | 角色正在执行某个技能/动作 |
| `Character.State.Movement.*` | `Character.State.Movement.Dash` | 角色正在执行移动动作 |
| `Character.State.Window.*` | `Character.State.Window.CanCombo` | 可取消/可衔接等动作窗口 |
| `Character.State.Equipment.*` | `Character.State.Equipment.SwitchWeapon` | 装备切换类动作状态 |
| `PlayerState.AnimLayer.*` | `AnimLayer.Default` | 当前动画层 |

**`Character.State.*` 的双重用途：**
这层 Tag 可以同时作为：
- GA 的 `AbilityTags`（GA 身份，用于 CancelAbilitiesWithTag）
- GA 的 `ActivationOwnedTags`（运行时状态，用于 StateConflict/HUD/AI 查询）

旧 `PlayerState.AbilityCast.*` 只作为资产迁移和兼容 fallback 保留；新配置不要继续填写。`SkillChargeComponent` 支持旧键 alias 到正式 `Character.State.*`。

---

### 2.5 `Enemy.*` — 敌人技能层

**职责：** 敌人身份特有的 AI/攻击选择标签。通用执行状态仍使用 `Character.State.*`。
...
**与其他层的关系：**
```
Character.State.Skill.Attack       ← GA 身份/运行时状态（具体是哪个动作槽）
...
| 示例 Tag | 含义 |
|---------|------|
| `Buff.Event.KnockbackApplied` | 击退完成，供联动符文监听 |
| `Buff.Event.Bleed` | 流血施加成功，供联动符文监听 |
| `Buff.Event.KillConfirmed` | 击杀确认，供联动符文监听 |

**与 `Action.*` 的区别：**
```
Action.Knockback            ← 触发 GA 的输入信号（FA → 目标）
Buff.Event.KnockbackApplied ← GA 执行完成的输出信号（GA → 玩家 ASC）
```
`Action.*` 是"请执行某动作"，`Buff.Event.*` 是"某动作已完成"。
...
|----------------|-------|------------|
| `Character.State.Skill.Attack` | 角色运行时状态层 | 角色正在普攻时 |
...
|-----------|-----------|------------|----------|------|
| `Character.State.Skill.Attack` | `Character.State.Movement.Dash` | — | 10 | 普攻中无法闪避 |
| `Character.State.Movement.Dash` | `Character.State.Skill.Attack` | `Character.State.Skill.Attack` | 20 | 闪避取消普攻 |
| `Buff.Status.HitReact` | `Character.State.*` | `Character.State.*` | 20 | 受击硬直打断所有角色动作 |
| `Buff.Status.Knockback` | `Character.State.*` | `Character.State.*` | 50 | 被击退时取消全部角色动作 |
| `Buff.Status.Dead` | `Character.State.*` | `Character.State.*` | 999 | 死亡优先级最高 |
| `State.Dialogue` | `Character.State.*` | `Character.State.*` | 100 | 对话中禁止所有战斗行为 |
| `Rune.Type.Attack` | `Rune.Type.Attack` | — | -1 | 同类攻击型符文只激活一个 |
...
├─ 描述"发生了一件事"（瞬时信号）
│   ├─ 触发某个 GA 激活 ──────────────→ Action.*
│   └─ 符文完成后通知其他符文 ─────────→ Buff.Event.*
│
├─ 描述"当前处于某状态"（持续存在）
│   ├─ 玩家/敌人正在执行通用动作 ───────→ Character.State.*
│   ├─ 身份特有行为或 AI 选择 ──────────→ Entity.Type.* / Enemy.*
│   ├─ 符文/Buff 效果产生的状态 ────────→ Buff.Status.*
│   └─ 全局状态（死亡/对话/交互）───────→ State.*
│
├─ 描述"这是什么"（身份标签）
│   ├─ 这个符文是什么类型/稀有度 ────────→ Rune.Type.* / Rune.Rarity.*
...
| `Docs/02_玩法设计与调研/设计文档/Buff_Tag_Spec.md` | `Buff.*` 详细规范 |
| **本文档** | 全命名空间总览 |
| `Docs/02_玩法设计与调研/设计文档/StateConflict_TagBlock.md` | 冲突规则配置指南 |
