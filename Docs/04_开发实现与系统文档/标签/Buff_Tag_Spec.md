...
> 2026-06-23 更新：当前大方向以 [GameplayTag_ReorgPlan_LOTF.md](GameplayTag_ReorgPlan_LOTF.md) 为准。
> `Buff.*` 继续作为独立一级大系统，但只负责 Buff/Debuff/状态效果、触发、效果行为、SetByCaller 数据通道。
> 早期文档里的 `Buff.Rune.*` 后续迁移到 `Rune.*`：符文身份、稀有度、动作绑定、生效方式、触发时机统一归 `Rune.ID.*`、`Rune.Type.*`、`Rune.Rarity.*`、`Rune.Binding.*`、`Rune.Activation.*`、`Rune.Trigger.*`、`Rune.FlowRole.*`。
> 下方旧章节在迁移完成前仍可作为现有资产理解资料，但新资产不要再新增 `Buff.Rune.*`。

> 适用范围：Buff/状态效果相关 GameplayTag，以及旧符文 Buff 标签的迁移说明。
> 配套文件：`Config/Tags/BuffTag.ini`、`Config/Tags/RuneTag.ini`
> 更新日期：2026-04-07
...
正式命名中，符文身份归 `Rune.*`，Buff 系统只保留效果、触发、状态和数据通道。每一层只做一件事。

```
Rune.*
├── Rune.Type.*       【身份层】符文数据资产（DA）的功能分类
├── Rune.Rarity.*     【身份层】符文稀有度
└── Rune.Element.*    【身份层】符文元素预留

Buff.*
├── Buff.Effect.*     【行为层】GE/GA 的效果描述标签
├── Buff.Trigger.*    【触发层】Buff/GE/GA 触发时机的过滤标签
...
| Tag | 层级 | 说明 |
|-----|------|------|
| `Rune.Type` | 父节点 | 功能大类，每个符文必须且只有一个 |
| `Rune.Type.Attack` | 叶子 | 增强攻击/伤害输出的符文 |
| `Rune.Type.Defense` | 叶子 | 护盾、减伤、生存相关的符文 |
| `Rune.Type.Utility` | 叶子 | 移速、冷却、能量类的功能性符文 |
| `Rune.Type.Summon` | 叶子 | 召唤实体或创建持续对象的符文 |
| `Rune.Rarity` | 父节点 | 稀有度，每个符文必须且只有一个 |
| `Rune.Rarity.Common` | 叶子 | 普通，基础数值，掉落频率高 |
| `Rune.Rarity.Rare` | 叶子 | 稀有，中等强度，有附加条件 |
| `Rune.Rarity.Epic` | 叶子 | 史诗，高强度或独特机制，稀少掉落 |
| `Rune.Element` | 父节点，预留 | 元素属性，当前无克制逻辑 |
| `Rune.Element.Fire` | 叶子，预留 | 火元素 |
| `Rune.Element.Ice` | 叶子，预留 | 冰元素 |
| `Rune.Element.Lightning` | 叶子，预留 | 雷元素 |
| `Rune.Element.Void` | 叶子，预留 | 虚空/无属性 |
...
| `Buff.Status.CC` | 父节点，预留 | 控制类状态父节点，当前无控制机制 |
| `Buff.Status.CC.Stun` | 叶子，预留 | 眩晕，无法行动 |
| `Buff.Status.CC.Frozen` | 叶子，预留 | 冰冻，受额外元素伤害 |

> 预留 = 已定义 Tag，当前无对应系统逻辑，可安全填写但不会产生效果。
...
GameplayTagList=(Tag="Rune.Type",            DevComment="符文功能大类，每个符文必须且只有一个")
GameplayTagList=(Tag="Rune.Type.Attack",      DevComment="【Type】增强攻击/伤害输出的符文")
GameplayTagList=(Tag="Rune.Type.Defense",     DevComment="【Type】护盾、减伤、生存相关的符文")
GameplayTagList=(Tag="Rune.Type.Utility",     DevComment="【Type】移速、冷却、能量类的功能性符文")
GameplayTagList=(Tag="Rune.Type.Summon",      DevComment="【Type】召唤实体或创建持续对象的符文")
```
...
#### 3.1.2 稀有度 `Rune.Rarity.*`

描述符文的**获取难度和强度区间**。每个符文必须有且只有一个 Rarity 标签。

```ini
; 填写规范：Rune.Rarity.<等级名>
; DevComment 格式：【Rarity】数值强度区间说明

GameplayTagList=(Tag="Rune.Rarity",           DevComment="符文稀有度，每个符文必须且只有一个")
GameplayTagList=(Tag="Rune.Rarity.Common",    DevComment="【Rarity】普通，基础数值，掉落频率高")
GameplayTagList=(Tag="Rune.Rarity.Rare",      DevComment="【Rarity】稀有，中等强度，有附加条件")
GameplayTagList=(Tag="Rune.Rarity.Epic",      DevComment="【Rarity】史诗，高强度或独特机制，稀少掉落")
```

---

#### 3.1.3 元素属性 `Rune.Element.*`（预留接口）

描述符文的**元素归属**，用于未来元素克制/共鸣系统。
**当前状态**：已定义节点，暂不赋予实际逻辑，可以安全填写标签但不需要接入系统。

```ini
; 填写规范：Rune.Element.<元素名>
; DevComment 格式：【Element-Reserved】说明当前未激活

GameplayTagList=(Tag="Rune.Element",              DevComment="元素属性（预留接口，当前无克制逻辑）")
GameplayTagList=(Tag="Rune.Element.Fire",         DevComment="【Element-Reserved】火元素")
GameplayTagList=(Tag="Rune.Element.Ice",          DevComment="【Element-Reserved】冰元素")
GameplayTagList=(Tag="Rune.Element.Lightning",    DevComment="【Element-Reserved】雷元素")
GameplayTagList=(Tag="Rune.Element.Void",         DevComment="【Element-Reserved】虚空/无属性")
```
...
GA_RuneShield_OnKill:
  BlockAbilitiesWithTag:
    Rune.Type.Defense     ← 触发时阻止同类型其他符文 GA 激活
```
...
| 场景 | 使用的 Tag 层 | 填写位置 |
|------|--------------|----------|
| 背包显示符文分类图标 | `Rune.Type.*` | RuneDataAsset → IdentityTags |
| 限制同类符文只能装一个 | `Rune.Type.*` | GA → BlockAbilitiesWithTag |
...
| 热度第三阶段符文加成 | `Buff.Status.Heat.Phase.3` | GA → RequireTags |
...
A：用 `Rune.Type.Attack`，在 Flow Graph 中查询 ASC 上是否有这个 Tag。
...
| 文件 | 管理范围 |
|------|----------|
| `Config/Tags/BuffTag.ini` | Buff/状态效果/数据通道 |
| `Config/Tags/RuneTag.ini` | 符文身份、触发、绑定和效果 ID |
