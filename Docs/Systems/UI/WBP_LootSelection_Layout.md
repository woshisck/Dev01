# WBP_LootSelection 布局规格

> 战利品三选一 / 多选一 UI（动态卡牌 N=1~5 + 跳过 + 背包预览）。
...
| 卡片来源 | C++ `RebuildCards(Options)` 用 `RuneCardClass` 动态创建 N≤5 张实例 AddChild 到 `CardContainer` |
...
└── Overlay（RootOverlay，Full Screen）
    ...
    └── VerticalBox（CenterColumn，HCenter VCenter）
        ...
        ├── HorizontalBox（CardContainer [BindWidget]，HCenter VCenter，PadB=24)
        │   └── (运行时由 C++ RebuildCards 动态填充 N≤5 张 WBP_RuneInfoCard，外包 UButton)
...
> **重要**：`CardContainer` **必须是 `UHorizontalBox` 类型**（不可用 WrapBox / VerticalBox / UPanelWidget 通配）—— C++ `BindWidget` 严格类型匹配，且 Slot cast 是 `UHorizontalBoxSlot`。
...
### HorizontalBox（CardContainer）⭐ BindWidget
...
- 单卡间距由 C++ 设置（`UHorizontalBoxSlot::SetPadding(8, 0, 8, 0)`，详见 `LootSelectionWidget.cpp:RebuildCards`）

> **若想给单卡更大间距 / 顶部对齐**，可在 C++ 里调，不在 WBP 里调
...
|---|---|---|---|
| `CardContainer` | **UHorizontalBox** | `CardContainer` | C++ `RebuildCards` 调 `ClearChildren` + `AddChild` 动态填充 |
...
`CardContainer` **必须是 `UHorizontalBox`**，`BtnSkip` / `BtnBackpackPreview` **必须是 `UButton`**（不是 `UCommonButtonBase` 等子类）。Common UI 的派生 Button 会因 C++ BindWidget 类型校验失败。
...
- 单卡 Padding 由 C++ 控制（默认左右各 8px），改要去 [LootSelectionWidget.cpp](../../../Source/DevKit/Private/UI/LootSelectionWidget.cpp) `RebuildCards` 函数
- 容器宽度由 N 张 WBP_RuneInfoCard 撑开，建议 RuneInfoCard 设固定宽度（如 320px）以保证 N=5 时不溢出
...
- 配 1 个 LootOption → ✅ 显示 1 张卡
- 配 3 个 LootOption → ✅ 显示 3 张卡
- 配 5 个 LootOption → ✅ 显示 5 张卡
- 配 7 个 LootOption → ✅ 显示 5 张卡 + Output Log 警告"截断到 5"
...
| 现象 | 原因 | 解决 |
|---|---|---|
| 编辑器编译报错 "BindWidget CardContainer not found" | WBP 没命名 / 名错 / 类型错 | 命名 `CardContainer`，类型必须 UHorizontalBox |
| PIE 弹 LootSelection 但没卡片 | RuneCardClass 没配 | Class Defaults 选 WBP_RuneInfoCard |
| 卡片重叠 / 跑屏外 | RuneInfoCard 没设固定宽度 / 卡数 N>5 但生成超 5 | 给 WBP_RuneInfoCard 加 SizeBox 限宽；GameMode 限制 LootOptions ≤ 5 |
