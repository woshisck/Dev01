...
| MUSKET-001 | 火绳枪 BP GAs：Light / Heavy / Sprint Attack | 用户 | [Musket_System_Guide](../../04_开发实现与系统文档/系统/Weapon/Musket_System_Guide.md) | C++ GA 框架已完成 |
| MUSKET-002 | 火绳枪 BP GAs：Reload_Single / Reload_All / Sprint_Reload | 用户 | [Musket_System_Guide](../../04_开发实现与系统文档/系统/Weapon/Musket_System_Guide.md) | C++ GA 框架已完成 |
| MUSKET-003 | GE 伤害 + 弹药 Attribute 初始值 | 用户 | [Musket_System_Guide](../../04_开发实现与系统文档/系统/Weapon/Musket_System_Guide.md) | - |
...
| VFX-004 | 所有**敌人**攻击蒙太奇：前摇帧拖入 `ANS Pre Attack Flash` | 用户 | [CharacterFlash_Technical](../../04_开发实现与系统文档/系统/VFX/CharacterFlash_Technical.md) | ANS 类已完成 |
| VFX-005 | 玩家攻击蒙太奇命中帧：添加 `AN Hit Stop` + 配置参数 | 用户 | 轻攻 F=50ms；重攻 F=80ms/S=120ms@25% | AN 类已完成 |
| RUNE-P0-1 | 1017 符文 FA 制作（高感知测试符文） | 用户 | [TestRune_HighPerception_Guide](../../04_开发实现与系统文档/系统/Rune/TestRune_HighPerception_Guide.md) | - |
...
| DATA-P0-DONE | 数值统一访问器 + RuneIdTag 字段 + `Tools > DataEditor` C++菜单面板 + DataEditor 脚本工具 + Tag .ini ✅ 已完成 | Claude/Codex | [DataAuthoring](../../04_开发实现与系统文档/编码规范/DataAuthoring.md) · [DataEditor测试说明](../../03_策划配置与制作手册/核心配置说明/编辑器工具/DataEditor数值编辑器测试说明.md) | - |
...
| ID | 任务 | 执行方 | 参考文档 | 依赖 |
|---|---|---|---|---|
| VFX-006 | 所有**敌人** BP Details → Combat\|Visual → `CharacterFlashMaterial` 填入 Overlay 材质 | 用户 | [CharacterFlash_Technical](../../04_开发实现与系统文档/系统/VFX/CharacterFlash_Technical.md) | - |
...
| UI-001 | WBP_WeaponGlassIcon + WeaponGlassAnimDA 编辑器配置 | 用户 | [WeaponPickupAnim_Technical](../../04_开发实现与系统文档/系统/UI/WeaponPickupAnim_Technical.md) | - |
| UI-002 | WBP_WeaponTrail 创建 | 用户 | 同上 | - |
| CONTENT-001 | 献祭恩赐 FA（In → BFNode_SacrificeDecay）+ BP_SacrificePickup | 用户 + Claude | [EditorSetup_ChainAndSacrifice](../../99_归档/TODO/EditorSetup_ChainAndSacrifice.md) | - |
...
| ID | 任务 | 执行方 | 备注 |
|---|---|---|---|
| RUNE-VFX | 符文表现层：16 个 GameplayCue + Niagara + SFX + 浮字 | 用户 + Claude | [RuneMaster_ProductionGuide](../../04_开发实现与系统文档/系统/Rune/RuneMaster_ProductionGuide.md) |
| RUNE-FA | 符文逻辑层 FA：1001–1016 | 用户 | [TestRune_CreationGuide](../../04_开发实现与系统文档/系统/Rune/TestRune_CreationGuide.md) |
