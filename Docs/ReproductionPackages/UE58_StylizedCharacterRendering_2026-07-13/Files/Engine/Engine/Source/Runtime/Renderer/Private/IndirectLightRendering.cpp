...
static TAutoConsoleVariable<int32> CVarStylizedReflectionKuwaharaEnable(
	TEXT("r.StylizedReflection.Kuwahara.Enable"),
	-1,
	TEXT("Character reflection Kuwahara filter. -1: automatic (Epic/Cinematic), 0: off, 1: on."),
	ECVF_RenderThreadSafe | ECVF_Scalability);

static TAutoConsoleVariable<float> CVarStylizedReflectionKuwaharaStrength(
	TEXT("r.StylizedReflection.Kuwahara.Strength"),
	1.0f,
	TEXT("Blend strength for the character-only indirect reflection Kuwahara filter."),
	ECVF_RenderThreadSafe);
...
class FDiffuseIndirectCompositePS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(FVector4f, StylizedCharacterLightingParams)
		SHADER_PARAMETER(FVector4f, StylizedCharacterProfileMeta)
		SHADER_PARAMETER_ARRAY(FVector4f, StylizedCharacterProfileData, [88])
		SHADER_PARAMETER(FVector4f, StylizedReflectionKuwaharaParams)
...
void SetupReflectionUniformParameters(FRDGBuilder& GraphBuilder, const FViewInfo& View, FReflectionUniformParameters& OutParameters)
{
	...
	{
		FVisualizeTexture::AddVisualizeTexturePass(GraphBuilder, GetGlobalShaderMap(GMaxRHIFeatureLevel), GraphBuilder.RegisterExternalTexture(Scene->ReflectionSceneData.CubemapArray.GetRenderTarget()), /*bUseConfig=*/ true);
	...
	FAsyncLumenIndirectLightingOutputs& AsyncLumenIndirectLightingOutputs)
{
	...
	for (int32 ViewIndex = 0; ViewIndex < Views.Num(); ++ViewIndex)
	{
		...
		auto ApplyDiffuseIndirect = [&](ESubstrateTileType TileType)
		{
			...
			PSParameters->StylizedCharacterLightingParams = GetStylizedCharacterLightingParams();
			uint32 StylizedCharacterProfileCount;
			TStaticArray<FVector4f, StylizedCharacterLighting::TotalFloat4s> StylizedCharacterProfileData;
			GetStylizedCharacterLightingProfiles(StylizedCharacterProfileData, StylizedCharacterProfileCount);
			...
			PSParameters->StylizedCharacterProfileMeta = FVector4f((float)StylizedCharacterProfileCount, 0.0f, (float)FMath::Max(StylizedCharacterProfileOverride, 0), StylizedCharacterProfileOverrideBlend);
			for (uint32 ProfileDataIndex = 0; ProfileDataIndex < StylizedCharacterLighting::TotalFloat4s; ++ProfileDataIndex)
			{
				PSParameters->StylizedCharacterProfileData[ProfileDataIndex] = StylizedCharacterProfileData[ProfileDataIndex];
			}
			...
			const int32 EffectsQuality = EffectsQualityCVar ? EffectsQualityCVar->GetValueOnRenderThread() : 3;
			const int32 KuwaharaMode = CVarStylizedReflectionKuwaharaEnable.GetValueOnRenderThread();
			const bool bEnableKuwahara = KuwaharaMode > 0 || (KuwaharaMode < 0 && EffectsQuality >= 3);
			const float KuwaharaRadius = EffectsQuality >= 4 ? 2.0f : 1.0f;
			const float KuwaharaSampleStride = EffectsQuality == 3 ? 2.0f : 1.0f;
			PSParameters->StylizedReflectionKuwaharaParams = FVector4f(
				bEnableKuwahara ? 1.0f : 0.0f,
				KuwaharaRadius,
				FMath::Clamp(CVarStylizedReflectionKuwaharaStrength.GetValueOnRenderThread(), 0.0f, 1.0f),
				KuwaharaSampleStride);
