	...
	: Super(ObjectInitializer)
{
	...
	LightComponentId = FLightComponentId::GetNextId();
}
	...
	: Super(ObjectInitializer)
{
	...
	SpecularScale = 1.0f;
	DiffuseScale = 1.0f;
	StylizedCharacterLightMode = EStylizedCharacterLightMode::Toon;
	StylizedCharacterShadowStrength = 1.0f;
	StylizedCharacterRimPower = 4.0f;
	StylizedCharacterRimIntensity = 1.0f;
...
bool ULightComponent::CastsStaticShadow() const
{
	return CastShadows && CastStaticShadows;
}

bool ULightComponent::CastsDynamicShadow() const
{
	return CastShadows && CastDynamicShadows;
}

bool ULightComponent::TransmitsThroughSurfaces() const
{
	return bTransmission && CastsDynamicShadow() && !HasStaticShadowing();
}

bool ULightComponent::GetContactShadowLengthInWS() const
{
	return ContactShadowLengthInWS ? true : false;
}

float ULightComponent::GetRayEndBias() const
{
	return bOverrideRayEndBias ? FMath::Max(RayEndBias, 0.0f) : -1.0f;
}

const FStaticShadowDepthMap* ULightComponent::GetStaticShadowDepthMap() const
{
	return &StaticShadowDepthMap;
}
...
void ULightComponent::PrecachePSOs()
{
	...
	{
		LightFunctionMaterial->ConditionalPostLoad();

		FMaterialInterfacePSOPrecacheParams Params;
		Params.Priority = EPSOPrecachePriority::High; //< No boosting right now - can be added
		Params.MaterialInterface = LightFunctionMaterial;
		...
		PrecacheMaterialPSOs( { Params }, MaterialPSOPrecacheRequestIDs, PSOPrecacheCompileEvents);
...
void ULightComponent::BuildSceneProxyDesc(const ULightComponent& InLightComponent, FLightSceneProxyDesc& OutProxyDesc)
{
	const FLightComponentMapBuildData* MapBuildData = InLightComponent.GetLightComponentMapBuildData();

	OutProxyDesc.LightComponent = &InLightComponent;
	OutProxyDesc.LightComponentId = InLightComponent.GetLightComponentId();
	OutProxyDesc.SceneInterface = InLightComponent.GetScene();
#if ACTOR_HAS_LABELS
	OutProxyDesc.OwnerNameOrLabel = InLightComponent.GetOwner() ? InLightComponent.GetOwner()->GetActorNameOrLabel() : InLightComponent.GetName();
#endif
	OutProxyDesc.bSelected = InLightComponent.IsSelected() || InLightComponent.IsOwnerSelected();
	OutProxyDesc.bContactShadowLengthInWS = InLightComponent.GetContactShadowLengthInWS();
	OutProxyDesc.bMovable = InLightComponent.IsMovable();
	OutProxyDesc.bStaticLighting = InLightComponent.HasStaticLighting();
	OutProxyDesc.bStaticShadowing = InLightComponent.HasStaticShadowing();
	OutProxyDesc.bCastDynamicShadow = InLightComponent.CastsDynamicShadow();
	OutProxyDesc.bCastStaticShadow = InLightComponent.CastsStaticShadow();
	OutProxyDesc.bCastTranslucentShadows = InLightComponent.CastTranslucentShadows;
	OutProxyDesc.bTransmission = InLightComponent.TransmitsThroughSurfaces();
	OutProxyDesc.bCastVolumetricShadow = InLightComponent.bCastVolumetricShadow;
	OutProxyDesc.bCastHairStrandsDeepShadow = InLightComponent.bCastDeepShadow;
	OutProxyDesc.bCastShadowsFromCinematicObjectsOnly = InLightComponent.bCastShadowsFromCinematicObjectsOnly;
	OutProxyDesc.bForceCachedShadowsForMovablePrimitives = InLightComponent.bForceCachedShadowsForMovablePrimitives;
	OutProxyDesc.bAffectReflection = InLightComponent.bAffectReflection;
	OutProxyDesc.bAffectGlobalIllumination = InLightComponent.bAffectGlobalIllumination;
	OutProxyDesc.bAffectTranslucentLighting = InLightComponent.bAffectTranslucentLighting;
	OutProxyDesc.bUsedAsAtmosphereSunLight = InLightComponent.IsUsedAsAtmosphereSunLight();
	OutProxyDesc.bUseRayTracedDistanceFieldShadows = InLightComponent.bUseRayTracedDistanceFieldShadows;
	OutProxyDesc.bAllowMegaLights = InLightComponent.bAllowMegaLights;
	OutProxyDesc.bIsPrecomputedLightingValid = InLightComponent.IsPrecomputedLightingValid();
	OutProxyDesc.bEnableLightShaftBloom = InLightComponent.bEnableLightShaftBloom;
	OutProxyDesc.Color = InLightComponent.GetColoredLightBrightness();
	OutProxyDesc.IndirectLightingScale = InLightComponent.IndirectLightingIntensity;
	OutProxyDesc.VolumetricScatteringIntensity = FMath::Max(InLightComponent.VolumetricScatteringIntensity, 0.0f);
	OutProxyDesc.RayEndBias = InLightComponent.GetRayEndBias();
	OutProxyDesc.ShadowResolutionScale = InLightComponent.ShadowResolutionScale;
	OutProxyDesc.ShadowBias = InLightComponent.ShadowBias;
	OutProxyDesc.ShadowSlopeBias = InLightComponent.ShadowSlopeBias;
	OutProxyDesc.ShadowSharpen = InLightComponent.ShadowSharpen;
	OutProxyDesc.ContactShadowLength = InLightComponent.ContactShadowLength;
	OutProxyDesc.ContactShadowCastingIntensity = InLightComponent.ContactShadowCastingIntensity;
	OutProxyDesc.ContactShadowNonCastingIntensity = InLightComponent.ContactShadowNonCastingIntensity;
	OutProxyDesc.SpecularScale = InLightComponent.SpecularScale;
	OutProxyDesc.DiffuseScale = InLightComponent.DiffuseScale;
	OutProxyDesc.StylizedCharacterLightMode = (uint8)InLightComponent.StylizedCharacterLightMode;
	OutProxyDesc.StylizedCharacterShadowStrength = FMath::Clamp(InLightComponent.StylizedCharacterShadowStrength, 0.0f, 1.0f);
	OutProxyDesc.StylizedCharacterRimPower = FMath::Max(InLightComponent.StylizedCharacterRimPower, 0.25f);
	OutProxyDesc.StylizedCharacterRimIntensity = FMath::Max(InLightComponent.StylizedCharacterRimIntensity, 0.0f);
	OutProxyDesc.LightGuid = InLightComponent.LightGuid;
	OutProxyDesc.MapBuildDataShadowMapChannel = MapBuildData ? MapBuildData->ShadowMapChannel : INDEX_NONE;
	OutProxyDesc.PreviewShadowMapChannel = InLightComponent.PreviewShadowMapChannel;
	OutProxyDesc.RayStartOffsetDepthScale = InLightComponent.RayStartOffsetDepthScale;
	OutProxyDesc.LightFunctionScale = InLightComponent.LightFunctionScale;
	OutProxyDesc.LightFunctionFadeDistance = InLightComponent.LightFunctionFadeDistance;
	OutProxyDesc.LightFunctionDisabledBrightness = InLightComponent.DisabledBrightness;
	OutProxyDesc.LightFunctionMaterial = InLightComponent.LightFunctionMaterial;
	OutProxyDesc.IESTexture = InLightComponent.IESTexture;
	OutProxyDesc.CastRaytracedShadow = InLightComponent.GetCastRaytracedShadow();
	OutProxyDesc.BloomScale = InLightComponent.BloomScale;
	OutProxyDesc.BloomThreshold = InLightComponent.BloomThreshold;
	OutProxyDesc.BloomMaxBrightness = InLightComponent.BloomMaxBrightness;
	OutProxyDesc.BloomTint = InLightComponent.BloomTint;
	OutProxyDesc.MegaLightsShadowMethod = InLightComponent.MegaLightsShadowMethod;
	OutProxyDesc.AtmosphereSunLightIndex = InLightComponent.GetAtmosphereSunLightIndex();
	OutProxyDesc.AtmosphereSunDiskColorScale = InLightComponent.GetAtmosphereSunDiskColorScale();
	OutProxyDesc.LightType = InLightComponent.GetLightType();
	OutProxyDesc.LightingChannelMask = GetLightingChannelMaskForStruct(InLightComponent.LightingChannels);
	OutProxyDesc.ViewLightingChannelMask = InLightComponent.ViewLightingChannels.GetMaskForStruct();
	OutProxyDesc.ComponentName = InLightComponent.GetFName();
	OutProxyDesc.LevelName = InLightComponent.GetOwner() ? InLightComponent.GetOwner()->GetLevel()->GetOutermost()->GetFName() : NAME_None;
	OutProxyDesc.SamplesPerPixel = InLightComponent.SamplesPerPixel;
	OutProxyDesc.DeepShadowLayerDistribution = InLightComponent.DeepShadowLayerDistribution;
}
		...
		if (Light->Mobility != EComponentMobility::Static)
		{
			...
			for (int32 ArgIndex = 0; ArgIndex < Args.Num(); ++ArgIndex)
			{
				...
				if (LightName.Contains(ToggleName) )
				{
					...
					UE_LOGF(LogConsoleResponse,Display,"Now%lsvisible: %ls",
