...
/** How this light contributes to MSM_StylizedCharacterLit surfaces. */
UENUM(BlueprintType)
enum class EStylizedCharacterLightMode : uint8
{
	Wash UMETA(DisplayName = "0 - Wash (No Shadow)"),
	Toon UMETA(DisplayName = "1 - Toon Ramp"),
	Rim UMETA(DisplayName = "2 - Rim Only"),
};
...
class ULightComponent : public ULightComponentBase
{
	...
	*/
	...
	/** World distance to where the light stops rendering (lighting & shadows). 0.0 or less means no max draw distance. This can be controlled via a global scalar scalability setting r.LightMaxDrawDistanceScale as well */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Performance)
	float MaxDrawDistance;

	/** Fade the light out as it approaches MaxDrawDistance. The fading starts when the light is MaxDistanceFadeRange distance away from MaxDrawDistance, where it ends up at 0 for a smooth fade-out without popping. Requires MaxDrawDistance to be set. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Performance, meta = (EditCondition = "MaxDrawDistance > 0.0"))
	...
	*/
	...
	/** Per-light behavior used only when shading MSM_StylizedCharacterLit. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Light|Stylized Character")
	EStylizedCharacterLightMode StylizedCharacterLightMode;

	/** Blends scene shadowing for Toon mode. 0 ignores the shadow map, 1 uses it fully. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Light|Stylized Character", meta = (ClampMin = "0.0", ClampMax = "1.0"))
	float StylizedCharacterShadowStrength;

	/** Fresnel exponent used by Rim Only mode. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Light|Stylized Character", meta = (ClampMin = "0.25", UIMin = "0.25", UIMax = "16.0"))
	float StylizedCharacterRimPower;

	/** Edge-light multiplier used by Rim Only mode. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Light|Stylized Character", meta = (ClampMin = "0.0", UIMin = "0.0", UIMax = "8.0"))
	float StylizedCharacterRimIntensity;
	...
	*/
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Light, meta=(InlineEditConditionToggle), AdvancedDisplay)
	uint32 bOverrideRayEndBias : 1;

	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = Light, meta = (EditCondition = "bOverrideRayEndBias", ClampMin = "0", UIMax = "100"), AdvancedDisplay)
	float RayEndBias;
...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "Rendering|Components|Light")
	ENGINE_API void SetMaxDrawDistance(float NewValue);

	UFUNCTION(BlueprintCallable, Category = "Rendering|Components|Light")
	ENGINE_API void SetMaxDistanceFadeRange(float NewValue);
...
public:
	...
	*/
	...
	ENGINE_API bool CastsStaticShadow() const;
	ENGINE_API bool CastsDynamicShadow() const;
	ENGINE_API bool TransmitsThroughSurfaces() const;
	ENGINE_API bool GetContactShadowLengthInWS() const;
	ENGINE_API float GetRayEndBias() const;
	ENGINE_API const FStaticShadowDepthMap* GetStaticShadowDepthMap() const;

	inline static FLightSceneProxyDesc BuildSceneProxyDesc(const ULightComponent& InLightComponent)
	{
		FLightSceneProxyDesc ProxyDesc;
		BuildSceneProxyDesc(InLightComponent, ProxyDesc);
		return ProxyDesc;
	}

protected:

	ENGINE_API static void BuildSceneProxyDesc(const ULightComponent& InLightComponent, FLightSceneProxyDesc& OutProxyDesc);
	...
	void UpdateProxy(FUpdateLightProxyCallback&& Func);
	...
	{
		return PSOPrecacheComponentData.IsPSOPrecaching();
	}
