...
{
	MaterialFlagsThatHaveChanged |= (1u << UsageFlag);
}

bool FMaterialsWithDirtyUsageFlags::IsUsageFlagDirty(EMaterialUsage UsageFlag)
{
	return (MaterialFlagsThatHaveChanged & (1u << UsageFlag)) != 0;
}
...
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FMaterialResource::BuildShaderMapIdOverride);
	...
	OutId.ExternalCodeReferencesHash = CachedData.GetExternalCodeReferencesHash();

	OutId.TextureReferencesHash = Material->GetForceRecompileTextureIdsHash();

	if (MaterialInstance)
	{
		OutId.BasePropertyOverridesHash = MaterialInstance->GetBasePropertyOverridesHash();
...
public:
	...
	virtual bool GetParameterValue(EMaterialParameterType Type, const FHashedMaterialParameterInfo& ParameterInfo, FMaterialParameterValue& OutValue, const FMaterialRenderContext& Context) const override
	{
		...
		if (MaterialResource && MaterialResource->GetRenderingThreadShaderMap())
		{
			...
			else if (Type == EMaterialParameterType::Scalar && NumToonProfileRT() > 0)
			{
				for (uint32 It=0,Count=NumToonProfileRT();It<Count;++It)
				{
					if (ParameterInfo.Name == ToonProfile::GetToonProfileParameterName(GetToonProfileRT(It)))
					{
						const UToonProfile* ToonProfile = GetToonProfileRT(It);
						const UToonProfile* ToonProfileOverride = GetToonProfileOverrideRT();
						OutValue = ToonProfile::GetToonProfileId(ToonProfileOverride ? ToonProfileOverride : ToonProfile);
						return true;
					}
				}
			}
...
static bool       GDefaultMaterialsInitialized[MD_MAX] = {false};
...
static bool bDefaultMaterialInitializationStarted = false;
...
UMaterial* TryResolveDefaultMaterial(int32 Domain)
{
	FString ResolvedPath = ResolveIniObjectsReference(GDefaultMaterialNames[Domain]);
	return FindObject<UMaterial>(nullptr, *ResolvedPath);
}
	...
	void ConditionalFinishInitialization(int32 Domain)
	{
		if (!bDefaultMaterialInitialized)
		{
			...
			if (!GDefaultMaterialsInitialized[Domain])
			{
				...
				GDefaultMaterialsInitialized[Domain] = true;
			}

			bool bAllDefaultMaterialInitialized = true;
			for (int32 Index = 0; Index < MD_MAX; ++Index)
			{
				bAllDefaultMaterialInitialized &= GDefaultMaterialsInitialized[Index];
			}
	
			if (bAllDefaultMaterialInitialized)
			{
				bDefaultMaterialInitialized = true;
				...
				// Now precache PSOs for MD_Surface default material after the default materials are marked initialize
				...
				{
					UMaterialInterface::PrecacheFallbackMaterialPSOs(GDefaultMaterials[MD_Surface]);
				}
			}
		}
	}
...
void UMaterialInterface::InitDefaultMaterials()
{
	...
	if (!bDefaultMaterialInitializationStarted)
	{
		SCOPED_BOOT_TIMING("UMaterialInterface::InitDefaultMaterials");
		...
		// We only do this step once, launch async loading tasks with top priority to fullfill these requests as soon as possible.
		// When done loading, these will call ConditionalFinishInitialization which handles the remaining of the initialization.
		bDefaultMaterialInitializationStarted = true;
		...
		for (int32 Domain = 0; Domain < MD_MAX; ++Domain)
		{
			...
			Params.PackagePriority = MAX_int32;
			Params.CompletionDelegate = MakeUnique<FLoadPackageAsyncDelegate>(FLoadPackageAsyncDelegate::CreateLambda(
				[Domain](const FName& PackageName, UPackage* Package, EAsyncLoadingResult::Type Result)
				{
					MaterialImpl::ConditionalFinishInitialization(Domain);
				}));

			LoadPackageAsync(FPackagePath::FromPackageNameChecked(FTopLevelAssetPath(ResolvedPath).GetPackageName()), MoveTemp(Params));
		}
	}
}
...
{
	PrecacheFallbackMaterialPSOs(GDefaultMaterials[MD_Surface]);
}

void UMaterialInterface::PrecacheFallbackMaterialPSOs(UMaterialInterface* MaterialInterface)
{
	if (!GIsRHIInitialized || (MaterialInterface == nullptr) || !UPSOPrecacheSettingsManager::GetSettings().bAllDefaultMaterialPSOs)
	{
		return;
	}
	...
	PrecachePSOParams.bRequireRTDynamicGeometryUpdate = true;
	...
	auto PrecachePSOs = [ & ](EComponentMobility::Type Mobility, bool bReverseCulling)
	{
		PrecachePSOParams.Mobility = Mobility;
		PrecachePSOParams.bReverseCulling = bReverseCulling;
		MaterialInterface->PrecachePSOs(AllVertexFactoryTypes, PrecachePSOParams, EPSOPrecachePriority::High, MaterialPrecacheRequestIDs);
	};

	PrecachePSOs(EComponentMobility::Static, false /*bReverseCulling*/);
	PrecachePSOs(EComponentMobility::Movable, false /*bReverseCulling*/);
	PrecachePSOs(EComponentMobility::Static, true /*bReverseCulling*/);
	PrecachePSOs(EComponentMobility::Movable, true /*bReverseCulling*/);
}
...
void UMaterialInterface::PostLoadDefaultMaterials()
{
	...
	static TSet<UMaterialInterface*> OnStack;
	if (!bPostLoaded)
	{
		check(IsInGameThread());
		bPostLoaded = true;
		...
		if (!bDefaultMaterialInitialized)
		{
			TRACE_CPUPROFILER_EVENT_SCOPE(UMaterialInterface::PostLoadDefaultMaterials);

			check(IsInGameThread());
			OnStack.Add(this);
			ON_SCOPE_EXIT { OnStack.Remove(this); };

			for (int32 Domain = 0; Domain < MD_MAX; ++Domain)
			{
				UMaterial* Material = TryResolveDefaultMaterial(Domain);
				if (OnStack.Contains(Material))
				{
					// We obviously can't sync load ourselves while we're in our own postload function
					continue;
				}

				// This will forceload the object if it hasn't already been loaded.
				// Sync loading from inside PostLoad is supported by the loader.
				Material = UMaterial::GetDefaultMaterial((EMaterialDomain)Domain);
				check(Material);
				if (Material)
				{
					// The only reason these would fail is if there is a recursive loads.. (i.e. a material trying to load and postload itself indirectly since we already handle the direct case)
					check(!Material->HasAnyFlags(RF_NeedLoad));
					check(!Material->HasAnyFlags(RF_NeedPostLoad));
				}
			}
		}
	}
...
UMaterial* UMaterial::GetDefaultMaterial(EMaterialDomain Domain)
{
	...
	{
		FString ResolvedPath = ResolveIniObjectsReference(GDefaultMaterialNames[Domain]);
		LoadObject<UMaterial>(nullptr, *ResolvedPath, nullptr, LOAD_DisableDependencyPreloading, nullptr);
		MaterialImpl::ConditionalFinishInitialization(Domain);
	}

	check(GDefaultMaterials[Domain] != nullptr);
...
bool UMaterial::IsDefaultMaterial() const
{
	...
	if (!bDefaultMaterialInitialized)
	{
		for (int32 Domain = 0; !bDefault && Domain < MD_MAX; ++Domain)
		{
			bDefault = (this == TryResolveDefaultMaterial(Domain));
		}
	}
...
void UMaterial::GetUsedTextures(TArray<UTexture*>& OutTextures, TOptional<EMaterialQualityLevel::Type> InSpecificQualityLevel, TOptional<EShaderPlatform> InSpecificShaderPlatform) const
{
	...
	{
		const FInt32Range QualityLevelRange = BuildQualityLevelRange(InSpecificQualityLevel);
		const FInt32Range ShaderPlatformRange = BuildShaderPlatformRange(InSpecificShaderPlatform);

		TArray<const FMaterialResource*, TInlineAllocator<4>> MatchedResources = GetMaterialResources(this, QualityLevelRange, ShaderPlatformRange);
		for (const FMaterialResource* CurrentResource : MatchedResources)
		{
			GetTextureExpressionValues(CurrentResource, OutTextures, nullptr, true, false);
		}
	}
}
...
void UMaterial::GetUsedTexturesAndIndices(TArray<UTexture*>& OutTextures, TArray< TArray<int32> >& OutIndices, EMaterialQualityLevel::Type QualityLevel, EShaderPlatform InShaderPlatform) const
{
	...
	if (!FPlatformProperties::IsServerOnly())
	{
		...
		if (CurrentResource)
		{
			GetTextureExpressionValues(CurrentResource, OutTextures, &OutIndices, false, false);
		}
	}
}
...
uint32 UMaterial::GetUsageFlags() const
{
	uint32 UsageFlags = 0;
	PRAGMA_DISABLE_DEPRECATION_WARNINGS
	UsageFlags |= bUsedWithSkeletalMesh ? (1u << MATUSAGE_SkeletalMesh) : 0;
	UsageFlags |= bUsedWithParticleSprites ? (1u << MATUSAGE_ParticleSprites) : 0;
	UsageFlags |= bUsedWithBeamTrails ? (1u << MATUSAGE_BeamTrails) : 0;
	UsageFlags |= bUsedWithMeshParticles ? (1u << MATUSAGE_MeshParticles) : 0;
	UsageFlags |= bUsedWithNiagaraSprites ? (1u << MATUSAGE_NiagaraSprites) : 0;
	UsageFlags |= bUsedWithNiagaraRibbons ? (1u << MATUSAGE_NiagaraRibbons) : 0;
	UsageFlags |= bUsedWithNiagaraMeshParticles ? (1u << MATUSAGE_NiagaraMeshParticles) : 0;
	UsageFlags |= bUsedWithStaticLighting ? (1u << MATUSAGE_StaticLighting) : 0;
	UsageFlags |= bUsedWithMorphTargets ? (1u << MATUSAGE_MorphTargets) : 0;
	UsageFlags |= bUsedWithSplineMeshes ? (1u << MATUSAGE_SplineMesh) : 0;
	UsageFlags |= bUsedWithInstancedStaticMeshes ? (1u << MATUSAGE_InstancedStaticMeshes) : 0;
	UsageFlags |= bUsedWithGeometryCollections ? (1u << MATUSAGE_GeometryCollections) : 0;
	UsageFlags |= bUsedWithClothing ? (1u << MATUSAGE_Clothing) : 0;
	UsageFlags |= bUsedWithGeometryCache ? (1u << MATUSAGE_GeometryCache) : 0;
	UsageFlags |= bUsedWithWater ? (1u << MATUSAGE_Water) : 0;
	UsageFlags |= bUsedWithHairStrands ? (1u << MATUSAGE_HairStrands) : 0;
	UsageFlags |= bUsedWithLidarPointCloud ? (1u << MATUSAGE_LidarPointCloud) : 0;
	UsageFlags |= bUsedWithVirtualHeightfieldMesh ? (1u << MATUSAGE_VirtualHeightfieldMesh) : 0;
	UsageFlags |= bUsedWithNanite ? (1u << MATUSAGE_Nanite) : 0;
	UsageFlags |= bUsedWithVoxels ? (1u << MATUSAGE_Voxels) : 0;
	UsageFlags |= bUsedWithCurves ? (1u << MATUSAGE_Curves) : 0;
	UsageFlags |= bUsedWithVolumetricCloud ? (1u << MATUSAGE_VolumetricCloud) : 0;
	UsageFlags |= bUsedWithHeterogeneousVolumes ? (1u << MATUSAGE_HeterogeneousVolumes) : 0;
	UsageFlags |= bUsedWithStaticMesh ? (1u << MATUSAGE_StaticMesh) : 0;
	UsageFlags |= bUsedWithEditorCompositing ? (1u << MATUSAGE_EditorCompositing) : 0;
	UsageFlags |= bUsedWithNeuralNetworks ? (1u << MATUSAGE_NeuralNetworks) : 0;
	UsageFlags |= bUsedWithMeshDeformer ? (1u << MATUSAGE_MeshDeformer) : 0;
	UsageFlags |= bUsedWithInstancedSkinnedMesh ? (1u << MATUSAGE_InstancedSkinnedMesh) : 0;
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	return UsageFlags;
}
...
bool UMaterial::GetUsageByFlag(EMaterialUsage Usage) const
{
	...
	switch(Usage)
	{
		...
		case MATUSAGE_Curves: UsageValue = bUsedWithCurves; break;
		...
		case MATUSAGE_EditorCompositing: UsageValue = bUsedWithEditorCompositing; break;
		case MATUSAGE_NeuralNetworks: UsageValue = bUsedWithNeuralNetworks; break;
		case MATUSAGE_MeshDeformer: UsageValue = bUsedWithMeshDeformer; break;
		case MATUSAGE_InstancedSkinnedMesh: UsageValue = bUsedWithInstancedSkinnedMesh; break;
		default: UE_LOGF(LogMaterial, Fatal,"Unknown material usage: %u", (int32)Usage);
	};
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
	return UsageValue;
}
...
void UMaterial::SetUsageByFlag(EMaterialUsage Usage, bool NewValue)
{
	...
	switch(Usage)
	{
		...
		case MATUSAGE_Curves:
		{
			bUsedWithCurves = NewValue; break;
		}
		...
		case MATUSAGE_EditorCompositing:
		{
			bUsedWithEditorCompositing = NewValue; break;
		}
		case MATUSAGE_NeuralNetworks:
		{
			bUsedWithNeuralNetworks = NewValue; break;
		}
		case MATUSAGE_MeshDeformer:
		{
			bUsedWithMeshDeformer = NewValue; break;
		}
		case MATUSAGE_InstancedSkinnedMesh:
		{
			bUsedWithInstancedSkinnedMesh = NewValue; break;
		}
		default: UE_LOGF(LogMaterial, Fatal, "Unknown material usage: %u", (int32)Usage);
	};
	PRAGMA_ENABLE_DEPRECATION_WARNINGS
...
bool UMaterial::SetMaterialUsage(EMaterialUsage Usage)
{
	...
	if (!GetUsageByFlag(Usage) && !bUsedAsSpecialEngineMaterial)
	{
		...
		// 
		// Using FApp::IsGame is not enough, during UWorld::SendAllEndOfFrameUpdates() it returns false even it PIE is running.
		// Ideally, FApp::IsGame should be revisited, but to avoid causing regressions, we also check IsPlayingSessionInEditor.
		...
		if (GIsEditor && !bIsGame && bAutomaticallySetUsageInEditor)
		{
			...
			{
				UE_LOGF(LogMaterial, Display, "Material %ls needed to set usage flag %ls", *GetPathName(), *GetUsageName(Usage));
			}
...
#if WITH_EDITOR
			// Notify any open material editors.
			FEditorSupportDelegates::MaterialUsageFlagsChanged.Broadcast(this, Usage);
#endif
		}
		else
		{
			const uint32 UsageFlagBit = (1u << Usage);
			if ((UsageFlagWarnings & UsageFlagBit) == 0)
			{
				UE_LOGF(LogMaterial, Warning, "Material %ls missing usage flag %ls! Default Material will be used in game.", *GetPathName(), *GetUsageName(Usage));

				if (bAutomaticallySetUsageInEditor)
				{
					UE_LOGF(LogMaterial, Warning, "     The material will recompile every editor launch until resaved.");
				}
#if WITH_EDITOR
...
void BuildExpressionParametersGuidsMapRecursive(TConstArrayView<TObjectPtr<UMaterialExpression>> InMaterialExpression, TMap<FGuid, TObjectPtr<UMaterialExpression>>& OutExpressionMap)
{
	const int32 NumExpressions = InMaterialExpression.Num();
	for (int32 ExpressionIndex = 0; ExpressionIndex < NumExpressions; ++ExpressionIndex)
	{
		...
		if (ExpressionPtr)
		{
			const FGuid ExpressionGuid = ExpressionPtr->GetParameterExpressionId();
			if (ExpressionPtr->bIsParameterExpression && !OutExpressionMap.Contains(ExpressionGuid))
			{
				OutExpressionMap.Add(ExpressionGuid, TObjectPtr<UMaterialExpression>(ExpressionPtr));
			}
		}
		else if (UMaterialFunctionInterface* MaterialFunction = UMaterial::GetExpressionFunctionPointer(ExpressionPtr))
		{
			BuildExpressionParametersGuidsMapRecursive(MaterialFunction->GetExpressions(), OutExpressionMap);
		}
		else if (TOptional<UMaterial::FLayersInterfaces> LayersInterfaces = UMaterial::GetExpressionLayers(ExpressionPtr))
		{
			for (const UMaterialFunctionInterface* Layer : LayersInterfaces->Layers)
			{
				if (Layer)
				{
					BuildExpressionParametersGuidsMapRecursive(Layer->GetExpressions(), OutExpressionMap);
				}
			}

			for (const UMaterialFunctionInterface* Blend : LayersInterfaces->Blends)
			{
				if (Blend)
				{
					BuildExpressionParametersGuidsMapRecursive(Blend->GetExpressions(), OutExpressionMap);
				}
			}
		}
	}
...
void UMaterial::BuildExpressionParametersGuidsMap(TMap<FGuid, TObjectPtr<UMaterialExpression>>& OutExpressionMap)
{
	BuildExpressionParametersGuidsMapRecursive(GetExpressions(), OutExpressionMap);
}
...
void UMaterialInterface::OverrideBlendableSettings(class FSceneView& View, float Weight) const
{
	...
	if (View.IsRuntimeReflectionCapture())
	{
		// Runtime reflection captures support applying a post process material, even when no View.State is set.
		// These can't use a MID because they have no View.State to store it on, but since it's a singleton material,
		// it can't blend, and so it doesn't need one.  The "const_cast" is unavoidable here, as FPostProcessMaterialNode
		// supports modification of the material interface when it's a MID, forcing the member to be non-const, but it
		// won't actually be modified when not a MID.
		FPostProcessMaterialNode MaterialNode(const_cast<UMaterialInterface*>(this), GetBlendableLocation(Base), 0, false);
		Dest.BlendableManager.PushBlendableData(1.0f, MaterialNode);
		return;
	}
	else if(!View.State)
	{
...
void UMaterial::UpdateCachedExpressionData()
{
	...
	{
		const FMaterialCachedExpressionData& UsedCachedData = GetCachedExpressionData();
		// Try to always use Editor Only Data textures in Editor, if it exists
		if (UsedCachedData.EditorOnlyData->EditorReferencedDefaultTextures.Num() != 0)
		{
			ReferencedDefaultTextures = UsedCachedData.EditorOnlyData->EditorReferencedDefaultTextures;
		}
	...
	ReferencedDefaultTextures = CachedExpressionData->EditorOnlyData->EditorReferencedDefaultTextures;
...
#if WITH_EDITOR
uint64 UMaterial::GetForceRecompileTextureIdsHash() const
{
	...
	FXxHash64Builder TextureCompileDependencies;

	for (UTexture *ForceRecompileTexture : ForceRecompileTextures)
	{
		TextureCompileDependencies.Update(&ForceRecompileTexture->GetLightingGuid(), sizeof(FGuid));
	}

	return TextureCompileDependencies.Finalize().Hash;
}
...
const FParameterGroupData* UMaterial::GetSortingGroup(const FString& InGroupName)
{
#if WITH_EDITOR
	UMaterialEditorOnlyData* LocalData = GetEditorOnlyData();
	if (LocalData)
	{
		return LocalData->ParameterGroupData.FindByKey(InGroupName);
	}
#endif
	return nullptr;
}
...
bool UMaterial::AttemptInsertNewGroupName(const FString& InNewName)
{
	...
	if (LocalData)
	{
		FParameterGroupData* ParameterGroupDataElement = LocalData->ParameterGroupData.FindByKey(InNewName);
...
void UMaterial::Serialize(FArchive& Ar)
{
...
#if WITH_EDITOR
	if (Ar.IsSaving() && Ar.IsCooking() && ShouldStripUnusedDefaultTextures())
	{
		constexpr bool bKeepProjectPrivateTextures = true;
		StripUnusedDefaultTextures(bKeepProjectPrivateTextures);
	}
#endif
	...
	if (Ar.IsLoading() && Ar.CustomVer(FFortniteMainBranchObjectVersion::GUID) < FFortniteMainBranchObjectVersion::AddMeshDeformerUsageFlag)
	{
		// Default mesh deformer usage to match the skeletal mesh usage.
		PRAGMA_DISABLE_DEPRECATION_WARNINGS
		bUsedWithMeshDeformer = bUsedWithSkeletalMesh;
		PRAGMA_ENABLE_DEPRECATION_WARNINGS
	}
	...
	if (Ar.IsSaving() && Ar.IsCooking() && ShouldStripUnusedDefaultTextures())
	{
		// Undo textures removal for Multi Platform Cook
		ReferencedDefaultTextures = CachedExpressionData->EditorOnlyData->EditorReferencedDefaultTextures;
	}
#endif
}
...
void UMaterial::BackwardsCompatibilityInputConversion()
{
	if( ShadingModel != MSM_Unlit )
	{
		...
		{
			// ConvertFromDiffSpec
			if (!GConvertFromDiffSpecMaterialFunction)
			{
				...
				check(!GConvertFromDiffSpecMaterialFunction->HasAnyFlags(RF_NeedLoad | RF_NeedPostLoad));
			}
...
static void AddSurfaceSubstrateShadingModelFromMaterialShadingModels(FSubstrateMaterialInfo& OutInfo, const FMaterialShadingModelField& InShadingModels)
{
	...
	if (InShadingModels.HasShadingModel(MSM_StylizedCharacterLit)) { OutInfo.AddShadingModel(ESubstrateShadingModel::SSM_DefaultLit); }
}
...
void UMaterial::PostLoad()
{
	...
	if(bUseFullPrecision_DEPRECATED && (FloatPrecisionMode == EMaterialFloatPrecisionMode::MFPM_Half || FloatPrecisionMode == EMaterialFloatPrecisionMode::MFPM_Half_WithShaderParameters))
	{
	...
	{
		// Fix up if we are missing expressions
		if (GetExpressions().Num() == 0)
		{
			FMaterialEditorUtilities::InitExpressions(this);
		}
		UpdateCachedExpressionData();
	}
	else
	...
	// Avoid running the following checks when running cooks
	if (!IsRunningCookCommandlet())
	{
		if(FloatPrecisionMode == EMaterialFloatPrecisionMode::MFPM_Half && GetLinkerCustomVersion(FFortniteReleaseBranchCustomObjectVersion::GUID) < FFortniteReleaseBranchCustomObjectVersion::MaterialHalfPrecisionFixup)
		{
			static const auto CVar = IConsoleManager::Get().FindConsoleVariable(TEXT("r.Mobile.FloatPrecisionMode"));
			if (CVar)
			{
				int CVarMobilePrecisionMode = FMath::Clamp(CVar->GetInt(), (int32)EMobileFloatPrecisionMode::Half, (int32)EMobileFloatPrecisionMode::Full);
				if (CVarMobilePrecisionMode == EMobileFloatPrecisionMode::Full)
				{
					// Choosing 'Use Half Precision' on a per material property in the Material Editor was not working as intended when the cvar r.Mobile.FloatPrecisionMode=2 (it was a no-op)
					// With this option now fixed, we revert this material's Float Precision option from Half --> Default to maintain the generated code the same and avoid performance/visual regressions
					FloatPrecisionMode = EMaterialFloatPrecisionMode::MFPM_Default;

					const FText FloatPrecisionModeSelectedOnMaterial =
						FText::Format(LOCTEXT("MapCheck_Message_MaterialUseHalfPrecisionCheck",
							"Material {0} had the option \"Use Half Precision\" from Float Precision Mode checked. This option was not working as intended before so it's being reverted to Default to maintain previous behaviour."),
							FText::FromName(this->GetFName()));

					FMessageLog("MapCheck").Warning()
						->AddToken(FTextToken::Create(FloatPrecisionModeSelectedOnMaterial))
						->AddToken(FActionToken::Create(LOCTEXT("MapCheck_SaveMaterialWithDefaultFloatingPointPrecision", "Please save the material to keep this change"),
							LOCTEXT("MapCheck_SaveMaterialWithDefaultFloatingPointPrecision_Desc", "Reverts this material precision from Half -> Default to preserve previous behaviour and saves it"),
	...
	STAT(double MaterialLoadTime = 0);
	{
		...
		if (FApp::CanEverRender() && !bIsMaterialEditorStatsMaterial && GAllowCompilationInPostLoad)
		{
			...
			for (UObject* Texture : ReferencedDefaultTextures)
			{
				if (Texture)
				{
	...
	else if (IsDeferredDecal() || IsUIMaterial() || (MaterialDomain == MD_LightFunction) || IsPostProcessMaterial())
	{
		...
		PrecachePSOs(VertexFactoryDataList, PSOPrecacheParams, EPSOPrecachePriority::High, MaterialPSOPrecacheRequestIDs);
	}
...
TObjectPtr<UObject> UMaterial::GetDefaultTextureAtIdx(const int32 InIdx, TMicRecursionGuard RecursionGuard)
{
	const TArrayView<const TObjectPtr<UObject>> UsedDefaultTextures = GetReferencedTextures();

	if (!UsedDefaultTextures.IsValidIndex(InIdx))
	{
		static bool bWarnedOnce = false;
		if (!bWarnedOnce)
		{
			UE_LOGF(LogMaterial, Warning, "%ls: Requesting an invalid TextureIndex! (%u / %u)", *GetName(), InIdx, UsedDefaultTextures.Num());
			bWarnedOnce = true;
		}
		return nullptr;
	}

	return UsedDefaultTextures[InIdx];
}
...
void UMaterial::GetShaderTypes(EShaderPlatform ShaderPlatform, const ITargetPlatform* TargetPlatform, TArray<FDebugShaderTypeInfo>& OutShaderInfo)
{
	...
	for (FMaterialResource* Resource : NewResourcesToCache)
	{
		Resource->GetShaderTypes(LayoutParams, OutShaderInfo, FConservativeShaderParameters(Resource));
		delete Resource;
	}
...
bool UMaterial::IsCachedCookedPlatformDataLoaded( const ITargetPlatform* TargetPlatform ) 
{
	...
	if (UE_LOG_ACTIVE(LogMaterial, VeryVerbose))
	{
		...
		UE_LOGF(LogMaterial, VeryVerbose, "Material [%ls] Cook State:", *GetName());

		UE::String::ParseTokens(CookStateInfo.ToView(), TEXT('\n'),
			[](FStringView Line)
			{
				UE_LOGF(LogMaterial, VeryVerbose, "%.*ls", Line.Len(), Line.GetData());
...
bool UMaterial::CanEditChange(const FProperty* InProperty) const
{
	if (InProperty)
	{
		...
		const bool bSubstrateEnabled = Substrate::IsSubstrateEnabled();
		const bool bFrontMaterialConnected = EditorOnly->FrontMaterial.IsConnected();
		const bool bSubstrateHiddenMaterialAssetConversion = bSubstrateEnabled && Substrate::IsHiddenMaterialAssetConversionEnabled() && !bFrontMaterialConnected;
		...
		{
			if (bSubstrateEnabled && bFrontMaterialConnected)
			{
...
void UMaterial::RebuildShadingModelField()
{
	...
	if (Substrate::IsSubstrateEnabled() && EditorOnly->FrontMaterial.IsConnected())
	{
...
#if WITH_EDITOR
		extern bool IsPreviewUnlitMaterial(const UMaterial * InMaterial);
		const bool bIsMaterialFunctionPreview = IsPreviewUnlitMaterial(this);
#else
		const bool bIsMaterialFunctionPreview = false;
#endif
		...
		if (SubstrateMaterialInfo.IsValid() && SubstrateMaterialInfo.GetSubstrateTreeOutOfStackDepthOccurred())
		{
			...
			ShadingModels.AddShadingModel(MSM_Unlit);
			CancelOutstandingCompilation();
			UE_LOGF(LogMaterial, Error, "%ls: Substrate - Cyclic graph detected when we only support acyclic graph.", *GetName());
			return;
		}
		...
		if (!SubstrateMaterialInfo.IsValid() && !SubstrateMaterialInfo.HasShadingModelFromExpression() )
		{
			...
			UE_LOGF(LogMaterial, Error, "%ls: Material information is invalid.", *GetName());
		}
		...
		if (SubstrateMaterialInfo.CountShadingModels() > 1)
		{
			...
			else
			{
				...
				bSanitizeMaterial = true;
				UE_LOGF(LogMaterial, Error, "%ls: Material has more than a single material represented.", *GetName());
			}
		}
		...
		{
			if (bIsMaterialFunctionPreview)
			{
				// Keep material domain selected from the preview window and set unlit for preview to work.
				ShadingModel = MSM_Unlit;
			}
			// Now derive some properties from the material into the legacy fields
			else if (SubstrateMaterialInfo.HasOnlyShadingModel(SSM_Unlit))
			{
				MaterialDomain = EMaterialDomain::MD_Surface;
				ShadingModel = MSM_Unlit;
			}
			...
			else if (SubstrateMaterialInfo.HasOnlyShadingModel(SSM_SubsurfaceProfile) || SubstrateMaterialInfo.HasOnlyShadingModel(SSM_SubsurfaceMFP))
			{
				...
				if (BlendMode != EBlendMode::BLEND_Opaque && BlendMode != EBlendMode::BLEND_Masked)
				{
					UE_LOGF(LogMaterial, Error, "%ls: Material has subsurface data, and its blending mode is not set to Opaque or Masked. Forcing blend mode to Opaque.", *GetName());
			...
			else if (SubstrateMaterialInfo.HasOnlyShadingModel(SSM_Toon))
			{
				MaterialDomain = EMaterialDomain::MD_Surface;
				ShadingModel = MSM_DefaultLit;
			}
		...
		// Set Toon profile if any
		ToonProfiles.SetNum(SubstrateMaterialInfo.CountToonProfiles());
		for (int32 It = 0, Count = SubstrateMaterialInfo.CountToonProfiles(); It<Count; ++It)
		{
			ToonProfiles[It] = SubstrateMaterialInfo.GetToonProfile(It);
		}
	}
													   ...
													   TFunction<UMaterialInterface*(const FString&)> ODSCCustomLoadMaterial)
{
	...
	{
		UE_LOGF(LogShaders, Display, "Received %d shaders to compile.", ShadersToRecompile.Num());
	}
	...
	{
		UE_LOGF(LogShaders, Verbose, "");
		UE_LOGF(LogShaders, Verbose, "Material:    %ls ", *payload.MaterialName);
		...
		if (MaterialInterface)
		{
			...
			if (VFType)
			{
				UE_LOGF(LogShaders, Verbose, "VF Type:     %ls ", *payload.VertexFactoryName);
			}

			if (PipelineType)
			{
				UE_LOGF(LogShaders, Verbose, "Pipeline Type: %ls", *payload.PipelineName);
			...
			else
			{
				for (const FString& ShaderTypeName : payload.ShaderTypeNames)
				{
					...
					if (ShaderType)
					{
						UE_LOGF(LogShaders, Verbose, "\tShader Type: %ls", *ShaderTypeName);
	...
	for (TMap<FString, TArray<FMaterialResource*> >::TIterator It(CompilingResources); It; ++It)
	{
		...
		for (int32 Index = 0; Index < ResourceArray.Num(); Index++)
		{
			...
			FMaterialShaderMap* MaterialShaderMap = CurrentResource->GetGameThreadShaderMap();
			OutShaderMapArray.Add(MaterialShaderMap);

			int32 ThisMaterialNumShaders = 0;
			int32 ThisMaterialNumShaderPipelines = 0;
			MaterialShaderMap->CountNumShaders(ThisMaterialNumShaders, ThisMaterialNumShaderPipelines);

			NumShaders += ThisMaterialNumShaders;
			NumShaderPipelines += ThisMaterialNumShaderPipelines;
			++NumShaderMaps;
		}
	}

	UE_LOGF(LogShaders, Display, "Sending %d shaders, %d pipelines for %d shader maps", NumShaders, NumShaderPipelines, NumShaderMaps);
	...
	{
		UE_LOGF(LogShaders, Warning, "Failed to load %ls, skipping shader reloading", *MaterialFailingToLoad);
	}
}
#endif // WITH_EDITOR
	...
	ERHIFeatureLevel::Type InFeatureLevel, EMaterialQualityLevel::Type InQuality, ERHIShadingPath::Type InShadingPath, const bool bInRecurseIntoMaterialFunctions, TSet<UClass*>* InMobileCustomOutputExpressionTypesToQuery)
{
	...
	TSet<UMaterialExpression*> TmpExpressions;
	...
	if (InFeatureLevel <= ERHIFeatureLevel::ES3_1)
	{
		if (bUseMaterialAttributes)
		{
			...
			if (GetExpressionsInPropertyChain(MP_MaterialAttributes, MPRefdExpressions, InStaticParameterSet, InFeatureLevel, InQuality, InShadingPath, bInRecurseIntoMaterialFunctions) == true)
			{
				...
				{
					TmpExpressions.Add(MPRefdExpressions[AddIdx]);
				}
			}
		}
		else
		...
		if (bMobileUseVirtualTexturing || (InMobileCustomOutputExpressionTypesToQuery && InMobileCustomOutputExpressionTypesToQuery->Num() > 0))
		{
			...
			for (UMaterialExpressionCustomOutput* Expression : CustomOutputExpressions)
			{
				...
				{
					TSet<FExpressionInput*> ProcessedInputs;
					RecursiveGetExpressionChain(Expression, ProcessedInputs, TmpExpressions, InStaticParameterSet, InFeatureLevel, InQuality, InShadingPath, SF_NumFrequencies, MP_MAX, bInRecurseIntoMaterialFunctions);
				}
			}
		}
	}
	...
	OutExpressions = TmpExpressions.Array();
	...
	ERHIFeatureLevel::Type InFeatureLevel, EMaterialQualityLevel::Type InQuality, ERHIShadingPath::Type InShadingPath, const bool bInRecurseIntoMaterialFunctions)
{
	...
	{
		ProcessedInputs.Add(StartingExpression);
		...
		RecursiveGetExpressionChain(StartingExpression->Expression, ProcessedInputs, TmpExpressions, InStaticParameterSet, InFeatureLevel, InQuality, InShadingPath, ShaderFrequency, InProperty, bInRecurseIntoMaterialFunctions);
	}
	OutExpressions = TmpExpressions.Array();
	return true;
}
...
bool UMaterial::RecursiveGetExpressionChain(
	UMaterialExpression* InExpression, TSet<FExpressionInput*>& InOutProcessedInputs, 
	TSet<UMaterialExpression*>& OutExpressions, struct FStaticParameterSet* InStaticParameterSet,
	ERHIFeatureLevel::Type InFeatureLevel, EMaterialQualityLevel::Type InQuality, ERHIShadingPath::Type InShadingPath, EShaderFrequency InShaderFrequency, EMaterialProperty InProperty, const bool bInRecurseIntoMaterialFunctions)
{
	...
	OutExpressions.Add(InExpression);
	TArray<FExpressionInput*, TInlineAllocator<8>> Inputs;
	TArray<EShaderFrequency, TInlineAllocator<8>> InputsFrequency;
	...
	for (int32 InputIdx = 0; InputIdx < Inputs.Num(); InputIdx++)
	{
		...
		if (InnerInput != NULL)
		{
			if (!InOutProcessedInputs.Contains(InnerInput))
			{
...
#if WITH_EDITOR
bool UMaterial::TryGetUsedTexturesInCookCache(TArray<TObjectPtr<UObject>>& OutTextures)
{
	OutTextures.Empty();
	const FMaterialCachedExpressionData& UsedCachedData = GetCachedExpressionData();
	TArray<const FMaterialResource*, TInlineAllocator<4>> MatchedResources = GetCookedCacheMaterialResources();

	for (const FMaterialResource* CurrentResource : MatchedResources)
	{
		const FUniformExpressionSet& CurrentSet = CurrentResource->GetUniformExpressions();

		for (int32 TypeIdx = 0; TypeIdx < NumMaterialTextureParameterTypes; ++TypeIdx)
		{
			for (int32 TextureIdx = 0; TextureIdx < CurrentSet.GetNumTextures((EMaterialTextureParameterType)TypeIdx); ++TextureIdx)
			{
				const FMaterialTextureParameterInfo& Parameter = CurrentSet.GetTextureParameter((EMaterialTextureParameterType)TypeIdx, TextureIdx);
				OutTextures.AddUnique(UsedCachedData.EditorOnlyData->EditorReferencedDefaultTextures[Parameter.TextureIndex]);
			}
		}
	}

	const bool bValidCompilationOutput = MatchedResources.Num() > 0;
	return bValidCompilationOutput;
}
#endif
...
bool UMaterial::CheckInValidStateForCompilation(class FMaterialCompiler* Compiler) const
{
	...
	for (const auto& Param : CachedExpressionData->ParameterExpressionsAndValues)
	{
		const FString ParamName = Param.Key.ToString();
		const FMaterialCachedExpressionData::FParamExpressionsValues& ParameterExpressionsValues = Param.Value;
		if (ParameterExpressionsValues.bHasDuplicatesWithDifferentValues)
		{
			bSuccess = false;
			for (const auto& ExpressionsValues : ParameterExpressionsValues.ExpressionsValues)
			{
				UMaterialExpression* Expression = ExpressionsValues.Get<0>();
				const FString& ParamValue = ExpressionsValues.Get<1>();

				FString ErrorMessage = FString::Printf(TEXT("Parameter %s with default value %s from asset %s has duplicates with different default values."),
					*ParamName, *ParamValue, *Expression->GetAssetPathName());

				Compiler->AppendExpressionError(Expression,  *ErrorMessage);
			}
		}
	}
	return bSuccess;
...
static void ListSceneColorMaterials()
{
	...
	UMaterialInterface::IterateOverActiveFeatureLevels([&](ERHIFeatureLevel::Type FeatureLevel) 
	{
		...
		for (TObjectIterator<UMaterialInterface> It(/*AdditionalExclusionFlags = */RF_ClassDefaultObject, /*bIncludeDerivedClasses = */true, /*InInternalExclusionFlags = */EInternalObjectFlags::Garbage); It; ++It)
		{
			...
			if (MatRes && MatRes->RequiresSceneColorCopy_GameThread())
			{
				...
				UE_LOGF(LogConsoleResponse, Display, "[TransPass=%d][FeatureLevel=%ls] %ls",
	...
	UE_LOGF(LogConsoleResponse,Display,"%d loaded materials read from scene color.",NumSceneColorMaterials);
}
...
bool UMaterial::IsLightFunctionMaterial() const
{
	return MaterialDomain == MD_LightFunction;
}
...
uint32 UMaterial::NumToonProfile_Internal() const
{
	return ToonProfiles.Num();
}

UToonProfile* UMaterial::GetToonProfile_Internal(uint32 Index) const
{
	checkSlow(IsInGameThread());
	check(Index<uint32(ToonProfiles.Num()));
	return ToonProfiles[Index];
}
	...
	bool bFrontMaterialIsConnected)
{
	...
	else
	{
		...
		case MP_Roughness:
			...
			break;
		case MP_Specular:
			Active = ShadingModels.IsLit() && !ShadingModels.HasShadingModel(MSM_StylizedCharacterLit) && (!bIsTranslucentBlendMode || !bIsVolumetricTranslucencyLightingMode);
			break;
		case MP_Anisotropy:
...
{
	BaseColor.Constant = FMaterialAttributeDefinitionMap::GetDefaultValue(MP_BaseColor);
