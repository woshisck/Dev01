...
BEGIN_GLOBAL_SHADER_PARAMETER_STRUCT(FDeferredLightUniformStruct,)
	...
	SHADER_PARAMETER(FVector4f, StylizedLumenLightingParams0)
	SHADER_PARAMETER(FVector4f, StylizedLumenLightingParams1)
	// x: direct light color influence, y: diffuse indirect color influence, z: reflection color influence
	SHADER_PARAMETER(FVector4f, StylizedCharacterLightingParams)
	// x: mode (0 wash, 1 toon, 2 rim), y: shadow strength, z: rim power, w: rim intensity
	SHADER_PARAMETER(FVector4f, StylizedCharacterLightParams)
	// x: profile count, y: valid ramp atlas.
	SHADER_PARAMETER(FVector4f, StylizedCharacterProfileMeta)
	SHADER_PARAMETER_ARRAY(FVector4f, StylizedCharacterProfileData, [88])
	SHADER_PARAMETER_TEXTURE(Texture2D, StylizedCharacterRampAtlas)
	SHADER_PARAMETER_SAMPLER(SamplerState, StylizedCharacterRampAtlasSampler)
...
extern FVector4f GetStylizedLumenLightingParams1();
extern FVector4f GetStylizedCharacterLightingParams();

extern float GetLightFadeFactor(const FSceneView& View, const FLightSceneProxy* Proxy);

extern uint32 GetShadowedBits(const FSceneView& View, const FLightSceneInfo& LightSceneInfo, bool bUseLightFunctionAtlas = false);
