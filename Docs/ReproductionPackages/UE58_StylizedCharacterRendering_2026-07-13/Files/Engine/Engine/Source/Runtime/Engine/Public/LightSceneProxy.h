...
public:
	...
	ENGINE_API FLightSceneProxy(const FLightSceneProxyDesc& Desc);
	...
	// Max world-space distance for CSM receivers (near + far cascades combined).
	virtual float GetMaxShadowReceiverDistance(const FSceneView& View, bool bPrecomputedLightingIsValid) const { return 0.0f; }
	 ...
	 */
	UE_DEPRECATED(5.8, "Use GetLightComponentId instead.")
	inline const ULightComponent* GetLightComponent() const { return LightComponent; }
	inline const FLightComponentId& GetLightComponentId() const { return LightComponentId; }
	...
	inline float GetRayEndBias() const { return RayEndBias; }
	...
	inline uint8 GetStylizedCharacterLightMode() const { return StylizedCharacterLightMode; }
	inline float GetStylizedCharacterShadowStrength() const { return StylizedCharacterShadowStrength; }
	inline float GetStylizedCharacterRimPower() const { return StylizedCharacterRimPower; }
	inline float GetStylizedCharacterRimIntensity() const { return StylizedCharacterRimIntensity; }
	...
	// Necessary to build FLightSceneInfo
	inline bool IsPrecomputedLightingValid() const { return bIsPrecomputedLightingValid; }
	inline bool IsLightShaftBloomEnabled() const { return bEnableLightShaftBloom; }
	inline float GetBloomScale() const { return BloomScale; }
	inline float GetBloomThreshold() const { return BloomThreshold; }
	inline float GetBloomMaxBrightness() const { return BloomMaxBrightness; }
	inline const FColor& GetBloomTint() const { return BloomTint; }
...
protected:
	...
	/** The light component. (Only necessary for FScene::GetRelevantLights) */
	const ULightComponent* LightComponent;

	/** The runtime unique id used to identify a light component. */
	const FLightComponentId LightComponentId;
	...
	uint8 bIsPrecomputedLightingValid : 1;
	...
	*/
	...
	const uint8 StylizedCharacterLightMode;
	const float StylizedCharacterShadowStrength;
	const float StylizedCharacterRimPower;
	const float StylizedCharacterRimIntensity;
	...
	/** Lookup for cached shadow atlas information for timesliced runtime reflection captures. */
	uint8 TimesliceAtlasIndex[4];		// Index in array of cached atlased shadows, by ProjectionIndex
	uint32 TimesliceAtlasFrame;			// Frame number to track if AtlasIndex is valid (matches frame number of current active capture)
