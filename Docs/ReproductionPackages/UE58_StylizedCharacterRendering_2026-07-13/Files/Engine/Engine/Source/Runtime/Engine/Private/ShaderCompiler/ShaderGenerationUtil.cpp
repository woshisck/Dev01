...
void ApplyFetchEnvironmentInternal(FShaderMaterialPropertyDefines& SrcDefines, const ContainerType& Container)
{
	...
	FETCH_COMPILE_BOOL(MATERIAL_SHADINGMODEL_STYLIZED_CHARACTER_LIT);
...
static FShaderGlobalDefines FetchShaderGlobalDefines(EShaderPlatform TargetPlatform, EGBufferLayout GBufferLayout)
{
...

	{
		...
		Ret.CLEAR_COAT_BOTTOM_NORMAL = CVar ? (CVar->GetValueOnAnyThread() != 0 && (!bIsMobilePlatform || !IsMobileDeferredShadingEnabled(TargetPlatform) || MobileUsesExtendedGBuffer(TargetPlatform))) : 0;
	}
	...
	for (int32 ItemIndex = 0; ItemIndex < GBS_Num; ItemIndex++)
	{
		...
		if (Item.bIsValid)
		{
			...
			if (bIsCompressed)
			{
				...
				FString QuantizationValue = "0.0f";
	...
	ERHIFeatureLevel::Type FEATURE_LEVEL)
{
	...
	if (Mat.MATERIAL_SHADINGMODEL_STYLIZED_CHARACTER_LIT)
	{
		SetStandardGBufferSlots(Slots, bWriteEmissive, bHasTangent, bHasVelocity, bWritesVelocity, bHasStaticLighting, bIsSubstrateMaterial, bIsSubstrateNewGBuffer);
		Slots[GBS_CustomData] = GetGBufferSlotUsage(bUseCustomData);
	}
...
void FShaderCompileUtilities::WriteGBufferInfoAutogen(EShaderPlatform TargetPlatform, ERHIFeatureLevel::Type FeatureLevel = ERHIFeatureLevel::SM5)
{
	...
	if (FeatureLevel <= ERHIFeatureLevel::ES3_1)
	{
		...
		if (PlatformFile.FileExists(*AutogenHeaderFilename))
		{
			...
			{
				UE_LOGF(LogShaderCompilers, Display, "Failed to delete old and now unused shader autogen file: %ls", *AutogenHeaderFilename);
			}
		}
		else
		{
	...
	else
	{
		...
		UE_LOGF(LogShaderCompilers, Display, "Compiling shader autogen file: %ls", *AutogenHeaderFilename);
		...
		bool bWriteNeeded = true;
		{
			...
			{
				UE_LOGF(LogShaderCompilers, Display, "Autogen file is unchanged, skipping write.");
				bWriteNeeded = false;
			}
		}
		...
		if (bWriteNeeded)
		{
			...
			if (!bSaveOk)
			{
				UE_LOGF(LogShaderCompilers, Display, "Failed to save shader autogen file: %ls", *AutogenHeaderFilename);
			}
			...
			if (!bDeleteOk)
			{
				...
				UE_LOGF(LogShaderCompilers, Display, "Failed to delete old shader autogen file: %ls", *AutogenHeaderFilename);
			}

			bool bRenameOk = FPlatformFileManager::Get().GetPlatformFile().MoveFile(*AutogenHeaderFilename, *AutogenHeaderFilenameTemp);
			if (!bRenameOk)
			{
				UE_LOGF(LogShaderCompilers, Display, "Failed to rename shader autogen file: %ls", *AutogenHeaderFilename);
			}

			UE_LOGF(LogShaderCompilers, Display, "Shader autogen file written: %ls", *AutogenHeaderFilename);
		}
	}
}
