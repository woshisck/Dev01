	...
	: Super(ObjectInitializer)
{
	...
	bCastShadowOnOtherObjectsOnly = false;
	...
	SetAttachedToStreamingManagerAsStatic(false);
	SetAttachedToStreamingManagerAsDynamic(false);
	SetHandledByStreamingManagerAsDynamic(false);
	SetIgnoreStreamingManagerUpdate(false);	
	bBulkReregister = false;
	LastCheckedAllCollideableDescendantsTime = 0.f;

	PSOPrecacheComponentData.Reset();
	...
	bRegisteredForNavigation = false;
...
UStreamableRenderAsset* UPrimitiveComponent::GetStreamableNaniteAsset() const
{
	return {};
}
		...
		if (UsedMaterials.Num())
		{
			...
			while (UsedMaterials.Num())
			{
				...
				if (MaterialInterface)
				{
					...
					LevelContext.ProcessMaterial(Bounds, MaterialData, Bounds.SphereRadius, OutStreamingRenderAssets, bIsValidTextureStreamingBuiltData);
				}
...
{
	OutStreamingRenderAssets = LevelContext.GetStreamingRenderAssetInfoWithNULLRemoval(GetPrimitiveComponentInterface());
}
...
void UPrimitiveComponent::OnRegister()
{
	...
	SetLastRenderTime(SceneData.GetLastRenderTime());
}
...
void UPrimitiveComponent::OnUnregister()
{
	...
	if (IsAttachedToStreamingManagerAsDynamic())
	{
	...
	// This is necessary because, due to the SimpleConstructionScript system, it will register this component with default property values before having the correct serialized property values applied.
	// This was leading to this component registering itself with the navigation octree when it was not supposed to and never knowing to remove itself.
	// When this bCanEverAffectNavigation was being set, the order of events was:
	// 1. OnUnregister the object with the old property value.
	// 2. Change the property value.
	// 3. OnRegister the object with the new property value.
	// 4. UActorComponent::ConsolidatedPostEditChange gets called which...
	//     a. Calls OnUnregister on and destroys the old component.
	//     b. Creates a new component with default property values.
	//     c. Calls OnRegister on the new component with the default property values. <=== Source of the bug.
	//     d. Serializes in the correct property values.
	//     e. Calls OnUnregister with the correct property value, which has lost what happened during the OnRegister.
	//     f. Calls OnRegister again with the correct property value.
	// This was written up in UE-350551.

	const bool bUnregisterNavigation = (bCanEverAffectNavigation || bRegisteredForNavigation);
#else
	const bool bUnregisterNavigation = bCanEverAffectNavigation;
#endif

	if (bUnregisterNavigation)
	{
...
void UPrimitiveComponent::OnCreatePhysicsState()
{
	...
	{
		//UE_LOGF(LogPrimitiveComponent, Warning, "Creating Physics State (%ls : %ls)", *GetNameSafe(GetOuter()),  *GetName());
		...
		if(BodySetup)
		{
			...
			{
				UE_LOGF(LogPhysics, Warning, "Scale for %ls has a component set to zero, which will result in a bad body instance. Scale:%ls Package:%ls", *GetPathNameSafe(this), *BodyScale.ToString(), *GetPackage()->GetName());
			...
			if (BodyInstance.IsValidBodyInstance())
			{
				...
				{
					UE_LOGF(LogPhysics, Log, "Calculated mass differs from real mass for %ls:%ls. Mass: %f  CalculatedMass: %f",
	...
	if (!IsAsyncCreatePhysicsStateRunning())
	{
		// Update per-frame physics quantization registration
		UE::PhysicsReplicationQuantization::NotifyComponentPhysicsState(this, /*bRegister*/true);

		OnComponentPhysicsStateChanged.Broadcast(this, EComponentPhysicsStateChange::Created);
	}
}

void UPrimitiveComponent::OnAsyncCreatePhysicsStateBegin_GameThread(TSet<UObject*>& OutRootedObjects)
{
	BodyInstance.CachePhysicsCreationInputs(this)->GetReferencedObjects(OutRootedObjects);
}
...
void UPrimitiveComponent::ConditionalNotifyStreamingPrimitiveUpdated_Concurrent() const
{
	...
	if (!IsIgnoreStreamingManagerUpdate() && (Mobility != EComponentMobility::Static || IsHandledByStreamingManagerAsDynamic() || (!IsAttachedToStreamingManagerAsStatic() && OwnerLevelHasRegisteredStaticComponentsInStreamingManager(GetOwner()))))
	{
		if (FStreamingManagerCollection* Collection = IStreamingManager::Get_Concurrent())
		{
...
void UPrimitiveComponent::OnRenderAssetFirstLodChange(const UStreamableRenderAsset* RenderAsset, int32 FirstLodIndex)
{
...
void UPrimitiveComponent::OnAsyncDestroyPhysicsStateBegin_GameThread(TSet<UObject*>& OutRootedObjects)
{
	check(AllowsAsyncPhysicsStateDestruction());

	// Update per-frame physics quantization registration
	UE::PhysicsReplicationQuantization::NotifyComponentPhysicsState(this, /*bRegister*/false);
	...
	Super::OnAsyncDestroyPhysicsStateBegin_GameThread(OutRootedObjects);
}
...
void UPrimitiveComponent::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		//Reset the CPD arrays in Serialize to avoid a race condition between PostLoad (Game thread) and FObjectInitializer::InitProperties (async loading thread)
...
void UPrimitiveComponent::MigratePhysicsState(FArchive& Ar)
{
	...
	if (Ar.IsLoading())
	{
		...
		{
			UE_LOGF(LogPrimitiveComponent, Verbose, "Migrated an empty physics state (this can be expected) | [%ls] will use its default physics state or the one already had | Migration Context [%ls].", *GetPathNameSafe(this), ToString(ObjMigrationRecvType));
			return;
		}
	...
	{		
		...
		else
		{
			...
			{
				UE_LOGF(LogPrimitiveComponent, Error, "[%s] Attempted to migrated an object physics state, before processing a previous pending physics state| Discarding pending physics state and sending an empty one...", __func__);
			}
		...
		if (ObjMigrationSendType == EObjectMigrationSendType::ReassignOwnership)
		{
			if (FPhysScene* PhysicsScene = FPhysicsInterface::GetCurrentSceneForComponent(this))
			{
				PhysicsScene->RemoveFromComponentOwnerMigratedSet(TObjectPtr<AActor>(GetOwner()).GetRemoteId());
			}
		}
	}
}
...
void UPrimitiveComponent::ApplyMigratedPhysicsState(Chaos::FSerializedDataBufferPtr MigratedStateData)
{
	...
	if (RawMigratedData)
	{
		...
		{
			UE_LOGF(LogPrimitiveComponent, Verbose, "[%s] Applying migrated physics state | Data context [%ls]", __func__, Chaos::LexToString(MigratedStateHeader.DataContext));
	...
	if (MigrationContext.MigrationSide == EObjectMigrationSide::Receive)
	{
		...
		if (RecvType == EObjectMigrationRecvType::ReturnedLoan)
		{
			...
			else
			{
				...
				if (!ensure(!RawMigratedData))
				{
					UE_LOGF(LogPrimitiveComponent, Error, "[%s] Received a loaned object that destroyed our physics state, but we still have pending data to process. This should not happen...", __func__);
				}
			}
		}
		...
		if (RecvType == EObjectMigrationRecvType::AssignedOwnership)
		{
			if (FPhysScene* PhysicsScene = FPhysicsInterface::GetCurrentSceneForComponent(this))
			{
				PhysicsScene->AddToComponentOwnerMigratedSet(TObjectPtr<AActor>(GetOwner()).GetRemoteId());
			}
		}
...
bool UPrimitiveComponent::CanEditChange(const FProperty* InProperty) const
{
	...
	if (bIsEditable && InProperty)
	{
		...
		static FName LightmassSettingsName = "LightmassSettings";
		...
		static FName SelfShadowOnlyName = GET_MEMBER_NAME_CHECKED(UPrimitiveComponent, bSelfShadowOnly);
		static FName CastShadowOnOtherObjectsOnlyName = GET_MEMBER_NAME_CHECKED(UPrimitiveComponent, bCastShadowOnOtherObjectsOnly);
		...
		{
			return !bSelfShadowOnly && !bCastShadowOnOtherObjectsOnly;
		}

		if (PropertyName == SelfShadowOnlyName)
		{
			return !bCastShadowOnOtherObjectsOnly;
		}

		if (PropertyName == CastShadowOnOtherObjectsOnlyName)
		{
...
{
	const FName NAME_Scale3D("Scale3D");
	const FName NAME_Scale("Scale");
	const FName NAME_Translation("Translation");
...
void UPrimitiveComponent::BeginDestroy()
{
	...
	{
		// SSAM uses lock-free atomics + queues whose closed-code clones the AutoRTFM compiler
		// cannot emit. The atomic accesses (Exchange, IsStale) must run from the open;
		// OnCommit must be registered from the closed (OnCommit from inside an open nest
		// runs immediately, defeating the deferral). Reconstruction (StaticConstructObject_Internal)
		// snapshots the full UObject memory via RecordOpenWrite before BeginDestroy, so the
		// Exchange below is implicitly rolled back on abort. The queue push to
		// RecoverMissedUnregister is global state outside that snapshot, so we defer it to
		// commit via AutoRTFM::OnCommit -- which from closed code is a no-op outside transactions
		// (work runs immediately) and deferred to commit otherwise.

		// Capture-and-clear in one atomic op so the leak check sees a stable snapshot. A real
		// leak is a captured handle that's sentinel-valid AND still points at a live slot
		// (not stale); stale or default-invalid captures mean cleanup already happened or
		// never registered.
		const FSSAMPrimitiveHandle CapturedSSAMHandle = AutoRTFM::Open([&]
		{
			return SSAMPrimitiveHandle.Exchange(FSSAMPrimitiveHandle{});
		});

		// IsValid is a non-atomic SlotIndex check -- safe to call from closed. IsStale reaches
		// into the SSAM handle table's atomic generation counter, so it must run from the open.
		bool bMissedUnregister = false;
		if (CapturedSSAMHandle.IsValid())
		{
			const bool bStale = AutoRTFM::Open([&]
			{
				return CapturedSSAMHandle.IsStale();
			});
			bMissedUnregister = !bStale;
		}

		if (bMissedUnregister)
		{
			// Downgraded from ensureMsgf and rate-limited to once per session: a live SSAM handle
			// at BeginDestroy is recoverable here (we queue the unregister below) and signaling
			// it as an ensure produced noise on lifecycles where component teardown legitimately
			// races scene proxy teardown. Logging once keeps the underlying lifecycle bug visible
			// without flooding logs when many components hit the same path.
			static bool bLoggedOnce = false;
			if (!bLoggedOnce)
			{
				bLoggedOnce = true;
				UE_LOGF(LogPrimitiveComponent, Warning,
					"Component '%ls' held a live SSAM handle at BeginDestroy -- Unregister was missed. "
					"This component should be created and destroyed alongside its scene proxy. "
					"Further occurrences this session will be suppressed.",
					*GetPathName());
			}

			// Queue an unregister so the streaming arrays drop their references and the slot is
			// reclaimed instead of being leaked.
			AutoRTFM::OnCommit([CapturedSSAMHandle]
			{
				FSimpleStreamableAssetManager::RecoverMissedUnregister(CapturedSSAMHandle);
			});
		}
	}
...
bool UPrimitiveComponent::ShouldComponentAddToScene() const
{
	...
	return bSceneAdd && (ShouldRender() ||
						 (CastShadow && bCastHiddenShadow && !bIsHiddenInEditor) ||
						 (bAffectDynamicIndirectLighting && bAffectIndirectLightingWhileHidden && !bIsHiddenInEditor) ||
						 bRayTracingFarField);
}
	...
	if (BodyInstance.bSimulatePhysics)
	{
		if(UWorld* World = GetWorld())
		{
			if(World->IsGameWorld())
			{
				...
				{
					UE_LOGF(LogPhysics, Warning, "Invalid Simulate Options: Body (%ls) is set to simulate physics but Collision Enabled is incompatible", *GetReadableName());
				}
			}
		}
	}
...
void UPrimitiveComponent::SetCastShadowOnOtherObjectsOnly(bool bInCastShadowOnOtherObjectsOnly)
{
	if (bInCastShadowOnOtherObjectsOnly != bCastShadowOnOtherObjectsOnly)
	{
		bCastShadowOnOtherObjectsOnly = bInCastShadowOnOtherObjectsOnly;
		if (bCastShadowOnOtherObjectsOnly)
		{
...
UMaterialInstanceDynamic* UPrimitiveComponent::CreateAndSetMaterialInstanceDynamic(int32 ElementIndex)
{
	...
	{
		UE_LOGF(LogPrimitiveComponent, Warning, "CreateAndSetMaterialInstanceDynamic on %ls: Material index %d is invalid.", *GetPathName(), ElementIndex);
	}
...
UMaterialInstanceDynamic* UPrimitiveComponent::CreateDynamicMaterialInstance(int32 ElementIndex, class UMaterialInterface* SourceMaterial, FName OptionalName)
{
	...
	else if (!MaterialInstance)
	{
		...
		UE_LOGF(LogPrimitiveComponent, Warning, "CreateDynamicMaterialInstance on %ls: Material index %d is invalid.", *GetPathName(), ElementIndex);
#endif
	}
...
void UPrimitiveComponent::GatherDefaultCustomPrimitiveDataParametersFromMaterials(TArray<TPair<uint8, FCustomPrimitiveDataParameter>>& OutParameters, int32& OutMaxPrimitiveDataIndex)
{
	OutParameters.Reset();
	...
	for (int32 i = 0; i < NumMaterials; ++i)
	{
		UMaterialInterface* MaterialInterface = GetMaterial(i);
		...
		auto GatherForType = [this, &Parameters, &OutParameters, MaterialInterface, &OutMaxPrimitiveDataIndex](const EMaterialParameterType Type, const int32 TypeSize)
		{
			Parameters.Reset();

			MaterialInterface->GetAllParametersOfType(Type, Parameters);
			...
			for (const TPair<FMaterialParameterInfo, FMaterialParameterMetadata>& Parameter : Parameters)
			{
				...
				{
					OutParameters.Emplace(
						static_cast<uint8>(ParameterMetadata.PrimitiveDataIndex),
						FCustomPrimitiveDataParameter
						{
							.Component = this,
							...
							.Type = Type,
#if WITH_EDITOR
							.ExpressionID = ParameterMetadata.ExpressionGuid,
#endif // WITH_EDITOR
						}
					);

					OutMaxPrimitiveDataIndex = FMath::Max(OutMaxPrimitiveDataIndex, ParameterMetadata.PrimitiveDataIndex + TypeSize - 1);
				}
			}
		};

		GatherForType(EMaterialParameterType::Vector, 4);
		GatherForType(EMaterialParameterType::Scalar, 1);
	}
}

void UPrimitiveComponent::SetupDefaultCustomPrimitiveDataFromMaterials()
{
	TArray<TPair<uint8, FCustomPrimitiveDataParameter>> Parameters;
	int32 MaxPrimitiveDataIndex = INDEX_NONE;

	GatherDefaultCustomPrimitiveDataParametersFromMaterials(Parameters, MaxPrimitiveDataIndex);

	CustomPrimitiveData.Data.SetNumZeroed(MaxPrimitiveDataIndex + 1);

	for (const auto& [PrimIdx, Parameter] : Parameters)
	{
		if (!Parameter.Material.IsValid())
		{
			continue;
		}
		...
		{
			float Value = 0.f;
			if (Parameter.Material->GetScalarParameterValue(Parameter.Info, Value))
			{
				SetDefaultCustomPrimitiveDataFloat(PrimIdx, Value);
			}
		}
		else if (Parameter.Type == EMaterialParameterType::Vector)
		{
			FVector4 Value(ForceInitToZero);
			if (Parameter.Material->GetDoubleVectorParameterDefaultValue(Parameter.Info, Value))
			{
				SetDefaultCustomPrimitiveDataVector4(PrimIdx, Value);
			}
		}
		else
		{
		...
		if(GShouldLogOutAFrameOfMoveComponent)
		{
			...
			if(Depth == 0)
			{
				for(int32 MoveIdx=0; MoveIdx<Moves.Num(); MoveIdx++)
				{
					...
					UE_LOGF(LogPrimitiveComponent, Log, "MOVE%ls - %ls %5.2fms (%f %f %f) %d %d %ls", *Indent, *Move.Actor->GetName(), Move.Time * 1000.f, Move.Delta.X, Move.Delta.Y, Move.Delta.Z, Move.bDidLineCheck, Move.bDidEncroachCheck, *Move.Actor->GetDetailedInfo());
				}
	...
	if (TestHit.bBlockingHit)
	{
		...
		if ( (TestHit.Distance < PrimitiveComponentCVars::HitDistanceToleranceCVar || TestHit.bStartPenetrating) && !(MoveFlags & MOVECOMP_NeverIgnoreBlockingOverlaps) )
		{
	...
	#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
			{
				...
				{
					UE_LOGF(LogTemp, Log, "Overlapping %ls Dir %ls Dot %f Normal %ls Depth %f", *GetNameSafe(TestHit.Component.Get()), *MovementDir.ToString(), MoveDot, *TestHit.ImpactNormal.ToString(), TestHit.PenetrationDepth);
...
bool UPrimitiveComponent::MoveComponentImpl( const FVector& Delta, const FQuat& NewRotationQuat, bool bSweep, FHitResult* OutHit, EMoveComponentFlags MoveFlags, ETeleportType Teleport)
{
...
#endif 
			static const FName TraceTagName = "MoveComponent";
	...
	if( MSec > PERF_SHOW_MOVECOMPONENT_TAKING_LONG_TIME_AMOUNT )
	{
		if (GetOwner())
		{
			UE_LOGF(LogPrimitiveComponent, Log, "%10f executing MoveComponent for %ls owned by %ls", MSec, *GetName(), *GetOwner()->GetFullName() );
		}
		else
		{
			UE_LOGF(LogPrimitiveComponent, Log, "%10f executing MoveComponent for %ls", MSec, *GetFullName() );
		}
	}
#endif
...
bool UPrimitiveComponent::LineTraceComponent(FHitResult& OutHit, const FVector Start, const FVector End, ECollisionChannel TraceChannel, const struct FCollisionQueryParams& Params, const struct FCollisionResponseParams& ResponseParams, const struct FCollisionObjectQueryParams& ObjectParams)
{
	...
	if (FBodyInstance* ThisBodyInstance = GetBodyInstance(); ThisBodyInstance && ThisBodyInstance->IsValidBodyInstance())
	{
	...
	else
	{
		...
		if (CollisionInterface.LineTrace(Objects, Start, End, Params.bTraceComplex, BestHit))
		{
			...
			const Chaos::Filter::FQueryFilterData QueryFilter = Chaos::Filter::FQueryObjectFilterBuilder::BuildBlockAll();
...
bool UPrimitiveComponent::ComponentOverlapComponentImpl(class UPrimitiveComponent* PrimComp, const FVector Pos, const FQuat& Quat, const struct FCollisionQueryParams& Params)
{
	...
	if (OtherComp)
	{
		UE_LOGF(LogCollision, Warning, "ComponentOverlapMulti : (%ls) Does not support skeletalmesh with Physics Asset", *PrimComp->GetPathName());
		return false;
	}
	...
	if (InstancedStaticMesh)
	{
		if (FBodyInstance* ThisBodyInstance = GetBodyInstance())
		{
			...
			{
				return ThisBodyInstance->OverlapTestForBodies(Pos, Quat, InstancedStaticMesh->GetInstanceBodies());
			}
		}
		return false;
	}
...
void UPrimitiveComponent::BeginComponentOverlap(const FOverlapInfo& OtherOverlap, bool bDoNotifies)
{
	...
	if (!bComponentsAlreadyTouching)
	{
		...
		{
			//UE_LOGF(LogActor, Log, "BEGIN OVERLAP! Self=%ls SelfComp=%ls, Other=%ls, OtherComp=%ls", *GetNameSafe(this), *GetNameSafe(MyComp), *GetNameSafe(OtherComp->GetOwner()), *GetNameSafe(OtherComp));
			...
			AddUniqueOverlapFast(OtherComp->OverlappingComponents, FOverlapInfo(this, OtherOverlap.OverlapInfo.MyItem));	// uniqueness unverified, so addunique
			...
			if (bDoNotifies && ((World && World->HasBegunPlay()) || bLevelStreamingOverlap))
			{
				...
				if (IsValid(OtherComp))
				{
					...
					OtherComp->OnComponentBeginOverlap.Broadcast(OtherComp, MyActor, this, OtherOverlap.OverlapInfo.MyItem, OtherOverlap.bFromSweep, OtherOverlap.bFromSweep ? FHitResult::GetReversedHit(OtherOverlap.OverlapInfo) : OtherOverlap.OverlapInfo);
				}
...
void UPrimitiveComponent::EndComponentOverlap(const FOverlapInfo& OtherOverlap, bool bDoNotifies, bool bSkipNotifySelf)
{
	...
	const int32 OtherOverlapIdx = IndexOfOverlapFast(OtherComp->OverlappingComponents, FOverlapInfo(this, OtherOverlap.OverlapInfo.MyItem));
	...
	{
		//UE_LOGF(LogActor, Log, "END OVERLAP! Self=%ls SelfComp=%ls, Other=%ls, OtherComp=%ls", *GetNameSafe(this), *GetNameSafe(MyComp), *GetNameSafe(OtherActor), *GetNameSafe(OtherComp));
		GlobalOverlapEventsCounter++;
		...
		if (bDoNotifies && ((World && World->HasBegunPlay()) || bLevelStreamingOverlap))
		{
			...
			if (OtherActor)
			{
				...
				{
					OtherComp->OnComponentEndOverlap.Broadcast(OtherComp, MyActor, this, OtherOverlap.OverlapInfo.MyItem);
				}
	...
	if (GetGenerateOverlapEvents() && IsQueryCollisionEnabled())	//TODO: should modifying query collision remove from mayoverlapevents?
	{
		...
		if (MyActor)
		{
			...
			if (IsValidChecked(this) && GetGenerateOverlapEvents())
			{
				...
				{
					UE_LOGF(LogPrimitiveComponent, VeryVerbose, "%ls->%ls Skipping overlap test!", *GetNameSafe(GetOwner()), *GetName());
				...
				else
				{
					...
					UE_LOGF(LogPrimitiveComponent, VeryVerbose, "%ls->%ls Performing overlaps!", *GetNameSafe(GetOwner()), *GetName());
					UWorld* const MyWorld = GetWorld();
					TArray<FOverlapResult> Overlaps;
					...
					for (int32 ResultIdx=0; ResultIdx < Overlaps.Num(); ResultIdx++)
					{
						...
						if (HitComp && (HitComp != this) && HitComp->GetGenerateOverlapEvents())
						{
							...
							{
								FOverlapInfo& NewOverlap = OverlapMultiResult.Emplace_GetRef(HitComp, Result.GetItemIndex());		// don't need to add unique unless the overlap check can return dupes
								NewOverlap.OverlapInfo.MyItem = Result.MyItemIndex;
							}
						}
					}
...
void UPrimitiveComponent::UpdateBounds()
{
	...
	{
		if (!IsIgnoreStreamingManagerUpdate() && !IsHandledByStreamingManagerAsDynamic() && IsAttachedToStreamingManagerAsStatic())
		{
...
void UPrimitiveComponent::DispatchTouchHitResult(const FTouchId TouchId, const ETouchType::Type Type, const FHitResult& HitResult, const bool bHit,
	APlayerController* const Controller, const ETouchEventFamilies EnabledFamilies,
	TArrayView<TWeakObjectPtr<UPrimitiveComponent>> TouchedPrimitives)
{
	TWeakObjectPtr<UPrimitiveComponent>& Slot = TouchedPrimitives[TouchId.GetIndex()];
	UPrimitiveComponent* PreviousComponent = Slot.Get();
	UPrimitiveComponent* CurrentComponent = (bHit ? HitResult.Component.Get() : nullptr);

	const bool bBeginEnd = EnumHasAnyFlags(EnabledFamilies, ETouchEventFamilies::BeginEnd);
	const bool bOver = EnumHasAnyFlags(EnabledFamilies, ETouchEventFamilies::Over);

	switch (Type)
	{
	case ETouchType::Began:
		if (bBeginEnd && CurrentComponent != nullptr)
		{
			CurrentComponent->DispatchOnInputTouchBegin(TouchId, Controller, HitResult);
		}

		if (bOver)
		{
			DispatchTouchOverEvents(TouchId, Controller, HitResult, PreviousComponent, CurrentComponent);
	...
	case ETouchType::Ended:
		if (bBeginEnd && CurrentComponent != nullptr)
		{
			CurrentComponent->DispatchOnInputTouchEnd(TouchId, Controller, HitResult);
		}

		if (bOver)
		{
			// If the finger moved faster than tick and is being released over a different component than it was last dragged over,
			// notify the old component that the touch left it before reporting release on the current one.
			if (PreviousComponent != CurrentComponent && PreviousComponent != nullptr)
			{
				DispatchTouchOverEvents(TouchId, Controller, HitResult, PreviousComponent, CurrentComponent);
			}
...
void UPrimitiveComponent::DispatchOnInputTouchBegin(const FTouchId TouchId, APlayerController* const Controller, const FHitResult& HitResult)
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	DispatchOnInputTouchBegin(TouchId.GetIndex());
PRAGMA_ENABLE_DEPRECATION_WARNINGS

	if (IsValidChecked(this))
	{
		OnTouchBegan.Broadcast(TouchId, Controller, HitResult, this);
	}
}

void UPrimitiveComponent::DispatchOnInputTouchEnd(const FTouchId TouchId, APlayerController* const Controller, const FHitResult& HitResult)
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	DispatchOnInputTouchEnd(TouchId.GetIndex());
PRAGMA_ENABLE_DEPRECATION_WARNINGS

	if (IsValidChecked(this))
	{
		OnTouchEnded.Broadcast(TouchId, Controller, HitResult, this);
	}
}

void UPrimitiveComponent::DispatchTouchOverEvents(const FTouchId TouchId, APlayerController* const Controller, const FHitResult& HitResult, UPrimitiveComponent* const CurrentComponent, UPrimitiveComponent* const NewComponent)
{
PRAGMA_DISABLE_DEPRECATION_WARNINGS
	DispatchTouchOverEvents(TouchId.GetIndex(), CurrentComponent, NewComponent);
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	if (CurrentComponent != NewComponent)
	{
		if (IsValid(CurrentComponent))
		{
			CurrentComponent->OnTouchExited.Broadcast(TouchId, Controller, HitResult, CurrentComponent);
		}
		if (IsValid(NewComponent))
		{
			NewComponent->OnTouchEntered.Broadcast(TouchId, Controller, HitResult, NewComponent);
		}
	}
}
...
void UPrimitiveComponent::SetRenderCustomDepth(bool bValue)
{
	if( bRenderCustomDepth != bValue )
	{
		...
		// Precache the PSOs again when custom depth mode is set to 1 (only precache custom depth PSOs when bRenderCustomDepth is explicitly set)
		if (UPSOPrecacheSettingsManager::GetSettings().CustomDepthMode == 1)
		{
			PrecachePSOs();
		}
...
{
	SceneData.SetLastRenderTime(InLastRenderTime);
...
float UPrimitiveComponent::GetLastRenderTime() const
{
	...
	return SceneData.GetLastRenderTime();
}
...
float UPrimitiveComponent::GetLastRenderTimeOnScreen() const
{
	...
	return SceneData.GetLastRenderTimeOnScreen();
}
...
void UPrimitiveComponent::PrecachePSOs()
{
	...
	TRACE_CPUPROFILER_EVENT_SCOPE(UPrimitiveComponent::PrecachePSOs);
	...
	SetMaterialPSOPrecacheData(MaterialPSOPrecacheRequestIDs, PSOPrecacheCompileEvents);
#endif
}

void UPrimitiveComponent::SetMaterialPSOPrecacheData(TArray<FMaterialPSOPrecacheRequestID>& MaterialPSOPrecacheRequestIDs, const FGraphEventArray& PSOPrecacheCompileEvents)
{
#if UE_WITH_PSO_PRECACHING
	auto Callback = [this]()
	{
		if (GetWorld())
		{
			GetWorld()->AddPrimitiveComponentPSOPrecacheCompleted(this);
		}
	};
	PSOPrecacheComponentData.SetMaterialPSOPrecacheData(this, MaterialPSOPrecacheRequestIDs, PSOPrecacheCompileEvents, Callback);
...
void UPrimitiveComponent::AssignSceneProxy(FPrimitiveSceneProxy* InSceneProxy)
{
	...
	if (SceneData.SceneProxy)
	{
		...
		{
			SceneData.OwnerLastRenderTimePtr->IncrementAlwaysVisibleComponents();
		}
...
void UPrimitiveComponent::PostRename(UObject* OldOuter, const FName OldName)
{
	...
	if (SceneData.bAlwaysVisible && OldLastRenderTimePtr)
	{
		OldLastRenderTimePtr->DecrementAlwaysVisibleComponents();
	}

	SceneData.OwnerLastRenderTimePtr = NewLastRenderTimePtr;

	if (SceneData.bAlwaysVisible && NewLastRenderTimePtr)
	{
		NewLastRenderTimePtr->IncrementAlwaysVisibleComponents();
	}
}
...
void UPrimitiveComponent::ReleaseSceneProxy()
{
	...
	if (SceneData.bAlwaysVisible && SceneData.OwnerLastRenderTimePtr)
	{
		const int32 NumRefs = SceneData.OwnerLastRenderTimePtr->DecrementAlwaysVisibleComponents();
		check(NumRefs > 0);
	}
