...
namespace UE::MaterialAttributeDefinitionMap
{

FRWLock GlobalDDCStringLock;
...
{
	FReadScopeLock ReadScopeLock(UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock);
...
FText FMaterialAttributeDefinitionMap::GetDisplayNameForMaterial(EMaterialProperty Property, UMaterial* Material)
{
	...
	FReadScopeLock ReadScopeLock(UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock);
	FMaterialAttributeDefintion* Attribute = GMaterialPropertyAttributesMap.Find(Property);
	return GetAttributeOverrideForMaterialInLock(Attribute->AttributeID, Material);
}
...
FText FMaterialAttributeDefinitionMap::GetDisplayNameForMaterial(const FGuid& AttributeID, UMaterial* Material)
{
	...
	FReadScopeLock ReadScopeLock(UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock);
	FMaterialAttributeDefintion* Attribute = GMaterialPropertyAttributesMap.Find(AttributeID);
	return GetAttributeOverrideForMaterialInLock(AttributeID, Material);
}
...
{
	// GetIndexInLock that takes a Property actually doesn't need the lock, so
	// we don't enter it here; this function is just an alias for GetIndexInLock.
	return GetIndexInLock(Property);
}

int32 FMaterialAttributeDefinitionMap::GetIndexInLock(EMaterialProperty Property)
{
...
{
	FReadScopeLock ReadScopeLock(UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock);
	return GetIndexInLock(AttributeID);
}

int32 FMaterialAttributeDefinitionMap::GetIndexInLock(const FGuid& AttributeID)
{
...
{
	// GetBitmaskInLock that takes a Property actually doesn't need the lock, so
	// we don't enter it here; this function is just an alias for GetBitmaskInLock.
	return GetBitmaskInLock(Property);
}

uint64 FMaterialAttributeDefinitionMap::GetBitmaskInLock(EMaterialProperty Property)
{
	const int32 Index = FMaterialAttributeDefinitionMap::GetIndexInLock(Property);
	if (Index != INDEX_NONE)
	{
...
uint64 FMaterialAttributeDefinitionMap::GetBitmask(const FGuid& AttributeID)
{
	...
	return GetBitmaskInLock(AttributeID);
}

uint64 FMaterialAttributeDefinitionMap::GetBitmaskInLock(const FGuid& AttributeID)
{
	const int32 Index = FMaterialAttributeDefinitionMap::GetIndexInLock(AttributeID);
	if (Index != INDEX_NONE)
	{
		const uint64 Result = 1ull << static_cast<uint32>(Index);
		return Result;
...
void FMaterialAttributeDefinitionMap::InitializeAttributeMap()
{
	...
	// Basic attributes
	Add(FGuid(0x69B8D336, 0x16ED4D49, 0x9AA49729, 0x2F050F7A), TEXT("BaseColor"),		MP_BaseColor,		MCT_Float3,	FVector4(0,0,0,1),	SF_Pixel);
	...
	Add(FGuid(0x5973A03E, 0x13A74E08, 0x92D0CEDD, 0xF2936CF8), TEXT("FrontMaterial"),			MP_FrontMaterial,			MCT_Substrate, FVector4(0,0,0,0),	SF_Pixel);
	...
	AddCustomAttributeInner(FGuid(0xfbd7b46e, 0xb1234824, 0xbde76b23, 0x609f984c), UMaterialExpressionBentNormalCustomOutput::StaticClass(), "BentNormal", "GetBentNormal", 0, MCT_Float3, FVector4(0, 0, 1, 0), SF_Pixel);
	AddCustomAttributeInner(FGuid(0xAA3D5C04, 0x16294716, 0xBBDEC869, 0x6A27DD72), UMaterialExpressionClearCoatNormalCustomOutput::StaticClass(), "ClearCoatBottomNormal", "ClearCoatBottomNormal", 0, MCT_Float3, FVector4(0, 0, 1, 0), SF_Pixel);
	AddCustomAttributeInner(FGuid(0x8EAB2CB2, 0x73634A24, 0x8CD14F47, 0x3F9C8E55), UMaterialExpressionTangentOutput::StaticClass(), "CustomEyeTangent", "GetTangentOutput", 0, MCT_Float3, FVector4(0, 0, 0, 0), SF_Pixel);
	AddCustomAttributeInner(FGuid(0xF2D8C70E, 0x42ECA0D1, 0x4652D0AD, 0xB785A065), UMaterialExpressionThinTranslucentMaterialOutput::StaticClass(), "TransmittanceColor", "GetThinTranslucentMaterialOutput", 0, MCT_Float3, FVector4(0.5, 0.5, 0.5, 0), SF_Pixel);
	AddCustomAttributeInner(FGuid(0xAAA1D0E0, 0xE312458E, 0xF3556BAD, 0xA01277CD), UMaterialExpressionThinTranslucentMaterialOutput::StaticClass(), "ThinTranslucentSurfaceCoverage", "GetThinTranslucentMaterialOutput", 1, MCT_Float, FVector4(1, 0, 0, 0), SF_Pixel);
	AddCustomAttributeInner(FGuid(0xDB380E51, 0xBDED47B6, 0xB185A8BE, 0x8EACBFA4), UMaterialExpressionFirstPersonOutput::StaticClass(), "FirstPersonInterpolationAlpha", "GetFirstPersonOutput", 0, MCT_Float, FVector4(1, 0, 0, 0), SF_Vertex, true /*bNeedsPreviousFrameEvaluation*/);
	AddCustomAttributeInner(FGuid(0x6B1237CA, 0x1B8C43C2, 0xA9C8A020, 0x430F1258), UMaterialExpressionMaterialCache::StaticClass(), "MaterialCache", "MaterialCache", 0, MCT_Float4, FVector4(0, 0, 0, 0), SF_Pixel);
	AddCustomAttributeInner(FGuid(0xB58CD3FB, 0x8CB4411D, 0x83C4C11E, 0xF89953AD), UMaterialExpressionTemporalResponsivenessOutput::StaticClass(), "TemporalResponsiveness", "GetTemporalResponsivenessOutput", 0, MCT_Float, FVector4(0, 0, 0, 0), SF_Pixel);
	AddCustomAttributeInner(FGuid(0x48A2CEAB, 0x8E63473F, 0x821345BC, 0x6FA907F7), UMaterialExpressionMotionVectorWorldOffsetOutput::StaticClass(), "MotionVectorWorldOffset", "GetMotionVectorWorldOffsetOutput", 0, MCT_Float3, FVector4(0, 0, 0, 0), SF_Pixel);
}
...
FMaterialAttributeDefintion* FMaterialAttributeDefinitionMap::Find(const FGuid& AttributeID)
{
	...
	UE_LOGF(LogMaterial, Warning, "Failed to find material attribute, AttributeID: %ls.", *AttributeID.ToString(EGuidFormats::Digits));
	return Find(MP_MAX);
}
...
FMaterialAttributeDefintion* FMaterialAttributeDefinitionMap::Find(EMaterialProperty Property)
{
	...
	UE_LOGF(LogMaterial, Warning, "Failed to find material attribute, PropertyType: %i.", (uint32)Property);
	return Find(MP_MAX);
}

FText FMaterialAttributeDefinitionMap::GetAttributeOverrideForMaterialInLock(const FGuid& AttributeID, UMaterial* Material)
{
	...
	const bool bHasSubstrateFrontMaterialConnected = Material->HasSubstrateFrontMaterialConnected();
	...
	case MP_Opacity:
		return bSubstrateEnabled && bHasSubstrateFrontMaterialConnected ? LOCTEXT("OpacityOverride", "Opacity Override") : LOCTEXT("Opacity", "Opacity");
	case MP_OpacityMask:
	...
	case MP_Metallic:
		...
		return FText::FromString(GetPinNameFromShadingModelFieldInLock(Material->GetShadingModels(), CustomPinNames,
			LOCTEXT("Metallic", "Metallic").ToString()));
	case MP_Specular:
		return FText::FromString(GetPinNameFromShadingModelFieldInLock(Material->GetShadingModels(), CustomPinNames,
			LOCTEXT("Specular", "Specular").ToString()));
	case MP_Roughness:
		return LOCTEXT("Roughness", "Roughness");
	case MP_Anisotropy:
		return FText::FromString(GetPinNameFromShadingModelFieldInLock(Material->GetShadingModels(), CustomPinNames,
			LOCTEXT("Anisotropy", "Anisotropy").ToString()));
	case MP_Normal:
		CustomPinNames.Add({ MSM_Hair, LOCTEXT("Tangent", "Tangent").ToString() });
		return FText::FromString(GetPinNameFromShadingModelFieldInLock(Material->GetShadingModels(), CustomPinNames,
			LOCTEXT("Normal", "Normal").ToString()));
	case MP_Tangent:
	...
	case MP_SubsurfaceColor:
		...
		return FText::FromString(GetPinNameFromShadingModelFieldInLock(Material->GetShadingModels(), CustomPinNames,
			LOCTEXT("SubsurfaceColor", "Subsurface Color").ToString()));
	case MP_CustomData0:
		...
		return FText::FromString(GetPinNameFromShadingModelFieldInLock(Material->GetShadingModels(), CustomPinNames,
			LOCTEXT("CustomData0", "Custom Data 0").ToString()));
	case MP_CustomData1:
		...
		return FText::FromString(GetPinNameFromShadingModelFieldInLock(Material->GetShadingModels(), CustomPinNames,
			LOCTEXT("CustomData1", "Custom Data 1").ToString()));
...
FString FMaterialAttributeDefinitionMap::GetPinNameFromShadingModelFieldInLock(FMaterialShadingModelField InShadingModels,
	const TArray<TKeyValuePair<EMaterialShadingModel, FString>>& InCustomShadingModelPinNames,
	const FString& InDefaultPinName)
{
	FString OutPinName;
...
void FMaterialAttributeDefinitionMap::AppendDDCKey(FShaderKeyGenerator& KeyGen)
{
	{
		FReadScopeLock ReadScopeLock(UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock);
		const FString& DDCString = GMaterialPropertyAttributesMap.AttributeDDCString;
		if (DDCString.Len() != 0)
		{
	...
	FWriteScopeLock WriteScopeLock(UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock);
	...
	MaterialAttributeBlendFunction BlendFunction /*= nullptr*/)
{
	...
	GMaterialPropertyAttributesMap.AddCustomAttributeInner(AttributeID, ExpressionClass, AttributeName, FunctionName,
		OutputIndex, ValueType, DefaultValue, ShaderFrequency, bNeedsPreviousFrameEvaluation, BlendFunction);
}

void FMaterialAttributeDefinitionMap::AddCustomAttributeInner(
	const FGuid& AttributeID,
	UClass* ExpressionClass,
	const FString& AttributeName,
	const FString& FunctionName,
	...
	MaterialAttributeBlendFunction BlendFunction /*= nullptr*/)
{
	...
	check(AttributeDDCString.Len() == 0);
	...
	checkf(!CustomAttributes.Contains(UserAttribute), TEXT("Tried to add duplicate custom output attribute (%s)."), *AttributeName);
#endif
	CustomAttributes.Add(UserAttribute);
...
{
	// This lock is mostly useless since it goes out of scope before we finish reading the out result
	FReadScopeLock ReadScopeLock(UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock);
...
TOptional<FMaterialCustomOutputAttributeDefintion> FMaterialAttributeDefinitionMap::GetCustomAttributeCopy(const FString& AttributeName)
{
	FReadScopeLock ReadScopeLock(UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock);
	...
	return TOptional<FMaterialCustomOutputAttributeDefintion>();
}
...
TOptional<FMaterialCustomOutputAttributeDefintion> FMaterialAttributeDefinitionMap::GetCustomAttributeCopy(const FGuid& AttributeID)
{
	FReadScopeLock ReadScopeLock(UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock);
...
const TArray<FGuid>& FMaterialAttributeDefinitionMap::GetOrderedVisibleAttributeList()
{
	// This lock is mostly useless since it goes out of scope before we finish reading the out result
	FReadScopeLock ReadScopeLock(UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock);
...
FMaterialAttributeDefinitionMap::FReadLockedOrderVisibleAttributeList::FReadLockedOrderVisibleAttributeList()
	: List(GMaterialPropertyAttributesMap.OrderedVisibleAttributeList)
{
	UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock.ReadLock();
}

FMaterialAttributeDefinitionMap::FReadLockedOrderVisibleAttributeList::~FReadLockedOrderVisibleAttributeList()
{
	UE::MaterialAttributeDefinitionMap::GlobalDDCStringLock.ReadUnlock();
}

FMaterialAttributeDefinitionMap::FReadLockedOrderVisibleAttributeList FMaterialAttributeDefinitionMap::GetReadLockedOrderedVisibleAttributeList()
{
	return FReadLockedOrderVisibleAttributeList();
}
	...
	for (const UMaterialExpressionCustomOutput* Expression : Expressions)
	{
		if (Expression)
		{
			for (auto& Attribute : GMaterialPropertyAttributesMap.CustomAttributes)
			{
				...
				{
					Out |= GetBitmaskInLock(Attribute.AttributeID);
				}
			}
		}
	}
