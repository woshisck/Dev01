...
bool GEnableObjectWorldPositionFix = true;
static FAutoConsoleVariableRef CVarEnableObjectWorldPositionFix(
	TEXT("r.EnableObjectWorldPositionFix"),
	GEnableObjectWorldPositionFix,
	TEXT("If enabled (default) any primitives where HasInstanceDataBuffers() is false will use the previous behavior for calculating ObjectWorldPosition."),
	FConsoleVariableDelegate::CreateLambda([](IConsoleVariable* InVariable)
	{
		FGlobalComponentRecreateRenderStateContext Context;
	}),
...
// NOTE: required because of: bCanSkipRedundantTransformUpdates, remove when cleaned up
...
FPrimitiveSceneProxy::FPrimitiveSceneProxy(FPrimitiveSceneProxy&&) = default;
FPrimitiveSceneProxy& FPrimitiveSceneProxy::operator=(const FPrimitiveSceneProxy&) = default;
FPrimitiveSceneProxy& FPrimitiveSceneProxy::operator=(FPrimitiveSceneProxy&&) = default;
...
,	bVisibleInReflections(InProxyDesc.bVisibleInReflections)
...
,	bSelfShadowOnly(InProxyDesc.bSelfShadowOnly && !InProxyDesc.bCastShadowOnOtherObjectsOnly)
,	bCastShadowOnOtherObjectsOnly(InProxyDesc.bCastShadowOnOtherObjectsOnly)
,	bCastInsetShadow((InProxyDesc.bSelfShadowOnly || InProxyDesc.bCastShadowOnOtherObjectsOnly) ? true : InProxyDesc.bCastInsetShadow)	// Shadow-only modes require an inset shadow.
...
,	MinDrawDistance(InProxyDesc.MinDrawDistance)
{
	...
	PSOPrecacheRequestsToBoostOnDraw = InProxyDesc.MaterialPSOPrecacheRequestIDsToBoostOnDraw;
#endif
...
{
	// Safety net for proxy-deletion paths that bypass FScene::RemovePrimitive (proxy-only
	// callers without a UPrimitiveComponent, error paths that abort before AddPrimitive
	// completes, custom proxy lifecycles, unit tests). Without this the slot in
	// PrimitiveHandleTable plus any FAssetBoundElement rows accumulated from queued
	// FRegister processing would leak. Idempotent against the component-side recovery in
	// UPrimitiveComponent::BeginDestroy: RecoverMissedUnregister re-checks
	// IsPrimitiveHandleLive, so whichever path fires second is a no-op.
	if (FSimpleStreamableAssetManager::IsEnabled())
	{
		const FSSAMPrimitiveHandle Captured = SSAMPrimitiveHandle.Exchange(FSSAMPrimitiveHandle{});
		if (Captured.IsValid() && !Captured.IsStale())
		{
			FSimpleStreamableAssetManager::RecoverMissedUnregister(Captured);
		}
	}
}
...
FMatrix FPrimitiveSceneProxy::GetMeshCardsToWorld(int32 InstanceIndex) const
{
	if (const FInstanceSceneDataBuffers* InstanceData = GetInstanceSceneDataBuffers())
	{
		// Instance data must be available on CPU.
		check(!InstanceData->IsInstanceDataGPUOnly());

		const int32 PrimitiveInstanceIndex = FMath::Clamp(InstanceIndex, 0, InstanceData->GetNumInstances() - 1);
		return InstanceData->GetInstanceToWorld(PrimitiveInstanceIndex);
	}
	else
	{
...
{
	Builder = FPrimitiveUniformShaderParametersBuilder{}.Defaults();
	// 1. Set up parameters that depend on the proxy.
	BuildUniformShaderParametersInternal(Builder);
	// 2. Set up those that depend on renderer internal representations.
	Scene->BuildPrimitiveUniformShaderParameters_RenderThread(PrimitiveSceneInfo, Builder);
}

void FPrimitiveSceneProxy::BuildUniformShaderParametersInternal(FPrimitiveUniformShaderParametersBuilder& Builder) const
{
	...
	// Update the uniform shader parameters.
	Builder.ActorWorldPosition(ActorPosition)
			.WorldBounds(Bounds)
			...
			.VisibleInMainPass(ShouldRenderInMainPass())
			...
			.EnableObjectWorldPositionFix(!HasInstanceDataBuffers() && GEnableObjectWorldPositionFix)
	...
	// Note, always set the value to useful defaults (InstancingFadeOutParams.y == 0.0f means it works out to fully visible)
	FVector2f InstancingFadeOutParams = FVector2f::ZeroVector;
	GetInstancingFadeOutParams(InstancingFadeOutParams);
	Builder.InstancingFadeOutParams(InstancingFadeOutParams);
...
void FPrimitiveSceneProxy::UpdateInstances_RenderThread(FRHICommandListBase& RHICmdList, const FBoxSphereBounds& InBounds, const FBoxSphereBounds& InLocalBounds)
{
	...
	bAlwaysHasVelocity = CVarVelocityForceOutput.GetValueOnAnyThread() || GetInstanceDataHeader().Flags.bHasPerInstanceDynamicData || GetInstanceDataHeader().Flags.bHasPerInstanceSkinningData;
}
...
/**
* Set the DrawInGame and the DrawInEditor flags (RENDER THREAD)
*
* @param the new value
*/
void FPrimitiveSceneProxy::SetIsDrawn_RenderThread(bool bIsDrawn)
{
	check(IsInRenderingThread());
	
	if ((DrawInGame != DrawInEditor) || (DrawInGame != bIsDrawn))
	{
		DrawInGame = bIsDrawn;
		DrawInEditor = bIsDrawn;
...
void FPrimitiveSceneProxy::SetLightingChannels_GameThread(FLightingChannels LightingChannels)
{
		...
		[PrimitiveSceneProxy, LocalLightingChannelMask, Scene = Scene, PrimitiveSceneInfo = PrimitiveSceneInfo](FRHICommandListImmediate& RHICmdList)
	{
		...
		Scene->UpdateCachedRayTracingState(PrimitiveSceneProxy, EPrimitiveDirtyRayTracingState::Default | EPrimitiveDirtyRayTracingState::LightingChannelMask);
...
{
	ensureMsgf(!bSupportsMaterialCache || MaterialCacheDescriptor.TagOffset != UINT16_MAX, TEXT("Primitive writes to material cache with an unallocated descriptor"));
	return MaterialCacheDescriptor.Opaque;
}
...
void FPrimitiveSceneProxy::EnableGPUSceneSupportFlags()
{
	...
	const bool bUseGPUScene = UseGPUScene(ShaderPlatform);
	const bool bMobilePath = IsMobilePlatform(ShaderPlatform);
...
void FPrimitiveSceneProxy::BoostPrecachedPSORequestsOnDraw()
{
	...
	{
		TArray<FMaterialPSOPrecacheRequestID> LocalPSOPrecacheRequestsToBoostOnDraw = MoveTemp(PSOPrecacheRequestsToBoostOnDraw);
		BoostPSOPriorityOnDraw(LocalPSOPrecacheRequestsToBoostOnDraw);
	}
}
#endif // UE_WITH_PSO_PRECACHING

#if RHI_RAYTRACING
ERayTracingPrimitiveFlags FPrimitiveSceneProxy::GetRayTracingPrimitiveFlags()
{
	...
	// TODO: Ideally all these other conditions should be folded into HasRayTracingRepresentation() 
	// to avoid primitives being unecessarily registered with FRayTracingGeometryManager / Nanite::FRayTracingManager
	// however would need to make sure they would get registered/unregistered as the conditions change
	...
	if (bSkinnedMesh && RayTracingSkeletalMeshesCVar && RayTracingSkeletalMeshesCVar->GetValueOnRenderThread() <= 0)
	{
		return ERayTracingPrimitiveFlags::Exclude;
	}
...
void FPrimitiveSceneProxy::GetStaticRayTracingInstances(FStaticRayTracingInstances& OutRayTracingInstances)
{
#if DO_CHECK
	ERayTracingPrimitiveFlags Flags = GetRayTracingPrimitiveFlags();

	if (EnumHasAnyFlags(Flags, ERayTracingPrimitiveFlags::CacheInstances))
	{
		checkf(false, TEXT("Proxy classes returning ERayTracingPrimitiveFlags::CacheInstances must override GetStaticRayTracingInstances(...)"));
	}
#endif
}
...
ERayTracingPrimitiveFlags FPrimitiveSceneProxy::GetCachedRayTracingInstance(FRayTracingInstance& OutRayTracingInstance)
{
	...
	GetStaticRayTracingInstances(StaticRayTracingInstances);
	if (StaticRayTracingInstances.LODs.Num() > 0 && StaticRayTracingInstances.LODs[0].Instances.Num() > 0)
	{
		OutRayTracingInstance = StaticRayTracingInstances.LODs[0].Instances[0];
	}
	return Flags;
}
...
ERayTracingProxyType FPrimitiveSceneProxy::GetRayTracingProxyType() const
{
	...
	if (bIsStaticMesh || bIsNaniteMesh)
	{
