...
bool GAllowSkipAlphaComputationForMaterialBlend = true;
static FAutoConsoleVariableRef CVarAllowSkipAlphaComputationForMaterialBlend(
	TEXT("r.MaterialEditor.AllowSkipAlphaComputationForMaterialBlend"),
	GAllowSkipAlphaComputationForMaterialBlend,
	TEXT("If true, Allow to skip alpha computation when evaluating material blend nodes if the 2 values are equal"),
	ECVF_ReadOnly
);

static void ThrowSubstrateUnsupportedSwitchNodeError(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, const FString& NodeClassName)
{
	SubstrateTranslatorData.Errorf(FString::Printf(TEXT("%s nodes do not support flowing Substrate data.\nThis is because the Substrate tree representing material topology must be known at translation time.\nBut this node uses branching with usage known at compilation time using #define or at runtime using dynamic branching."), *NodeClassName));
}
...
EMaterialValueType UMaterialExpression::GetCompositeOutput(int32 OutputIndex)
{
...
TArray<FProperty*> UMaterialExpression::GetInputPinProperty(int32 PinIndex)
{
	...
	if (FExpressionInput* Input = GetInput(PinIndex))
	{
		...
		static FName OverridingInputPropertyMetaData("OverridingInputProperty");
	...
	for (int32 i = 0; i < Properties.Num(); ++i)
	{
		...
		else if (PropertyClass == FStructProperty::StaticClass())
		{
			...
			if (Struct == TBaseStructure<FLinearColor>::Get())
			{
				...
				{
					// This is a 3 element vector
					PropertyValue = FString::SanitizeFloat(Value.R) + FString(",") + FString::SanitizeFloat(Value.G) + FString(",") + FString::SanitizeFloat(Value.B);
				}
				else
				{
					// This is a 4 element vector
			...
			{
				FVector4 Value = *Property->ContainerPtrToValuePtr<FVector4>(this);		
				PropertyValue = FString::SanitizeFloat(Value.X) + FString(",") + FString::SanitizeFloat(Value.Y) + FString(",") + FString::SanitizeFloat(Value.Z) + FString(",") + FString::SanitizeFloat(Value.W);
			}
			else if (Struct == TVariantStructure<FVector4d>::Get())
			{
				FVector4d Value = *Property->ContainerPtrToValuePtr<FVector4d>(this);		
			...
			{
				FVector Value = *Property->ContainerPtrToValuePtr<FVector>(this);
				PropertyValue = FString::SanitizeFloat(Value.X) + FString(",") + FString::SanitizeFloat(Value.Y) + FString(",") + FString::SanitizeFloat(Value.Z);
			}
			else if (Struct == TVariantStructure<FVector3f>::Get())
			{
				FVector3f Value = *Property->ContainerPtrToValuePtr<FVector3f>(this);
...
TArray<FProperty*> UMaterialExpression::GetPropertyInputs() const
{
	...
	static FName OverridingInputPropertyMetaData("OverridingInputProperty");
	static FName ShowAsInputPinMetaData("ShowAsInputPin");
	for (TFieldIterator<FProperty> InputIt(GetClass(), EFieldIteratorFlags::IncludeSuper, EFieldIteratorFlags::ExcludeDeprecated); InputIt; ++InputIt)
	{
...
void UMaterialExpression::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	if( PropertyThatChanged != nullptr )
	{
		...
		const FName ParameterName = "ParameterName";
...
bool UMaterialExpression::CanEditChange(const FProperty* InProperty) const
{
	...
	if (bIsEditable && InProperty != nullptr)
	{
		...
		if (InProperty->HasMetaData(OverridingInputPropertyMetaData))
		{
			...
			if (ensure(StructProp != nullptr))
			{
				static FName RequiredInputMetaData("RequiredInput");
...
uint32 UMaterialExpression::GetOutputType(int32 OutputIndex)
{
	...
	EMaterialValueType CompositeResultOutput = GetCompositeOutput(OutputIndex);
	if (CompositeResultOutput != MCT_Unknown)
	{
		return CompositeResultOutput;
	}

	TArray<FExpressionOutput>& CurrentOutputs = GetOutputs();
	if (!CurrentOutputs.IsValidIndex(OutputIndex))
	{
		return MCT_Unknown;
	}

	FExpressionOutput& Output = CurrentOutputs[OutputIndex];
	if (Output.Mask)
	{
...
// A non public function to know if a material is a preview material that needs to be unlit.
// This is required for instance when previewing a material function using the UI domain. It needs unlit to fetch the right data and Substrate also uses unlit behind the scene to display that domain.
// Also this is used to maintain the correct MaterialDomain since Substrate can convert that otherwise in RebuildShadingModelField. 
// When in fact we must keep that for the ExpressionPreviewMaterial created from MaterialEditor.cpp for MAterialFunction asset preview for instance.
bool IsPreviewUnlitMaterial(const UMaterial* InMaterial)
{
	return InMaterial && InMaterial->MaterialDomain != MD_Surface && InMaterial->bIsPreviewMaterial && InMaterial->IsA(UPreviewMaterial::StaticClass());
}
...
void UMaterialExpression::ConnectToPreviewMaterial(UMaterial* InMaterial, int32 OutputIndex)
{
	...
	if (InMaterial && OutputIndex >= 0 && OutputIndex < Outputs.Num())
	{
		if (Substrate::IsSubstrateEnabled())
		{
			...
			{
				if (IsPreviewUnlitMaterial(InMaterial))
				{
			...
			else if (IsResultMaterialAttributes(0))
			{
				...
				if (!InMaterial->IsShadingModelFromMaterialExpression())
				{
					// Only set DefaultLit if we are not using the special FromMaterialExpression already as this is forbidden by SetShadingModel and Substrate deal with that case.
					InMaterial->SetShadingModel(MSM_DefaultLit);
					InMaterial->bUseMaterialAttributes = false;
				}
				else
				...
				if (IsPreviewUnlitMaterial(InMaterial))
				{
					ConvertAttributeNode->ShadingModelOverride = MSM_Unlit;
				}
				else
				{
...
{
	// Deprecated so call the general version.
	return SubstrateGenerateMaterialTopologyTree(Compiler->GetSubstrateTranslatorData(), Parent, OutputIndex);
}

FSubstrateOperator* UMaterialExpression::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	SubstrateTranslatorData.Errorf(FString::Printf(TEXT("%s nodes type does not support generating/processing/flowing Substrate data.\nPlease reach out to your development team for implementation, if it is supported and can be implemented."), *GetClass()->GetName()));
	return nullptr;
}
...
void UMaterialExpressionTextureBase::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		// Material is null when this expression lives inside a UMaterialFunction; the mesh paint default
		// only applies within a concrete UMaterial, so skip the duplicate-default check in that case.
		if (!Material)
		{
			return;
		}
...
void UMaterialExpressionTextureBase::AutoSetSampleType()
{
	if (Texture)
	{
		SamplerType = MaterialExpressionUtils::GetSamplerTypeForTexture(Texture);
	}
}

EMaterialSamplerType UMaterialExpressionTextureBase::GetSamplerTypeForTexture(const UTexture* Texture, bool ForceNoVT)
{
	return MaterialExpressionUtils::GetSamplerTypeForTexture(Texture, ForceNoVT);
}
...
bool UMaterialExpressionTextureBase::VerifySamplerType(const FString& TexturePathName, EMaterialSamplerType CorrectSamplerType, bool bSRGB, EMaterialSamplerType SamplerType, FString& OutErrorMessage)
{
	return MaterialExpressionUtils::VerifySamplerType(TexturePathName, CorrectSamplerType, bSRGB, SamplerType, OutErrorMessage);
}

bool UMaterialExpressionTextureBase::VerifySamplerType(EShaderPlatform ShaderPlatform, const ITargetPlatform* TargetPlatform, const UTexture* Texture, EMaterialSamplerType SamplerType, FString& OutErrorMessage)
{
	return MaterialExpressionUtils::VerifySamplerType(ShaderPlatform, TargetPlatform, Texture, SamplerType, OutErrorMessage);
}
...
int32 UMaterialExpressionTextureSample::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	if (Texture || InputExpression) // We deal with reroute textures later on in this function..
	{
		...
		if (EffectiveTexture && MaterialExpressionUtils::VerifySamplerType(Compiler->GetShaderPlatform(), Compiler->GetTargetPlatform(), EffectiveTexture, EffectiveSamplerType, SamplerTypeError))
		{
			if (TOptional<int32> MissingError = CheckForMissingUVWInput(TextureCodeIndex))
			{
				return *MissingError;
...
{
	OutCaptions.Add(FString("Runtime Virtual Texture Output"));
}
...
bool UMaterialExpressionRuntimeVirtualTextureSample::ValidateVirtualTextureParameters(FString& OutError, bool& bOutIsVirtualTextureValid) const
{
	bOutIsVirtualTextureValid = VirtualTexture != nullptr;
	if (!bOutIsVirtualTextureValid)
	{
		if (IsParameter())
		{
	...
	else if (VirtualTexture->GetMaterialType() != MaterialType)
	{
		...
		bOutIsVirtualTextureValid = false;
	}
...
{
	OutCaptions.Add(FString("Runtime Virtual Texture Sample"));
}
...
{
	OutCaptions.Add(FString("Runtime Virtual Texture Sample Param "));
...
int32 UMaterialExpressionTextureSampleParameter::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	FString SamplerTypeError;
	if (!MaterialExpressionUtils::VerifySamplerType(Compiler->GetShaderPlatform(), Compiler->GetTargetPlatform(), Texture, SamplerType, SamplerTypeError))
	{
		return Compiler->Errorf(TEXT("%s"), *SamplerTypeError);
	}
...
int32 UMaterialExpressionTextureObject::CompilePreview(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	return CompileTextureSample(Compiler, Texture, Compiler->TextureCoordinate(0, false, false), MaterialExpressionUtils::GetSamplerTypeForTexture( Texture ));
}
...
#if WITH_EDITOR
static FString BuildStaticComponentMaskOutputText(const bool R, const bool G, const bool B, const bool A)
{
	FString Str("( ");
	if (R) Str += TEXT("R ");
	if (G) Str += TEXT("G ");
...
{
	const FString Caption = FString("Mask ") + BuildStaticComponentMaskOutputText(R, G, B, A);
...
{
	const FString Caption = FString("Mask ") + BuildStaticComponentMaskOutputText(DefaultR, DefaultG, DefaultB, DefaultA);
	OutCaptions.Add(Caption);
}

void UMaterialExpressionStaticComponentMaskParameter::GetExpressionToolTip(TArray<FString>& OutToolTip)
{
	const FString MaskOutput = BuildStaticComponentMaskOutputText(DefaultR, DefaultG, DefaultB, DefaultA);

	OutToolTip.Add(TEXT("Outputs the selected channels."));
	OutToolTip.Add(TEXT("Note: This DOES create new permutations."));
	OutToolTip.Add(FString::Printf(TEXT("Current output: %s "), *MaskOutput));
}
...
bool  UMaterialExpressionStaticComponentMaskParameter::SetParameterValue(FName InParameterName, bool InR, bool InG, bool InB, bool InA, FGuid InExpressionGuid, EMaterialExpressionSetParameterValueFlags Flags)
{
	if (InParameterName == ParameterName)
	{
		const bool bHasRChanged = (DefaultR != InR);
		const bool bHasGChanged = (DefaultG != InG);
		const bool bHasBChanged = (DefaultB != InB);
		const bool bHasAChanged = (DefaultA != InA);

		if (bHasRChanged || bHasGChanged || bHasBChanged || bHasAChanged)
		{
			DefaultR = InR;
			DefaultG = InG;
			DefaultB = InB;
...
int32 UMaterialExpressionMakeMaterialAttributes::CompilePreview(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	// Material attributes "Compile" will return the value of the current attribute at the top of the compiler stack. By default it is EmissiveColor, so can be just black.
	// Instead we use BaseColor as a better preview representative for material attributes.
	FExpressionInput BaseColorTraced = BaseColor.GetTracedInput();
	UMaterialExpression* BaseColorExpression = BaseColor.Expression;
	if (BaseColorExpression)
	{
		return BaseColor.Compile(Compiler); 
	}
...
FSubstrateOperator* UMaterialExpressionGetMaterialAttributes::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	if (IsResultSubstrateMaterial(OutputIndex))
	{
		return MaterialAttributes.Expression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, Parent, MaterialAttributes.OutputIndex);
	}
	return nullptr;
}
...
FSubstrateOperator* UMaterialExpressionSetMaterialAttributes::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, UMaterialExpression* Parent, int32 OutputIndex)
{
	int32 InputIndex = INDEX_NONE;
	if (GetSubstrateMaterialInputIndex(OutputIndex, InputIndex))
	{
		return Inputs[InputIndex].Expression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, Parent, Inputs[InputIndex].OutputIndex);
	}
	return nullptr;
}
...
int32 UMaterialExpressionSetMaterialAttributes::CompilePreview(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	// Use the new base color as preview if specified
	int32 InputIndex = 0;
	for (const FGuid& AttributeType : AttributeSetTypes)
	{
		EMaterialProperty MaterialProperty = FMaterialAttributeDefinitionMap::GetProperty(AttributeType);
		if (MaterialProperty == MP_BaseColor && Inputs.Num() > (InputIndex + 1))
		{
			FExpressionInput BaseColorInput = Inputs[InputIndex + 1]; // +1 because first element is always material attribute input

			// Trace the base color expression to use as preview
			FExpressionInput BaseColorTraced = BaseColorInput.GetTracedInput();
			UMaterialExpression* BaseColorExpression = BaseColorTraced.Expression;
			if (BaseColorExpression)
			{
				return BaseColorInput.Compile(Compiler);
			}
		}
		InputIndex++;
	}

	// Otherwise display the one from the material attribute input
	if(Inputs.Num() > 0)
	{
		FExpressionInput MaterialAttributeInput = Inputs[0];
		FExpressionInput MaterialAttributeTraced = MaterialAttributeInput.GetTracedInput();
		UMaterialExpression* MaterialAttributeExpression = MaterialAttributeTraced.Expression;
		if (MaterialAttributeExpression)
		{
			// Material attributes "Compile" will return the value of the current attribute at the top of the compiler stack. By default it is EmissiveColor, so can be just black.
			// Instead we use BaseColor as a better preview representative for material attributes.
			return MaterialAttributeExpression->CompilePreview(Compiler, MaterialAttributeInput.OutputIndex);
		}
	}
...
static inline bool AllowSkipAlphaComputationForMaterialBlend()
{
	return GAllowSkipAlphaComputationForMaterialBlend;
}
...
int32 UMaterialExpressionBlendMaterialAttributes::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	if (AllowSkipAlphaComputationForMaterialBlend() && (ResultA == ResultB))
	{
		return ResultA;
	}
...
void UMaterialExpressionMaterialAttributeLayers::RebuildLayerGraph(bool bReportErrors)
{
	...
	{
		UE_LOGF(LogMaterial, Warning, "Failed to build layer graph for %ls.", Material ? *(Material->GetName()) : TEXT("Unknown"));
	}
}
...
void UMaterialExpressionMaterialAttributeLayers::LogError(FMaterialCompiler* Compiler, const TCHAR* Format, ...)
{
	...
	{
		UE_LOGF(LogMaterial, Warning, "%ls", ErrorMsg);
	}
}
...
const TCHAR* GetChannelMaskParameterName(TEnumAsByte<EChannelMaskParameterColor::Type> InMaskChannel)
{
	const TCHAR* Output = nullptr;
	switch (InMaskChannel)
	{
	case EChannelMaskParameterColor::Red:
		Output = TEXT("Red"); break;
	case EChannelMaskParameterColor::Green:
		Output = TEXT("Green"); break;
	case EChannelMaskParameterColor::Blue:
		Output = TEXT("Blue"); break;
	case EChannelMaskParameterColor::Alpha:
		Output = TEXT("Alpha"); break;
	default:
		checkf(false, TEXT("Invalid MaskChannel %d"), InMaskChannel); break;
	}
...
{
	const TCHAR* MaskName = GetChannelMaskParameterName(MaskChannel);
	OutCaptions.Add(MaskName);
	OutCaptions.Add(FString::Printf(TEXT("'%s'"), *ParameterName.ToString()));
}

void UMaterialExpressionChannelMaskParameter::GetExpressionToolTip(TArray<FString>& OutToolTip)
{
	const TCHAR* MaskName = GetChannelMaskParameterName(MaskChannel);
	OutToolTip.Add(TEXT("Outputs the selected channel."));
	OutToolTip.Add(TEXT("Note: This does NOT create new permutations."));
	OutToolTip.Add(FString::Printf(TEXT("Current output: %s "), MaskName));
}
...
bool UMaterialExpressionStaticSwitchParameter::IsResultMaterialAttributes(int32 OutputIndex)
{
	...
	const bool bAIsMaterialAttributes = (A.GetTracedInput().Expression && A.Expression->IsResultMaterialAttributes(A.OutputIndex));
	const bool bBIsMaterialAttributes = (B.GetTracedInput().Expression && B.Expression->IsResultMaterialAttributes(B.OutputIndex));

	return bAIsMaterialAttributes || bBIsMaterialAttributes;
}

EMaterialValueType GetRealInputValueType(const FExpressionInput& InExpression)
{
	const FExpressionInput& ExpressionInputRef = InExpression.GetTracedInput();
...
EMaterialValueType UMaterialExpressionStaticSwitchParameter::GetInputValueType(int32 InputIndex)
{
	if (InputIndex == 0)
	{
		if (B.IsConnected())
		{
			return GetRealInputValueType(B);
		}
	}
	else if (InputIndex == 1)
	{
		if (A.IsConnected())
		{
			return GetRealInputValueType(A);
		}
	}
...
EMaterialValueType UMaterialExpressionStaticSwitchParameter::GetOutputValueType(int32 OutputIndex)
{
	return GetCompositeOutput(OutputIndex);
}
...
FSubstrateOperator* UMaterialExpressionStaticSwitchParameter::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	if (DynamicBranch)
	{
		SubstrateTranslatorData.Errorf(TEXT("Static Switch nodes processing Substrate data do not support dynamic branching. The compiler must know the topology when translating HLSL (different branches could have different topologies)."));
		return nullptr;
	}

	FExpressionInput* EffectiveInput = nullptr;
	if (SubstrateTranslatorData.GetCompilerLegacy())
	{
		EffectiveInput = GetEffectiveInput(SubstrateTranslatorData.GetCompilerLegacy());
	}
	else
	{
		check(false);	// SUBSTRATE_TODO: Not implmenented. We need to do a selection of A or B based on static select evaluation.
		EffectiveInput = &A;
	}

	if (EffectiveInput && EffectiveInput->Expression)
	{
		return EffectiveInput->Expression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, Parent, EffectiveInput->OutputIndex);
	}
	return nullptr;
}
#endif // WITH_EDITOR
...
{
	OutCaptions.Add(FString("Static Bool ") + (Value ? TEXT("(True)") : TEXT("(False)")));
}
#endif // WITH_EDITOR
...
EMaterialValueType UMaterialExpressionStaticSwitch::GetOutputValueType(int32 OutputIndex)
{
	return GetCompositeOutput(OutputIndex);
}
...
{
	OutCaptions.Add(FString("Switch"));
}
...
FSubstrateOperator* UMaterialExpressionStaticSwitch::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FExpressionInput* EffectiveInput = nullptr;
	if (SubstrateTranslatorData.GetCompilerLegacy())
	{
...
{
	OutCaptions.Add(FString("PreviousFrameSwitch"));
}
...
{
	OutCaptions.Add(FString("Quality Switch"));
}
...
void UMaterialExpressionQualitySwitch::GatherSubstrateMaterialInfo(FSubstrateMaterialInfo& SubstrateMaterialInfo, int32 OutputIndex)
{
	...
	for (int i = 0; i < EMaterialQualityLevel::Num; ++i)
	{
		if (Inputs[i].GetTracedInput().Expression)
		{
...
FSubstrateOperator* UMaterialExpressionQualitySwitch::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FExpressionInput DefaultTraced = Default.GetTracedInput();
	if (!DefaultTraced.Expression)
	{
		SubstrateTranslatorData.Errorf(TEXT("Quality switch missing default input"));
		return nullptr;
	}
...
{
	const FStaticShaderPlatform ShaderPlatform = Compiler->GetShaderPlatform();
	ERHIFeatureLevel::Type FeatureLevelToCompile = GetFeatureLevelToCompile(ShaderPlatform, Compiler->GetFeatureLevel());
	if (GetMaterialSimulateMobileForLowQuality(ShaderPlatform) &&
		(Compiler->GetQualityLevel() == EMaterialQualityLevel::Low) &&
		(FeatureLevelToCompile > ERHIFeatureLevel::ES3_1))
	{
		FeatureLevelToCompile = ERHIFeatureLevel::ES3_1;
	}
...
{
	OutCaptions.Add(FString("Feature Level Switch"));
}
...
{
	OutCaptions.Add(FString("ShaderPlatformInfo Switch"));
}
...
FSubstrateOperator* UMaterialExpressionDataDrivenShaderPlatformInfoSwitch::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	ThrowSubstrateUnsupportedSwitchNodeError(SubstrateTranslatorData, *GetClass()->GetName());
	return nullptr;
}
#endif // WITH_EDITOR
...
bool IsBindlessEnabledForCompiler(EShaderPlatform ShaderPlatform, EShaderFrequency ShaderFrequency)
{
	const ERHIBindlessConfiguration BindlessConfiguration = FShaderPlatformConfig::GetBindlessConfiguration(ShaderPlatform);
	switch (BindlessConfiguration)
	{
		case ERHIBindlessConfiguration::All:        return true;
		case ERHIBindlessConfiguration::Minimal:    return true;
		case ERHIBindlessConfiguration::RayTracing: return IsRayTracingShaderFrequency(ShaderFrequency);
	}
	return false;
}
...
{
	return IsBindlessEnabledForCompiler(Compiler->GetShaderPlatform(), Compiler->GetCurrentShaderFrequency());
}
...
{
	OutCaptions.Add(FString("Required Samplers Switch"));
}
...
//
#if WITH_EDITOR
ERHIShadingPath::Type UMaterialExpressionShadingPathSwitch::GetShadingPathToCompile(EShaderPlatform ShaderPlatform, ERHIFeatureLevel::Type FeatureLevel, EMaterialQualityLevel::Type MaterialQualityLevel)
{
	...
	else
	{
		...
		else if (IsForwardShadingEnabled(ShaderPlatform))
		{
...
{
	ERHIShadingPath::Type ShadingPathToCompile = GetShadingPathToCompile(Compiler->GetShaderPlatform(), Compiler->GetFeatureLevel(), Compiler->GetQualityLevel());
...
{
	OutCaptions.Add(FString("Shading Path Switch"));
}
...
FSubstrateOperator* UMaterialExpressionShadingPathSwitch::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	FExpressionInput DefaultTraced = Default.GetTracedInput();
	if (!DefaultTraced.Expression)
	{
		SubstrateTranslatorData.Errorf(TEXT("Shading path switch missing default input"));
		return nullptr;
	}
...
void UMaterialExpressionParticlePositionWS::GetCaption(TArray<FString>& OutCaptions) const
{
	switch (OriginType)
	{
		...
		default:
		{
			UE_LOGF(LogMaterial, Fatal, "Unknown position origin type");
			break;
		}
	}
}
...
int32 UMaterialExpressionParticleSubUV::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	{
		if (UMaterialExpressionTextureObject* TextureObjectParameter = Cast<UMaterialExpressionTextureObject>(TextureObject.GetTracedInput().Expression))
		{
			TextureToCompile = TextureObjectParameter->Texture;
			SamplerTypeToUse = TextureObjectParameter->SamplerType;
		}
		else if (UMaterialExpressionTextureObjectParameter* TextureSampleParameter = Cast<UMaterialExpressionTextureObjectParameter>(TextureObject.GetTracedInput().Expression))
		{
			TextureToCompile = TextureSampleParameter->Texture;
			SamplerTypeToUse = TextureSampleParameter->SamplerType;
		}
	}
	...
	{
		FString SamplerTypeError;
		if (!MaterialExpressionUtils::VerifySamplerType(Compiler->GetShaderPlatform(), Compiler->GetTargetPlatform(), TextureToCompile, SamplerTypeToUse, SamplerTypeError))
		{
			return Compiler->Errorf(TEXT("%s"), *SamplerTypeError);
		}
...
void UMaterialExpressionSceneTexture::GetCaption(TArray<FString>& OutCaptions) const
{
	...
	OutCaptions.Add(FString("SceneTexture:") + Name);
}
#endif // WITH_EDITOR
...
{
	OutCaptions.Add(FString("UserSceneTexture:") + UserSceneTexture.ToString());
}
#endif // WITH_EDITOR
...
void UMaterialExpressionPinBase::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	{
		UE_LOGF(LogMaterial, Warning, "Null expressions detected in material expressions list after execution of UMaterialExpressionPinBase::PostEditChangeProperty.");
	}
}
...
TTuple<bool, UTexture*, EMaterialSamplerType, FString> UMaterialExpressionFontSample::ValidateAndGetTextureSampler(EShaderPlatform ShaderPlatform, const ITargetPlatform* TargetPlatform)
{
	...
	if (!Texture)
	{
		UE_LOGF(LogMaterial, Log, "Invalid font texture. Using default texture");
	...
	FString SamplerTypeError;
	if (!MaterialExpressionUtils::VerifySamplerType(ShaderPlatform, TargetPlatform, Texture, ExpectedSamplerType, SamplerTypeError))
	{
...
bool UMaterialExpressionFontSampleParameter::SetParameterValue(FName InParameterName, UFont* InFontValue, int32 InFontPage, EMaterialExpressionSetParameterValueFlags Flags)
{
	if (InParameterName == ParameterName)
	{
		const bool bFontValueChanged = (Font != InFontValue);
		const bool bFontPageChanged = (FontTexturePage != InFontPage);
		if (bFontValueChanged || bFontPageChanged)
		{
			Font = InFontValue;
			FontTexturePage = InFontPage;
			if (EnumHasAnyFlags(Flags, EMaterialExpressionSetParameterValueFlags::SendPostEditChangeProperty))
			{
				if (bFontValueChanged)
				{
...
void UMaterialExpressionWorldPosition::GetCaption(TArray<FString>& OutCaptions) const
{
	...
	default:
		{
			UE_LOGF(LogMaterial, Fatal, "Unknown world position shader offset type");
			break;
		}
	}
}
...
bool UMaterialExpressionCustom::HasPixelDiscard() const
{
	return ContainsClipInstruction == CMCI_Yes ||
		(ContainsClipInstruction == CMCI_Search && (Code.Contains(TEXT("clip("), ESearchCase::CaseSensitive) || Code.Contains(TEXT("discard"), ESearchCase::CaseSensitive)));
}
#endif // WITH_EDITOR
...
void UMaterialExpressionCustom::Serialize(FStructuredArchive::FRecord Record)
{
	...
	else if (UnderlyingArchive.UEVer() < VER_UE4_INSTANCED_STEREO_UNIFORM_REFACTOR)
	{
		...
		static const FString UniformMembers[] = {
			FString("FieldOfViewWideAngles"),
			FString("PrevFieldOfViewWideAngles"),
			FString("ViewRectMin"),
			FString("ViewSizeAndInvSize"),
			FString("BufferSizeAndInvSize"),
			FString("ExposureScale"),
			FString("DiffuseOverrideParameter"),
			FString("SpecularOverrideParameter"),
			FString("NormalOverrideParameter"),
			FString("RoughnessOverrideParameter"),
			FString("PrevFrameGameTime"),
			FString("PrevFrameRealTime"),
			FString("OutOfBoundsMask"),
			FString("WorldCameraMovementSinceLastFrame"),
			FString("CullingSign"),
			FString("NearPlane"),
			FString("GameTime"),
			FString("RealTime"),
			...
			FString("FrameNumber"),
			FString("CameraCut"),
			FString("UseLightmaps"),
			FString("UnlitViewmodeMask"),
			FString("DirectionalLightColor"),
			FString("DirectionalLightDirection"),
			FString("DirectionalLightShadowTransition"),
			FString("DirectionalLightShadowSize"),
			FString("DirectionalLightScreenToShadow"),
			FString("DirectionalLightShadowDistances"),
			FString("UpperSkyColor"),
			FString("LowerSkyColor"),
			FString("TranslucencyLightingVolumeMin"),
			FString("TranslucencyLightingVolumeInvSize"),
			FString("TemporalAAParams"),
			FString("CircleDOFParams"),
			FString("DepthOfFieldFocalDistance"),
			FString("DepthOfFieldScale"),
			FString("DepthOfFieldFocalLength"),
			FString("DepthOfFieldFocalRegion"),
			FString("DepthOfFieldNearTransitionRegion"),
			FString("DepthOfFieldFarTransitionRegion"),
			FString("MotionBlurNormalizedToPixel"),
			FString("GeneralPurposeTweak"),
			FString("DemosaicVposOffset"),
			FString("IndirectLightingColorScale"),
			FString("HDR32bppEncodingMode"),
			FString("AtmosphericFogSunDirection"),
			FString("AtmosphericFogSunPower"),
			FString("AtmosphericFogPower"),
			FString("AtmosphericFogDensityScale"),
			FString("AtmosphericFogDensityOffset"),
			FString("AtmosphericFogGroundOffset"),
			FString("AtmosphericFogDistanceScale"),
			FString("AtmosphericFogAltitudeScale"),
			FString("AtmosphericFogHeightScaleRayleigh"),
			FString("AtmosphericFogStartDistance"),
			FString("AtmosphericFogDistanceOffset"),
			FString("AtmosphericFogSunDiscScale"),
			FString("AtmosphericFogRenderMask"),
			FString("AtmosphericFogInscatterAltitudeSampleNum"),
			FString("AtmosphericFogSunColor"),
			FString("AmbientCubemapTint"),
			FString("AmbientCubemapIntensity"),
			FString("RenderTargetSize"),
			FString("SkyLightParameters"),
			FString("SceneFString(TEXTureMinMax"),
			FString("SkyLightColor"),
			FString("SkyIrradianceEnvironmentMap"),
			FString("MobilePreviewMode"),
			FString("HMDEyePaddingOffset"),
			FString("DirectionalLightShadowFString(TEXTure"),
			FString("SamplerState"),
		};

		const FString ViewUniformName("View.");
		const FString FrameUniformName("Frame.");
	...
	if (UnderlyingArchive.CustomVer(FNaniteResearchStreamObjectVersion::GUID) < FNaniteResearchStreamObjectVersion::LWCTypesInShaders)
	{
		...
		{
			const FString ViewSearchString = FString("View.") + Member;
			const FString ReplaceString = FString("DFDemote(ResolvedView.") + Member + FString(")");
	...
	if (bDidUpdate)
	{
		...
		Desc += PreFixUp;
		UE_LOGF(LogMaterial, Log, "Uniform references updated for custom material expression %ls.", *Description);
	}
#endif // WITH_EDITORONLY_DATA
}
...
{
	#if 0 // temporary to unblock people, tracked as UE-109810
	{
...
void UMaterialFunctionEditorOnlyData::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	{
		UE_LOGF(LogMaterial, Warning, "Material Function %ls editor only data contained null expression and some expressions may be missing. "
...
void UMaterialFunction::PreSave(FObjectPreSaveContext ObjectSaveContext)
{
	...
	{
		UE_LOGF(LogMaterial, Warning, "Material Function %ls contained some null dependent function expression calls. "
			"Please close and reopen this Material Function and verify it is still valid.",
			   *GetFullName());
	}
	#endif
...
void UMaterialFunction::PostDuplicate(bool bDuplicateForPIE)
{
	Super::PostDuplicate(bDuplicateForPIE);

	// Only clear when duplicating to a real package,
	// not when the material editor creates its transient editing copy
	if (!GetOutermost()->HasAnyFlags(RF_Transient))
	{
...
void UMaterialFunction::PostLoad()
{
	...
	if (!EditorOnly || (GetPackage()->HasAnyFlags(EObjectFlags::RF_WasLoaded) && !EditorOnly->HasAnyFlags(EObjectFlags::RF_WasLoaded)))
	{
		UE_LOGF(LogMaterial, Error, "Material function '%ls' was loaded, but it is missing editor-only data. Materials calling this function will not translate correctly.", *GetFullName());
	}
#endif // WITH_EDITORONLY_DATA
	...
	if (GIsEditor && EditorOnly)
	{
		...
		if (!bAllExpressionsLoadedCorrectly)
		{
			...
			UE_LOGF(LogMaterial, Log, "Some expression in Material Function %ls failed to load correctly. "
				...
				"Material Function's GUID was changed to %ls.", *GetFullName(), *StateId.ToString());
		}
	}
...
void UMaterialFunctionMaterialLayerBlend::PostLoad()
{
	...
	if (FortMainVer < FFortniteMainBranchObjectVersion::MaterialFunctionBlendTopBottomInputEnum)
	{
		...
		*/
		...
		if (!TopAssigned || !BottomAssigned)
		{
			UE_LOGF(LogMaterial, Warning, "Legacy Material Layer Blend Function \"%ls\" has renamed Top and Bottom input nodes. Please manually set BlendInputRelevance on these nodes to their relative type.", *GetFullName());
		}
	}
#endif
}
...
void UMaterialFunctionInstance::Serialize(FArchive& Ar)
{
	...
	if (Ar.IsLoading() && Ar.IsPersistent())
	{
		if (!StateId.IsValid())
		{
			UE_LOGF(LogMaterial, Warning, "%ls was saved without a valid StateId (old asset?). This will cause all materials using it to recompile their shaders on each load - please resave to fix.", *GetName());
			StateId = OldStateId;
		}
	}
}
...
UMaterialInterface* UMaterialFunctionInstance::GetPreviewMaterial()
{
	if (nullptr == PreviewMaterial)
	{
		PreviewMaterial = NewObject<UMaterialInstanceConstant>((UObject*)GetTransientPackage(), FName("None"), RF_Transient);
	}
...
void FMaterialLayersFunctionsID::UpdateHash(FXxHash64Builder& HashState) const
{
			...
			if (FunctionInputs.IsValidIndex(InputIndex))
			{
				...
				if (InputExpression)
				{
					...
					if (InputExpression->bUsePreviewValueAsDefault)
					{
						...
						{
							OutToolTip.Insert(FString("DefaultValue = Custom expressions"), 0);

							// Add a line after the default value string
							OutToolTip.Insert(FString(), 1);
						}
...
FSubstrateOperator* UMaterialExpressionMaterialFunctionCall::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	if (OutputIndex >= 0 && OutputIndex < FunctionOutputs.Num() && FunctionOutputs[OutputIndex].ExpressionOutput)
	{
		this->LinkFunctionIntoCaller(nullptr);
		FMaterialFunctionCompileState LocalState(this);
		SubstrateTranslatorData.PushFunction(SharedCompileState ? SharedCompileState : &LocalState);

		FSubstrateOperator* ResultingOperator = FunctionOutputs[OutputIndex].ExpressionOutput->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, Parent, 0);

		SubstrateTranslatorData.PopFunction();
		this->UnlinkFunctionFromCaller(nullptr);
...
void UMaterialExpressionFunctionInput::GetCaption(TArray<FString>& OutCaptions) const
{
	...
	check(InputType < FunctionInput_MAX);
	OutCaptions.Add(FString("Input ") + InputName.ToString() + TEXT(" (") + TypeNames[InputType] + TEXT(")"));
}
...
FSubstrateOperator* UMaterialExpressionFunctionInput::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	...
	if (IsResultSubstrateMaterial(OutputIndex))
	{
		...
		if (LocalPreviewDuringCompile && LocalPreviewDuringCompile->GetTracedInput().Expression)
		{
			...
			SlabOperator = LocalPreviewTracedInput.Expression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, Parent, LocalPreviewTracedInput.OutputIndex);
		}
		else if(Preview.Expression && bUsePreviewValueAsDefault)
		{
			SlabOperator = Preview.Expression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, Parent, Preview.OutputIndex);
		}
	...
	if(!SlabOperator)
	{
		SlabOperator = &SubstrateTranslatorData.SubstrateGetDefaultBSDFOperator(this->MaterialExpressionGuid, Parent);
	}
	return SlabOperator;
}
...
{
	OutCaptions.Add(FString("Output ") + OutputName.ToString());
}
...
{
	const EMaterialValueType RealInputValueType = GetRealInputValueType(A);

	if (RealInputValueType != MCT_Unknown)
	{
		return RealInputValueType;
	}
...
int32 UMaterialExpressionFunctionOutput::CompilePreview(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	// If this logic is changed, one need to test 
	//  - material function preview on nodes in materials, 
	//  - and in material functions with output node being SubstrateData (a slab) or a color,
	//  - and for material with Surface or UI domain preview.
	//  - This for r.substrate 1 and r.Substrate.ProjectGBufferFormat 0/1.
	if (Substrate::IsSubstrateEnabled() && AExpression && AExpression->IsResultSubstrateMaterial(ATraced.OutputIndex))
	{
		// Compile the SubstrateData output.
	...
	// If this is a material attribute node, let's have it decide of the preview 
	// (instead of using current attribute on the compiler stack which is not a good representative, default to EmissiveColor).
	if (AExpression && AExpression->IsResultMaterialAttributes(ATraced.OutputIndex))
	{
		return AExpression->CompilePreview(Compiler, ATraced.OutputIndex);
	}

	// Default preview value compilation.
...
FSubstrateOperator* UMaterialExpressionFunctionOutput::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	if (A.GetTracedInput().Expression)
	{
		return A.Expression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, Parent, A.OutputIndex);
	}
	return nullptr;
}
#endif // WITH_EDITOR
...
void UMaterialExpressionCollectionParameter::GetCaption(TArray<FString>& OutCaptions) const
{
	...
	if (Collection)
	{
		...
		OutCaptions.Add(FString("'") + ParameterName.ToString() + TEXT("'"));
	}
	else
	{
...
bool UMaterialExpressionCollectionParameter::SetParameterValue(const FName& Name, const FMaterialParameterMetadata& Meta, EMaterialExpressionSetParameterValueFlags Flags)
{
	if (Name == ParameterName && Meta.Value.Type == EMaterialParameterType::ParameterCollection)
	{
		...
		{
			if (Collection != ParameterCollection)
			{
				Collection = ParameterCollection;
				if (EnumHasAnyFlags(Flags, EMaterialExpressionSetParameterValueFlags::SendPostEditChangeProperty))
				{
					SendPostEditChangeProperty(this, GET_MEMBER_NAME_STRING_CHECKED(ThisClass, Collection));
				}
			}
			return true;
		}
...
FSubstrateOperator* UMaterialExpressionRayTracingQualitySwitch::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	ThrowSubstrateUnsupportedSwitchNodeError(SubstrateTranslatorData, *GetClass()->GetName());
	return nullptr;
}
...
FSubstrateOperator* UMaterialExpressionPathTracingQualitySwitch::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	ThrowSubstrateUnsupportedSwitchNodeError(SubstrateTranslatorData, *GetClass()->GetName());
	return nullptr;
}
...
FSubstrateOperator* UMaterialExpressionPathTracingRayTypeSwitch::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	ThrowSubstrateUnsupportedSwitchNodeError(SubstrateTranslatorData, *GetClass()->GetName());
	return nullptr;
}
...
UMaterialExpression* UMaterialExpressionRerouteBase::ResolveRerouteTarget(int32 OutputIndex, int32& OutRealOutputIndex)
{
	FExpressionInput Input;
	if (GetRerouteInput(Input))
	{
		...
		{
			// Guard against cycles that pass through non-reroute expressions (e.g. Reroute -> StaticSwitch -> Reroute).
			if (ContainsInputLoop())
			{
				return nullptr;
			}

			OutRealOutputIndex = -1;
			return TraceInputsToRealExpression(OutRealOutputIndex);
		}
	}
	return nullptr;
}
...
{
	int32 RealOutputIndex = -1;
	if (UMaterialExpression* RealExpression = ResolveRerouteTarget(OutputIndex, RealOutputIndex))
	{
		return RealExpression->IsResultMaterialAttributes(RealOutputIndex);
	}
	return false;
}
...
bool UMaterialExpressionRerouteBase::IsResultSubstrateMaterial(int32 OutputIndex)
{
	...
	if (UMaterialExpression* RealExpression = ResolveRerouteTarget(OutputIndex, RealOutputIndex))
	{
		return RealExpression->IsResultSubstrateMaterial(RealOutputIndex);
	}
	return false;
}
...
void UMaterialExpressionRerouteBase::GatherSubstrateMaterialInfo(FSubstrateMaterialInfo& SubstrateMaterialInfo, int32 OutputIndex)
{
	...
	if (UMaterialExpression* RealExpression = ResolveRerouteTarget(OutputIndex, RealOutputIndex))
	{
		RealExpression->GatherSubstrateMaterialInfo(SubstrateMaterialInfo, RealOutputIndex);
	}
}

FSubstrateOperator* UMaterialExpressionRerouteBase::SubstrateGenerateMaterialTopologyTree(struct FSubstrateTranslatorDataInterface& SubstrateTranslatorData, class UMaterialExpression* Parent, int32 OutputIndex)
{
	int32 RealOutputIndex = -1;
	if (UMaterialExpression* RealExpression = ResolveRerouteTarget(OutputIndex, RealOutputIndex))
	{
		return RealExpression->SubstrateGenerateMaterialTopologyTree(SubstrateTranslatorData, Parent, RealOutputIndex);
	}
	return nullptr;
}
...
{
	// If we are in the editor, and we don't have a valid GUID yet, generate one.
	if (GIsEditor && !IsRunningGame() && !GetPackage()->HasAnyPackageFlags(PKG_PlayInEditor) && !IsTemplate())
	{
...
int32 UMaterialExpressionMaterialCache::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	if (Compiler->GetTopCustomOutput() == this)
	{
		...
		for (int32 i = 0; i < Attributes.Num(); i++)
		{
			...
			{
				UE_LOGF(LogMaterial, Warning, "Material Cache expression with unbound inputs in '%ls', consider a minimal tag.", Material ? *(Material->GetName()) : TEXT("Unknown"));
				continue;
			}
			...
			AttributeIndex = Compiler->ForceCast(AttributeIndex, static_cast<EMaterialValueType>(Attribute.ValueType));

			Compiler->SetMaterialCacheAttribute(TagLayout, AttributeSet, GetPackedAttributeIndex(TagAttributes[i]), AttributeIndex);
		}
	...
	return Compiler->GetMaterialCacheAttribute(TagLayout, AttributeIndex, GetPackedAttributeIndex(TagAttributes[OutputIndex]));
}
...
void UMaterialExpressionMaterialCache::ConstructFromTag()
{
	...
	// Create output pins
	for (EMaterialCacheAttribute Attribute : TagAttributes)
	{
...
void UMaterialExpressionMaterialCache::ConstructLayout()
{
	...
	{
		TagAttributes = TArrayView<EMaterialCacheAttribute>(DefaultMaterialCacheAttributes);
...
uint32 UMaterialExpressionMaterialCache::GetPackedAttributeIndex(EMaterialCacheAttribute Attribute)
{
	uint32_t AttributeIndex = 0;
	
	for (const FMaterialCacheLayer& Layer : TagLayout.Layers)
	{
		for (EMaterialCacheAttribute PackedAttribute : Layer.Attributes)
		{
			if (PackedAttribute == Attribute)
			{
	...
	checkf(false, TEXT("Invalid attribute index"));
	return 0;
}
...
int32 UMaterialExpressionSamplePhysicsVectorField::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	return Compiler->ProcessPhysicsField(PositionArg, WorldPositionOriginType, EFieldOutputType::Field_Output_Vector, static_cast<uint8>(FieldTarget), true);
}
...
///////////////////////////////////////////////////////////////////////////////
// UMaterialExpressionEvalPhysicsVectorField
///////////////////////////////////////////////////////////////////////////////

int32 UMaterialExpressionEvalPhysicsVectorField::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	int32 PositionArg = INDEX_NONE;
	...
	return Compiler->ProcessPhysicsField(PositionArg, WorldPositionOriginType, EFieldOutputType::Field_Output_Vector, static_cast<uint8>(FieldTarget), false);
}

void UMaterialExpressionEvalPhysicsVectorField::GetCaption(TArray<FString>& OutCaptions) const
{
	OutCaptions.Add(TEXT("EvalPhysicsVectorField"));
}
...
int32 UMaterialExpressionSamplePhysicsScalarField::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	return Compiler->ProcessPhysicsField(PositionArg, WorldPositionOriginType, EFieldOutputType::Field_Output_Scalar, static_cast<uint8>(FieldTarget), true);
}
...
//////////////////////////////////////////////////////////////////////////////
// UMaterialExpressionEvalPhysicsScalarField
///////////////////////////////////////////////////////////////////////////////

int32 UMaterialExpressionEvalPhysicsScalarField::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	int32 PositionArg = INDEX_NONE;
	...
	return Compiler->ProcessPhysicsField(PositionArg, WorldPositionOriginType, EFieldOutputType::Field_Output_Scalar, static_cast<uint8>(FieldTarget), false);
}

void UMaterialExpressionEvalPhysicsScalarField::GetCaption(TArray<FString>& OutCaptions) const
{
	OutCaptions.Add(TEXT("EvalPhysicsScalarField"));
}
...
int32 UMaterialExpressionSamplePhysicsIntegerField::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	return Compiler->ProcessPhysicsField(PositionArg, WorldPositionOriginType, EFieldOutputType::Field_Output_Integer, static_cast<uint8>(FieldTarget), true);
}
...
///////////////////////////////////////////////////////////////////////////////
// UMaterialExpressionEvalPhysicsIntegerField
///////////////////////////////////////////////////////////////////////////////

int32 UMaterialExpressionEvalPhysicsIntegerField::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	int32 PositionArg = INDEX_NONE;
	...
	return Compiler->ProcessPhysicsField(PositionArg, WorldPositionOriginType, EFieldOutputType::Field_Output_Integer, static_cast<uint8>(FieldTarget), false);
}

void UMaterialExpressionEvalPhysicsIntegerField::GetCaption(TArray<FString>& OutCaptions) const
{
	OutCaptions.Add(TEXT("EvalPhysicsIntegerField"));
}
#endif // WITH_EDITOR
...
{
	OutCaptions.Add(FString("DepthOfFieldFunction"));
}
#endif // WITH_EDITOR
...
{
	OutCaptions.Add(FString("EyeAdaptation"));
}
#endif // WITH_EDITOR
...
{
	OutCaptions.Add(FString("EyeAdaptationInverse"));
}
#endif // WITH_EDITOR
...
{
	OutCaptions.Add(FString("ClearCoatBottomNormal"));
}
...
{
	OutCaptions.Add(FString("BentNormal"));
}
...
{
	OutCaptions.Add(FString("VertexInterpolator"));
}
...
///////////////////////////////////////////////////////////////////////////////
// UMaterialExpressionMainDirectionalLight
///////////////////////////////////////////////////////////////////////////////

UMaterialExpressionMainDirectionalLight::UMaterialExpressionMainDirectionalLight(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	struct FConstructorStatics
	{
		FText NAME_MainDirectionalLight;
		FConstructorStatics() : NAME_MainDirectionalLight(LOCTEXT("Forward Selected Directional Light", "Forward Selected Directional Light")) {}
	};
	static FConstructorStatics ConstructorStatics;
#if WITH_EDITORONLY_DATA
	MenuCategories.Add(ConstructorStatics.NAME_MainDirectionalLight);
#endif
...
#if WITH_EDITORONLY_DATA
	Outputs.Reset();
	Outputs.Add(FExpressionOutput(TEXT("Illuminance")));
	Outputs.Add(FExpressionOutput(TEXT("Direction")));
#endif
}

#if WITH_EDITOR

int32 UMaterialExpressionMainDirectionalLight::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	if (OutputIndex == 0)
	{
		return Compiler->MainDirectionalLightIlluminance();
	}
	else if (OutputIndex == 1)
	{
		return Compiler->MainDirectionalLightDirection();
	}
	check(false);
	return INDEX_NONE;
}

void UMaterialExpressionMainDirectionalLight::GetCaption(TArray<FString>& OutCaptions) const
{
	OutCaptions.Add(TEXT("Forward Selected Directional Light"));
}

EMaterialValueType UMaterialExpressionMainDirectionalLight::GetOutputValueType(int32 OutputIndex)
{
	return MCT_Float3; // Direction and illuminance are float3
}

void UMaterialExpressionMainDirectionalLight::GetExpressionToolTip(TArray<FString>& OutToolTip)
{
	OutToolTip.Add(FText(LOCTEXT("MainDirectionalLightTT", "This node returns attributes from the selected main directional light used for the forward shading and lighting of translucent, fog or single layer water. It accounts for ForwardShadingPriority then intensity.")).ToString());
}
...
bool UMaterialExpressionCurveAtlasRowParameter::SetParameterValue(const FName& Name, const FMaterialParameterMetadata& Meta, EMaterialExpressionSetParameterValueFlags Flags)
{
	...
	{
		const bool bHasCurveChanged = (Curve != Meta.ScalarCurve.Get());
		const bool bHasAtlasChanged = (Atlas != Meta.ScalarAtlas.Get());
		const bool bHasDefaultValueChanged = (DefaultValue != Meta.Value.AsScalar());
		if (bHasCurveChanged || bHasAtlasChanged || bHasDefaultValueChanged)
		{
...
UMaterialExpressionStylizedCharacterLightingOutput::UMaterialExpressionStylizedCharacterLightingOutput(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
#if WITH_EDITOR
	Outputs.Reset();
...
int32 UMaterialExpressionStylizedCharacterLightingOutput::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	int32 CodeInput = INDEX_NONE;
	...
	{
	case 0:
		CodeInput = DiffuseBias.IsConnected() ? DiffuseBias.Compile(Compiler) : Compiler->Constant(0.5f);
		break;
	case 1:
		CodeInput = LightingProfile.IsConnected() ? LightingProfile.Compile(Compiler) : Compiler->Constant(0.0f);
		break;
	default:
		return INDEX_NONE;
	}
...
void UMaterialExpressionStylizedCharacterLightingOutput::GetCaption(TArray<FString>& OutCaptions) const
{
	OutCaptions.Add(TEXT("Stylized Character Lighting"));
}

#endif // WITH_EDITOR

int32 UMaterialExpressionStylizedCharacterLightingOutput::GetNumOutputs() const
{
	return 2;
}

FString UMaterialExpressionStylizedCharacterLightingOutput::GetFunctionName() const
{
	return TEXT("GetStylizedCharacterLightingOutput");
}

FString UMaterialExpressionStylizedCharacterLightingOutput::GetDisplayName() const
{
	return TEXT("Stylized Character Lighting Output");
}
...
{
	OutCaptions.Add(FString("Single Layer Water Material"));
}
...
{
	OutCaptions.Add(FString("Volumetric Advanced Output"));
}
...
{
	OutCaptions.Add(FString("Volumetric Cloud Empty Space Skipping Output <Experimental>"));
}
...
{
	OutCaptions.Add(FString("Thin Translucent Material"));
}
...
{
	OutCaptions.Add(FString("Absorption Medium (Path Tracer Only)"));
}
...
int32 UMaterialExpressionIfThenElse::Compile(class FMaterialCompiler* Compiler, int32 OutputIndex)
{
	...
	{
		return Compiler->Errorf(TEXT("Missing IfThenElse Condition input"));
	}
	if (!True.GetTracedInput().Expression)
	{
		return Compiler->Errorf(TEXT("Missing IfThenElse True input"));
	}
	if (!False.GetTracedInput().Expression)
	{
		return Compiler->Errorf(TEXT("Missing IfThenElse False input"));
	}

	const int32 ConditionCode = Condition.Compile(Compiler);
	const int32 TrueCode = True.Compile(Compiler);
	const int32 FalseCode = False.Compile(Compiler);

	if (ConditionCode == INDEX_NONE || TrueCode == INDEX_NONE || FalseCode == INDEX_NONE)
	{
		return INDEX_NONE;
	}

	return Compiler->DynamicBranch(ConditionCode, TrueCode, FalseCode);
}
...
EMaterialValueType UMaterialExpressionIfThenElse::GetOutputValueType(int32 OutputIndex)
{
	return MCT_Unknown;
}
...
{
	OutCaptions.Add(FString("Sparse Volume Texture Sample"));
}
...
bool UMaterialExpressionSparseVolumeTextureSampleParameter::SetParameterValue(FName InParameterName, USparseVolumeTexture* InValue, EMaterialExpressionSetParameterValueFlags Flags)
{
	if (InParameterName == ParameterName)
	{
		if (SparseVolumeTexture != InValue)
		{
...
{
	OutCaptions.Add(FString("Sparse Volume Texture Sample Param "));
	OutCaptions.Add(FString::Printf(TEXT("'%s'"), *ParameterName.ToString()));
}
...
{
	OutCaptions.Add(FString("Subsurface Medium (Path Tracer Only)"));
}
...
{
	OutCaptions.Add(FString("First Person Output"));
}
...
const UMaterialAggregate* UMaterialExpressionAggregate::GetAggregate() const
{
	switch (Kind)
	{
		case EMaterialExpressionMakeAggregateKind::MaterialAttributes: return MaterialAttributesAggregate::Get();
...
static bool IsAggregateAttributeEnabled(const UMaterial* Material, const UMaterialAggregate* Aggregate, int AttributeIndex, const TArray<FName>& UsedAttributeNames)
{
	if (Aggregate == MaterialAttributesAggregate::Get())
	{
		EMaterialProperty Property = MaterialAttributesAggregate::AttributeIndexToMaterialProperty(AttributeIndex);
		if (Property <= MP_CustomizedUVs7 && Property >= MP_CustomizedUVs0 + Material->NumCustomizedUVs)
		{
			return false;
		}
	}
	if (UsedAttributeNames.Contains(Aggregate->Attributes[AttributeIndex].Name))
	{
		return false;
	}
	return true;
...
TArray<FString> UMaterialExpressionAggregate::GetPossibleAttributeNames() const
{
	...
	if (const UMaterialAggregate* Aggregate = GetAggregate())
	{
		...
		{
			if (IsAggregateAttributeEnabled(Material, Aggregate, i, AttributeNames))
			{
...
static EMaterialValueType GetPinValueType(const UMaterialExpressionAggregate* Expr, int32 PinIndex)
{
	...
	if (PinIndex == 0)
	{
		return Expr->GetAggregate() == MaterialAttributesAggregate::Get() ? MCT_MaterialAttributes : MCT_Unknown;
	}
...
static FName PickAvailableAttribute(const UMaterial* Material, const UMaterialAggregate* Aggregate, const TArray<FName>& AttributeNames)
{
	if (Aggregate)
	{
...
void UMaterialExpressionAggregate::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
    ...
    if (AttributeNames.Num() > PreEditData.AttributeNames.Num())
    {
        for (int32 i = 0; i < AttributeNames.Num(); )
        {
            ...
            {
                AttributeNames[i] = PickAvailableAttribute(Material, Aggregate, AttributeNames);
                if (AttributeNames[i] == NAME_None)
                {
...
{
	OutCaptions.Add(FString("Temporal Responsiveness"));
}
...
{
	OutCaptions.Add(FString("Motion Vector World Offset (Per-Pixel)"));
}
