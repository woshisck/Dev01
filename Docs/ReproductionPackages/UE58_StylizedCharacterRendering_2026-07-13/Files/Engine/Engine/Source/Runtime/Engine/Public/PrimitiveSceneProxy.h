...
#include "RaytracingVFAttributeData.h"
#include "PSOPrecacheFwd.h"
#include "RayTracingGeometryManagerTypes.h"
#include "Runtime/Engine/Internal/MaterialCache/MaterialCacheDescriptor.h"
...
union FSimpleLightId
{
	...
	FSimpleLightId()
		: LocalId(INDEX_NONE)
		, SourcePrimitiveId(INDEX_NONE)
	{}

	FSimpleLightId(int32 InLocalId, int32 InSourcePrimitiveId)
		: LocalId(InLocalId)
		, SourcePrimitiveId(InSourcePrimitiveId)
	{}
	...
	bool IsValid() const
	{
		return LocalId != INDEX_NONE && SourcePrimitiveId != INDEX_NONE;
	}
};

inline uint32 GetTypeHash(FSimpleLightId In)
{
	return GetTypeHash(In.Packed);
}
...
enum class ERayTracingProxyType : uint8
{
	...
	// Don't use as ERayTracingProxyType is bitpacked in 7 bits in FPrimitiveRayTracingData
	//DO_NOT_USE						= 1 << 7,
...
public:
	...
	ENGINE_API FPrimitiveSceneProxy();
	ENGINE_API FPrimitiveSceneProxy(FPrimitiveSceneProxy const&);
	ENGINE_API FPrimitiveSceneProxy(FPrimitiveSceneProxy&&);
	ENGINE_API FPrimitiveSceneProxy& operator=(const FPrimitiveSceneProxy&);
	ENGINE_API FPrimitiveSceneProxy& operator=(FPrimitiveSceneProxy&&);
	/** Virtual destructor. */
	 ...
	 */
	ENGINE_API void SetSelectionOverride_GameThread(bool bForceSelection);
	
	/**
	 * Enqueue updated selection outline color for the render thread to use.
	 */
	ENGINE_API void SetSelectionOutlineColorIndex_GameThread(uint8 ColorIndex);
	...
	*/
	...
	ENGINE_API virtual ERayTracingPrimitiveFlags GetRayTracingPrimitiveFlags();
	ENGINE_API virtual void GetStaticRayTracingInstances(struct FStaticRayTracingInstances& OutRayTracingInstances);
	 ...
	 */
	UE_DEPRECATED(5.8, "Use GetRayTracingPrimitiveFlags() and GetStaticRayTracingInstances() instead.")
	ENGINE_API virtual ERayTracingPrimitiveFlags GetCachedRayTracingInstance(struct FRayTracingInstance& OutRayTracingInstance);

	/**
	 * If the ray tracing data is streaming then get the coarse mesh streaming handle 
	 */
	UE_DEPRECATED(5.8, "Coarse Mesh Streaming is deprecated. Use reference based residency instead.")
	...
	virtual RayTracing::FGeometryGroupHandle GetRayTracingGeometryGroupHandle() const { return {}; }

	/** @return The handle of the ray tracing geometry group used by this primitive */
	ENGINE_API ERayTracingProxyType GetRayTracingProxyType() const;

	virtual FRaytracingVFAttributeData GetRaytracingVFAttributeData(const FSceneView* View) { return {}; }
	...
	virtual bool CanApplyStreamableRenderAssetScaleFactor() const { return false; }
	...
	ENGINE_API virtual FMatrix GetMeshCardsToWorld(int32 InstanceIndex) const;
	...
	*/
	...
	/**
	 */
	UE_DEPRECATED(5.8, "No longer called.")
	virtual void OnDetachLight(const FLightSceneInfo* Light) final
	{
	}
	...
	*/
	UE_DEPRECATED(5.8, "Will be removed as it is not generally thread safe.")
...
PRAGMA_ENABLE_DEPRECATION_WARNINGS
	...
	*/
	...
	inline bool IsVisibleInReflections() const { return bVisibleInReflections; }
	...
	inline bool CastsShadowOnOtherObjectsOnly() const { return bCastShadowOnOtherObjectsOnly; }
	...
	inline bool IsOwnerNoSee() const { return bOwnerNoSee; }
	inline bool IsOnlyOwnerSee() const { return bOnlyOwnerSee; }
	...
	inline bool IsLumenHeightfield() const { return bLumenHeightfield; }
	inline bool IsLandscapeProxy() const { return bIsLandscapeProxy; }
	...
	inline bool AnyMaterialUsesTemporalResponsiveness() const { return bAnyMaterialUsesTemporalResponsiveness; }
	inline bool AnyMaterialUsesMotionVectorWorldOffset() const { return bAnyMaterialUsesMotionVectorWorldOffset; }
	...
	inline bool DrawsVelocity() const { return HasDynamicTransform() || bAlwaysHasVelocity || AnyMaterialHasPixelAnimation() || bHasWorldPositionOffsetVelocity || AnyMaterialUsesTemporalResponsiveness() || AnyMaterialUsesMotionVectorWorldOffset(); }
	...
	inline bool AlwaysHasVelocity() const {	return bAlwaysHasVelocity || AnyMaterialHasPixelAnimation() || (bHasWorldPositionOffsetVelocity && EvaluateWorldPositionOffset()) || AnyMaterialUsesTemporalResponsiveness() || AnyMaterialUsesMotionVectorWorldOffset(); }
	...
	*/
	...
	/**
	 * Note: should not modify the OutInstancingFadeOutParams output unless it actually has a valid output - the default is initialized.
	 */
	virtual bool GetInstancingFadeOutParams(FVector2f& OutInstancingFadeOutParams) const
	{
		return false;
	}
	...
	*/
	...
	inline bool IsDrawnInGame() const { return DrawInGame; }

	UE_DEPRECATED(5.8, "Use SetIsDrawn_RenderThread instead.")
	inline void SetDrawnInGame_RenderThread(bool bInDrawInGame) {}

	/**
	 * Set the DrawInGame and the DrawInEditor flags, from the render thread.
	 * @param bIsDrawn		The new value.
 	 */
	ENGINE_API void SetIsDrawn_RenderThread(bool bIsDrawn);

	/** Tell us if this proxy is drawn in editor.*/
	inline bool IsDrawnInEditor() const { return DrawInEditor; }

	virtual uint8 GetCurrentFirstLODIdx_RenderThread() const { return 0; }

#if !UE_BUILD_SHIPPING
	/** Returns max(0, WantedLODs - ResidentLODs) for streaming deficit debug visualization. */
	virtual uint8 GetStreamingLODWantedDeficit() const { return 0; }
#endif
	...
	ENGINE_API virtual uint32 GetMaterialCacheDescriptor() const;
...
private:
	/**
	 * Internally used function that sets up the parameters that are not handled by the Scene BuildPrimitiveUniformShaderParameters_RenderThread implementations.
     * In general, this should not be expanded on, as the data representation should be kept internal in the renderer.
	 */
	ENGINE_API void BuildUniformShaderParametersInternal(FPrimitiveUniformShaderParametersBuilder& Builder) const;

public:
	/** Thread-safe accessor for this proxy's SimpleStreamableAssetManager (SSAM) handle. Returns
	 *  the default-invalid handle if the proxy was never registered (SSAM disabled, or
	 *  ClearPrimitiveHandle already ran). */
	FSSAMPrimitiveHandle GetSSAMPrimitiveHandle() const { return SSAMPrimitiveHandle; }

private:
	friend class FSimpleStreamableAssetManager;
	/** SSAM handle for this proxy. Paired with UPrimitiveComponent::SSAMPrimitiveHandle when
	 *  a matching UPrimitiveComponent exists. Mutated post-publication by Assign/Clear paths
	 *  (GT/PGT when a UPrimitiveComponent is bound, or true worker threads for proxy-only
	 *  callers like FastGeo's async render state creation) and by the RT FRemovePrimitiveCommand
	 *  teardown; concurrent readers on RT/PGT call GetSSAMPrimitiveHandle(). */
	...
	/** If true, this component will be visible in ray traced reflections. */
	...
	friend class FMaterialCacheSceneExtensionUpdater;
...
protected:
	...
	/** When enabled, the component casts its projected shadow on other primitives but not on itself. Forces bCastInsetShadow. */
	uint8 bCastShadowOnOtherObjectsOnly : 1;

	/** Whether this component should create a per-object shadow that gives higher effective shadow resolution. true if either shadow-only mode is enabled. */
...
public:
	...
	FMaterialCacheDescriptor MaterialCacheDescriptor = InvalidMaterialCacheDescriptor;
