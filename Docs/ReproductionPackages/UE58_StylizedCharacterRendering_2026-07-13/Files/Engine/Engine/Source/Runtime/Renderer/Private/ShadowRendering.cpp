	...
	const FInstanceCullingDrawParams& InstanceCullingDrawParams) const
{
	...
	else
	{
		...
		// Mask subject pixels out for mobile modulated shadows or environment-only caster projection.
		if (bCastShadowOnOtherObjectsOnly || (bMobileModulatedProjections && !CVarEnableModulatedSelfShadow.GetValueOnRenderThread()))
		{
