...
static TAutoConsoleVariable<float> CVarStylizedCharacterSelfShadowHalfViewBlend(
	TEXT("r.StylizedCharacter.SelfShadow.HalfViewBlend"),
	0.5f,
	TEXT("Blends the character self-shadow trace from light direction toward view direction."),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarStylizedCharacterSelfShadowStrength(
	TEXT("r.StylizedCharacter.SelfShadow.Strength"),
	1.0f,
	TEXT("Strength of the character half-view screen-space self shadow."),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<float> CVarStylizedCharacterSelfShadowMaxTraceDistance(
	TEXT("r.StylizedCharacter.SelfShadow.MaxTraceDistance"),
	200.0f,
	TEXT("Maximum world-space trace distance for stylized character self shadow."),
	ECVF_RenderThreadSafe);
...
class FFirstPersonSelfShadowInputsDownsamplePS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, bStylizedCharacterSelfShadow)
...
class FFirstPersonSelfShadowTracingPS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(uint32, bStylizedCharacterSelfShadow)
		SHADER_PARAMETER(float, StylizedCharacterHalfViewBlend)
		SHADER_PARAMETER(float, StylizedCharacterSelfShadowStrength)
		RENDER_TARGET_BINDING_SLOTS()
	END_SHADER_PARAMETER_STRUCT()
...
{
	const bool bFirstPersonEnabled = CVarFirstPersonSelfShadow.GetValueOnRenderThread() != 0
		&& HasFirstPersonGBufferBit(ViewFamily.GetShaderPlatform());
	const bool bStylizedCharacterEnabled = CVarStylizedCharacterSelfShadow.GetValueOnRenderThread() != 0
		&& IsFeatureLevelSupported(ViewFamily.GetShaderPlatform(), ERHIFeatureLevel::SM5);
	return ViewFamily.EngineShowFlags.DirectLighting && (bFirstPersonEnabled || bStylizedCharacterEnabled);
}
...
bool LightCastsFirstPersonSelfShadow(const FLightSceneInfo& LightSceneInfo)
{
	...
	const bool bFirstPersonLight = bSelfShadowEnabled && ((bIsDirectionalLight && bDirectionalLightSelfShadowEnabled) || (!bIsDirectionalLight && bLocalLightSelfShadowEnabled));
	const bool bStylizedCharacterLight = CVarStylizedCharacterSelfShadow.GetValueOnRenderThread() != 0
		&& (uint8)LightSceneInfo.Proxy->GetStylizedCharacterLightMode() == 1u;
	return bCastsShadow && (bFirstPersonLight || bStylizedCharacterLight);
}
...
bool ShouldRenderFirstPersonSelfShadowForLight(const FSceneRendererBase& SceneRenderer, const FSceneViewFamily& ViewFamily, const TArray<FViewInfo>& Views, const FLightSceneInfo& LightSceneInfo)
{
	...
	{
		if (CVarStylizedCharacterSelfShadow.GetValueOnRenderThread() != 0
			&& (uint8)LightSceneInfo.Proxy->GetStylizedCharacterLightMode() == 1u)
		{
...
FFirstPersonSelfShadowInputs CreateFirstPersonSelfShadowInputs(FRDGBuilder& GraphBuilder, const TArray<FViewInfo>& Views, const FMinimalSceneTextures& SceneTextures)
{
	...
	for (int32 ViewIndex = 0, ViewCount = Views.Num(); ViewIndex < ViewCount; ++ViewIndex)
	{
		...
		{
		case 0: CheckerboardMode = 1; break;
		case 1: CheckerboardMode = 0; break;
		...
		PassParameters->bStylizedCharacterSelfShadow = CVarStylizedCharacterSelfShadow.GetValueOnRenderThread() != 0 ? 1u : 0u;
...
void RenderFirstPersonSelfShadow(FRDGBuilder& GraphBuilder, const FSceneRendererBase& SceneRenderer, const TArray<FViewInfo>& Views, FRDGTextureRef ScreenShadowMaskTexture, const FFirstPersonSelfShadowInputs& Inputs, const FLightSceneInfo& LightSceneInfo)
{
	...
	const bool bStylizedCharacterLight = CVarStylizedCharacterSelfShadow.GetValueOnRenderThread() != 0
		&& (uint8)LightSceneInfo.Proxy->GetStylizedCharacterLightMode() == 1u;
	const FFirstPersonSceneExtensionRenderer* FPRenderer = SceneRenderer.GetSceneExtensionsRenderers().GetRendererPtr<FFirstPersonSceneExtensionRenderer>();
	if (!bStylizedCharacterLight && !ensure(FPRenderer))
	{
		return;
	}
	...
	for (int32 ViewIndex = 0, ViewCount = Views.Num(); ViewIndex < ViewCount; ++ViewIndex)
	{
		...
		const bool bViewRelevant = bStylizedCharacterLight
			? LightSceneInfo.ShouldRenderLight(View)
			: IsViewFirstPersonSelfShadowRelevant(View, FPRenderer->GetFirstPersonViewBounds(View), LightSceneInfo);
		if (!bViewRelevant)
		{
			continue;
		}
		...
		*DeferredLightStruct = GetDeferredLightParameters(View, LightSceneInfo);
		...
		// Trace screen space rays
		{
			...
			PassParameters->HZBMaxTraceDistance = FMath::Max(0.0f, FMath::Max(CVarFirstPersonSelfShadowMaxTraceDistance.GetValueOnRenderThread(), CVarStylizedCharacterSelfShadowMaxTraceDistance.GetValueOnRenderThread()));
			...
			PassParameters->bStylizedCharacterSelfShadow = CVarStylizedCharacterSelfShadow.GetValueOnRenderThread() != 0 ? 1u : 0u;
			PassParameters->StylizedCharacterHalfViewBlend = FMath::Clamp(CVarStylizedCharacterSelfShadowHalfViewBlend.GetValueOnRenderThread(), 0.0f, 1.0f);
			PassParameters->StylizedCharacterSelfShadowStrength = FMath::Clamp(CVarStylizedCharacterSelfShadowStrength.GetValueOnRenderThread(), 0.0f, 1.0f);
			if (!bRawFullResolution)
			{
