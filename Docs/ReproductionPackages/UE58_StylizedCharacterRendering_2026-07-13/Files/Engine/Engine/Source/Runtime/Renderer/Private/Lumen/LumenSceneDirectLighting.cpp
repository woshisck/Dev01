...
static TAutoConsoleVariable<bool> CVarLumenDirectLightingShowLightDrawEvents(
	TEXT("r.LumenScene.DirectLighting.ShowLightDrawEvents"),
	false,
	TEXT("Whether to show standalone light names. If all light names are needed, batching needs to be disabled."),
	ECVF_RenderThreadSafe);

static TAutoConsoleVariable<bool> CVarLumenDirectLightingBuildLightTilesOneGroupPerTile(
	TEXT("r.LumenScene.DirectLighting.BuildLightTiles.OneGroupPerTile"),
	true,
	TEXT("Whether to cull lights to tiles by dispatching one group per tile or one thread per tile."),
	ECVF_RenderThreadSafe);
...
bool LumenSceneDirectLighting::ShowStandaloneLightNames()
{
	return CVarLumenDirectLightingShowLightDrawEvents.GetValueOnRenderThread();
}
...
class FBuildLightTilesCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_SCALAR_ARRAY(float, ViewExposure, [LUMEN_MAX_VIEWS])
		SHADER_PARAMETER(int32, bUseLightTilesPerLightType)
	END_SHADER_PARAMETER_STRUCT()

	class FMaxLightSamples : SHADER_PERMUTATION_SPARSE_INT("MAX_LIGHT_SAMPLES", 1, 2, 4, 8, 16, 32);
	class FOneGroupPerTile : SHADER_PERMUTATION_BOOL("ONE_GROUP_PER_TILE");
	class FWaveOpWaveSize : SHADER_PERMUTATION_SPARSE_INT("WAVE_OP_WAVE_SIZE", 0, 32, 64);
	using FPermutationDomain = TShaderPermutationDomain<FMaxLightSamples, FOneGroupPerTile, FWaveOpWaveSize>;

	static FPermutationDomain RemapPermutation(FPermutationDomain PermutationVector)
	{
		if (!PermutationVector.Get<FOneGroupPerTile>())
		{
			PermutationVector.Set<FWaveOpWaveSize>(0);
		}
	...
	static void ModifyCompilationEnvironment(const FGlobalShaderPermutationParameters& Parameters, FShaderCompilerEnvironment& OutEnvironment)
	{
		...
		OutEnvironment.SetDefine(TEXT("THREADGROUP_SIZE"), GetGroupSize(PermutationVector));
		
	}

	static int32 GetGroupSize(FPermutationDomain PermutationVector)
	{
		return PermutationVector.Get<FWaveOpWaveSize>() == 32 ? 32 : 64;
	}
};
...
class FLumenCardBatchDirectLightingCS : public FGlobalShader
{
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(FVector4f, StylizedLumenLightingParameters0)
		SHADER_PARAMETER(FVector4f, StylizedLumenLightingParameters1)
	END_SHADER_PARAMETER_STRUCT()

	class FMultiView : SHADER_PERMUTATION_BOOL("HAS_MULTIPLE_VIEWS");
	class FHasRectLights : SHADER_PERMUTATION_BOOL("HAS_RECT_LIGHTS");
	class FWaveOpWaveSize : SHADER_PERMUTATION_SPARSE_INT("WAVE_OP_WAVE_SIZE", 0, 64);
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	PassParameters->StylizedLumenLightingParameters0 = GetStylizedLumenLightingParams0();
	PassParameters->StylizedLumenLightingParameters1 = GetStylizedLumenLightingParams1();
	...
	for (int32 OriginIndex = 0; OriginIndex < NumViewOrigins; ++OriginIndex)
	{
		...
		GET_SCALAR_ARRAY_ELEMENT(PassParameters->ViewExposure, OriginIndex) = ViewOrigin.LastEyeAdaptationExposure;
	}
	...
	if (GRHISupportsWaveOperations && RHISupportsWaveOperations(Views[0].GetShaderPlatform()))
	{
		...
		{
			// Wave32 will just use the groupshared path
		}
	}
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	auto SetCommonParameters = [&](FLumenDirectLightingNonRayTracedShadowsParameters& CommonParameters, bool bStandaloneLight)
	{
		...
		CommonParameters.ForwardDirLightShadowStruct = View.ForwardLightingResources.ForwardDirLightShadowUniformBuffer;
	...
	for (const int32 StandaloneLightIndex : StandaloneLightIndices)
	{
		...
		FString LightName;
		if (LumenSceneDirectLighting::ShowStandaloneLightNames() && Light.LightSceneInfo)
		{
			FSceneRenderer::GetLightNameForDrawEvent(Light.LightSceneInfo->Proxy, LightName);
		}
		...
		if (LightFunctionMaterialProxy)
		{
			...
			GraphBuilder.AddPass(
				RDG_EVENT_NAME("ShadowMaskFromLightAttenuationPass(LF,%s)", *LightName),
				PassParameters,
				ComputePassFlags,
		...
		else
		{
			...
			FComputeShaderUtils::AddPass(
				GraphBuilder,
				RDG_EVENT_NAME("ShadowMaskFromLightAttenuationPass(%s)", *LightName),
				ComputePassFlags,
				ComputeShader,
				PassParameters,
				...
				GraphBuilder.AddPass(
					...
					[PassParameters, ComputeShader, IndirectArgsBuffer, NumViews, ViewIndex, BatchedLightParameters](FRDGAsyncTask, FRHIComputeCommandList& RHICmdList)
					{
						...
						for (const FPerLightParameters& LightParameterValues : BatchedLightParameters)
						{
							...
							// TODO: Only set changed parameters
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	for (const int32 StandaloneLightIndex : StandaloneLightIndices)
	{
		...
		const bool bTraceMeshObjects = Light.bHasShadows
			...
			&& ShouldCompileDistanceFieldShaders(View.GetShaderPlatform())
		...
		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("DistanceFieldShadowPass %s", *LightName),
			ComputePassFlags,
			ComputeShader,
			PassParameters,
			LightTileScatterParameters.DispatchIndirectArgs,
	...
	ERDGPassFlags ComputePassFlags)
{
	...
	// Build a list of light tiles for future processing
	{
		...
		const bool bOneGroupPerTile = CVarLumenDirectLightingBuildLightTilesOneGroupPerTile.GetValueOnRenderThread();
		int32 WaveOpWaveSize = 0;

		if (Lumen::UseWaveOps(Views[0].GetShaderPlatform()))
		{
			// Wave32 is preferred
			if (GRHIMinimumWaveSize <= 32 && GRHIMaximumWaveSize >= 32)
			{
				WaveOpWaveSize = 32;
			}
			else if (GRHIMinimumWaveSize <= 64 && GRHIMaximumWaveSize >= 64)
			{
				WaveOpWaveSize = 64;
			}
		}
		...
		PermutationVector.Set<FBuildLightTilesCS::FOneGroupPerTile>(bOneGroupPerTile);
		PermutationVector.Set<FBuildLightTilesCS::FWaveOpWaveSize>(WaveOpWaveSize);
		PermutationVector = FBuildLightTilesCS::RemapPermutation(PermutationVector);

		auto ComputeShader = GlobalShaderMap->GetShader<FBuildLightTilesCS>(PermutationVector);

		FComputeShaderUtils::AddPass(
			GraphBuilder,
			RDG_EVENT_NAME("BuildLightTiles MaxLightsPerTile=%d GroupPerTile=%d", MaxLightsPerTile, bOneGroupPerTile ? 1 : 0),
			ComputePassFlags,
			ComputeShader,
			PassParameters,
			DispatchCardTilesIndirectArgs,
			bOneGroupPerTile ? (uint32)ELumenDispatchCardTilesIndirectArgsOffset::OneGroupPerCardTile : (uint32)ELumenDispatchCardTilesIndirectArgsOffset::OneThreadPerCardTile);
	}
