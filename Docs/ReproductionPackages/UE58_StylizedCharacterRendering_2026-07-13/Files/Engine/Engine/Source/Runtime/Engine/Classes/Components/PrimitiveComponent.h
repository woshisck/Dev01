...
#include "Streaming/RenderAssetOwnerStreamingState.h"
...
/**
 * Selects which families of touch component events UPrimitiveComponent::DispatchTouchHitResult should fire.
 * BeginEnd mirrors APlayerController::bEnableTouchEvents; Over mirrors bEnableTouchOverEvents.
 */
enum class ETouchEventFamilies : uint8
{
	None = 0,
	BeginEnd = 1 << 0,
	Over = 1 << 1,
...
ENUM_CLASS_FLAGS(ETouchEventFamilies);

namespace PrimitiveComponentCVars
{
	extern float HitDistanceToleranceCVar;
	extern float InitialOverlapToleranceCVar;
	ENGINE_API extern int32 bAllowCachedOverlapsCVar;
	ENGINE_API extern int32 bEnableFastOverlapCheck;
}
...
{
	static inline EStencilMask ToStencilMask(const ERendererStencilMask InEnum)
	{
		switch (InEnum)
		{
...
struct FPredicateOverlapHasSameActor
{
	...
	{
		// since we only care whether the overlaping actor is the same as MyOwnerPtr
		// we only call GetCachedActor (which resolves the stored handle and returns the actor)
		// rather than calling FindActor which could cause actor spawning if the entity hit
		// is an instanced actor. We obviously don't need to spawn an actor to determine 
		// that it won't be the same as MyOwnerPtr (which has already been spawned).
		const AActor* HitActor = Info.OverlapInfo.HitObjectHandle.GetCachedActor();
		// MyOwnerPtr is always valid, so we don't need the IsValid() checks in the WeakObjectPtr comparison operator.
		return HitActor && MyOwnerPtr.HasSameIndexAndSerialNumber(HitActor);
	}
...
struct FPredicateOverlapHasDifferentActor
{
	...
	bool operator() (const FOverlapInfo& Info)
	{
		// since we only care whether the overlaping actor is not the same as MyOwnerPtr
		// we only call GetCachedActor (which resolves the stored handle and returns the actor)
		// rather than calling FindActor which could cause actor spawning if the entity hit
		// is an instanced actor. We obviously don't need to spawn an actor to determine 
		// that it won't be the same as MyOwnerPtr (which has already been spawned).
...
DECLARE_DYNAMIC_MULTICAST_SPARSE_DELEGATE_FourParams( FComponentOnTouchBeganSignature, UPrimitiveComponent, OnTouchBegan, const FTouchId, TouchId, APlayerController*, Controller, const FHitResult&, HitResult, UPrimitiveComponent*, TouchedComponent );
DECLARE_DYNAMIC_MULTICAST_SPARSE_DELEGATE_FourParams( FComponentOnTouchEndedSignature, UPrimitiveComponent, OnTouchEnded, const FTouchId, TouchId, APlayerController*, Controller, const FHitResult&, HitResult, UPrimitiveComponent*, TouchedComponent );
DECLARE_DYNAMIC_MULTICAST_SPARSE_DELEGATE_FourParams( FComponentOnTouchEnteredSignature, UPrimitiveComponent, OnTouchEntered, const FTouchId, TouchId, APlayerController*, Controller, const FHitResult&, HitResult, UPrimitiveComponent*, TouchedComponent );
DECLARE_DYNAMIC_MULTICAST_SPARSE_DELEGATE_FourParams( FComponentOnTouchExitedSignature, UPrimitiveComponent, OnTouchExited, const FTouchId, TouchId, APlayerController*, Controller, const FHitResult&, HitResult, UPrimitiveComponent*, TouchedComponent );
...
class UPrimitiveComponent : public USceneComponent, public INavRelevantInterface, public IInterface_AsyncCompilation, public IPhysicsComponent, public FRenderAssetOwnerStreamingState, public IPhysicsBodyInstanceOwner, public IPhysicsBodyInstanceOwnerResolver
{
	GENERATED_BODY()
...
public:
	...
	FRenderAssetOwnerStreamingState& GetStreamingState() const
	{
		return static_cast<FRenderAssetOwnerStreamingState&>(const_cast<UPrimitiveComponent&>(*this));
	}
...
public:
	...
	bool IsMipStreamingForced() const
	{
		return bForceMipStreaming;
	}
	 ...
	 */
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadOnly, Category=Lighting, meta=(EditCondition="CastShadow && !bCastShadowOnOtherObjectsOnly"))
	uint8 bSelfShadowOnly:1;

	/**
	 * When enabled, the component casts its dynamic shadow onto other primitives but not onto its own visible pixels.
	 * This forces bCastInsetShadow and is intended for projected shadow maps where self-shadowing is supplied by a separate stylized pass.
	 */
	UPROPERTY(EditAnywhere, AdvancedDisplay, BlueprintReadOnly, Category=Lighting, meta=(EditCondition="CastShadow && !bSelfShadowOnly", DisplayName = "Shadow Other Objects Only"))
	uint8 bCastShadowOnOtherObjectsOnly:1;
	...
	*/
	...
	/** If true, this component will be included in the ray tracing far field layer instead of the near field. The ray tracing far field layer ignores whether the component is hidden. */
...
#if WITH_EDITOR
	/** If true, then this component was added to the navigation octree when it was registered. */
	uint8 bRegisteredForNavigation : 1;
...
public:	
	...
	FAttachmentCounter* GetAttachmentCounter() { return &GetSceneData().AttachmentCounter; }
	...
	/** Store all the PSO precache data and schedule task to mark when precaching is done so component can be notified about it */
	ENGINE_API void SetMaterialPSOPrecacheData(TArray<FMaterialPSOPrecacheRequestID>& MaterialPSOPrecacheRequestIDs, const FGraphEventArray& PSOPrecacheCompileEvents);

	/** Check if PSOs are still precaching */	
	bool IsPSOPrecaching() const { return PSOPrecacheComponentData.IsPSOPrecaching(); }

	/** Get the PSO precaching data */
	FPSOPrecacheComponentData& GetPSOPrecacheComponentData() { return PSOPrecacheComponentData; }
	const FPSOPrecacheComponentData& GetPSOPrecacheComponentData() const { return PSOPrecacheComponentData; }
	 ...
	 */
	bool CheckPSOPrecachingAndBoostPriority(EPSOPrecachePriority PSOPrecachePriority = EPSOPrecachePriority::High)
	{
		return PSOPrecacheComponentData.CheckPSOPrecachingAndBoostPriority(PSOPrecachePriority);
	}
...
protected:
	...
	ENGINE_API FPrimitiveMaterialPropertyDescriptor GetUsedMaterialPropertyDesc(EShaderPlatform InShaderPlatform) const;
...
public:
	...
	*/
	...
	/**
	 * Gather the default custom primitive data parameters. Non-const as it keeps a non-const weak pointer on 'this'.
	 * Outputs an array of pairs of primitive indices with the corresponding parameter and the max primitive data index.
	 */
	ENGINE_API void GatherDefaultCustomPrimitiveDataParametersFromMaterials(TArray<TPair<uint8 /*PrimitiveIndex*/, FCustomPrimitiveDataParameter>>& OutParameters, int32& OutMaxPrimitiveDataIndex);

	/**
	 * Setup the Default Custom Primitive Data based on the assigned materials. Wipes any existing data.
	 */
	UFUNCTION(BlueprintCallable, Category = "Rendering|Material")
	ENGINE_API void SetupDefaultCustomPrimitiveDataFromMaterials();
...
public:
	 ...
	 * This will be deprecated in the next engine release.  Use OnTouchBegan with TouchId, Player Controller, and Hit Result arguments instead.
	 */
	UPROPERTY(BlueprintAssignable, Category="Input|Touch Input", DisplayName="On Touch Began (finger only)")
	 ...
	 * This will be deprecated in the next engine release.  Use OnTouchEnded with TouchId, Player Controller, and Hit Result arguments instead.
	 */
	UPROPERTY(BlueprintAssignable, Category="Input|Touch Input", DisplayName="On Touch Ended (finger only)")
	 ...
	 * This will be deprecated in the next engine release.  Use OnTouchEntered with TouchId, Player Controller, and Hit Result arguments instead.
	 */
	UPROPERTY(BlueprintAssignable, Category="Input|Touch Input", DisplayName="On Touch Entered (finger only)")
	 ...
	 * This will be deprecated in the next engine release.  Use OnTouchExited with TouchId, Player Controller, and Hit Result arguments instead.
	 */
	UPROPERTY(BlueprintAssignable, Category="Input|Touch Input", DisplayName="On Touch Exited (finger only)")
	FComponentEndTouchOverSignature OnInputTouchLeave;

	/** Event called when a touch input is received over this component when touch events are enabled in the player controller */
	UPROPERTY(BlueprintAssignable, Category="Input|Touch Input")
	FComponentOnTouchBeganSignature OnTouchBegan;

	/** Event called when a touch input is released over this component when touch events are enabled in the player controller */
	UPROPERTY(BlueprintAssignable, Category="Input|Touch Input")
	FComponentOnTouchEndedSignature OnTouchEnded;

	/** Event called when a finger is moved over this component when touch over events are enabled in the player controller */
	UPROPERTY(BlueprintAssignable, Category="Input|Touch Input")
	FComponentOnTouchEnteredSignature OnTouchEntered;

	/** Event called when a finger is moved off this component when touch over events are enabled in the player controller */
	UPROPERTY(BlueprintAssignable, Category="Input|Touch Input")
	FComponentOnTouchExitedSignature OnTouchExited;
	...
	ENGINE_API virtual class UMaterialInterface* GetMaterial(int32 ElementIndex) const override;
	...
	*/
	...
	ENGINE_API virtual const struct FWalkableSlopeOverride& GetWalkableSlopeOverride() const override;
	...
	*/
	 ...
	 */
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category="Physics", meta=(UnsafeDuringActorConstruction="true"))	
	ENGINE_API FVector GetPhysicsLinearVelocity(FName BoneName = NAME_None) const;
	...
	*/
	UFUNCTION(BlueprintCallable, BlueprintPure=false, Category = "Physics", meta=(UnsafeDuringActorConstruction="true"))
	ENGINE_API FVector GetPhysicsLinearVelocityAtPoint(FVector Point, FName BoneName = NAME_None) const;
	...
	*/
	...
	/** Changes the value of bCastShadowOnOtherObjectsOnly. */
	UFUNCTION(BlueprintCallable, Category="Rendering")
	ENGINE_API void SetCastShadowOnOtherObjectsOnly(UPARAM(DisplayName="ShadowOtherObjectsOnly") bool bInCastShadowOnOtherObjectsOnly);
...
public:
	...
	/**
	 * Returns the streamable Nanite asset owned by this primitive.
	 */
	ENGINE_API virtual UStreamableRenderAsset* GetStreamableNaniteAsset() const;
	 ...
	 */
	UE_DEPRECATED(5.8, "This has been replaced with LevelContext.GetStreamingRenderAssetInfoWithNULLRemoval")
	...
	*/
	...
	/** Called when the CollisionEnabled of the BodyInstance is changed. */
	ENGINE_API virtual void RecreateInstanceBody(int32 InstanceBodyIndex);
...
public:
	...
	ENGINE_API virtual void OnAsyncCreatePhysicsStateBegin_GameThread(TSet<UObject*>& OutRootedObjects) override;
...
public:
	...
	ENGINE_API virtual void OnRenderAssetFirstLodChange(const UStreamableRenderAsset* RenderAsset, int32 FirstLodIndex);
	...
	*/
	...
	UFUNCTION(BlueprintCallable, Category = "Physics")
	ENGINE_API virtual void SetAllowPartialIslandSleep(bool InAllowPartialIslandSleep = true, FName BoneName = NAME_None);

	/** [EXPERIMENTAL] Set whether all bodies in this component allow Partial Island Sleeping for the island they are in.
	Note that Partial Island Sleeping is a CPU optimization disabling non-moving bodies in the solver.
	This feature is useful for scenes with many rigid bodies but can affect the physics behavior. */
	UFUNCTION(BlueprintCallable, Category = "Physics")
	ENGINE_API virtual void SetAllAllowPartialIslandSleep(bool InAllowPartialIslandSleep = true);
...
public:	
	...
	inline bool ShouldCheckOverlapFlagToQueueOverlaps(const UPrimitiveComponent& ThisComponent) const;
	...
	/**
	 * Dispatches Begin/End/Over component events for a single touch.
	 *
	 * @param Controller the responsible Player Controller, or null.
	 * @param EnabledFamilies selects which event families to fire (BeginEnd, Over, or both).
	 * @param TouchedPrimitives per-finger-index array of the caller's currently touched components.  Assumes caller is coupled to a single device.
	 */
	static ENGINE_API void DispatchTouchHitResult(
		const FTouchId TouchId,
		const ETouchType::Type Type,
		const FHitResult& HitResult,
		const bool bHit,
		APlayerController* const Controller,
		const ETouchEventFamilies EnabledFamilies,
	...
	UE_DEPRECATED(5.8, "Use overload with Touch ID, Player Controller, and Hit Result." )
	...
	static ENGINE_API void DispatchTouchOverEvents(const FTouchId TouchId, APlayerController* const Controller, const FHitResult& HitResult, UPrimitiveComponent* const CurrentComponent, UPrimitiveComponent* const NewComponent);
	...
	ENGINE_API void DispatchOnInputTouchBegin(const FTouchId TouchId, APlayerController* const Controller, const FHitResult& HitResult);

	ENGINE_API void DispatchOnInputTouchEnd(const FTouchId TouchId, APlayerController* const Controller, const FHitResult& HitResult);
...
public:
	...
	ENGINE_API virtual IPhysicsBodyInstanceOwner* ResolvePhysicsBodyInstanceOwner(Chaos::FConstPhysicsObjectHandle PhysicsObject) override;
	...
	virtual bool IsStaticPhysics() const override { return Mobility != EComponentMobility::Movable && bStaticWhenNotMoveable; }
	 ...
	 */
	virtual bool IsMultiBodyOverlap() const override { return bMultiBodyOverlap; }

	/** Returns the source UObject of this body instance owner. */
	virtual UObject* GetSourceObject() const override { return const_cast<UPrimitiveComponent*>(this); }

	/** Returns the owner of the source UObject of this body instance owner. */
	ENGINE_API virtual UObject* GetSourceObjectOwner() const override;

	/** Returns the transform of this body instance owner. */
	virtual FTransform GetPhysicsOwnerTransform() const override { return GetComponentTransform(); }

	/** Return the BodySetup to use for this body instance owner (single body case) */
	virtual UBodySetup* GetPhysicsBodySetup() const override { return const_cast<UPrimitiveComponent*>(this)->GetBodySetup(); }

	/** Returns the body instance's physical material override. */
	ENGINE_API virtual UPhysicalMaterial* GetPhysicsMaterialOverride() const override;

	/** Returns the body instance owner mesh material. */
	ENGINE_API virtual UMaterialInterface* GetPhysicsMaterialBase() const override;

	/** Whether this body instance owner is movable or not. On UPrimitiveComponents this checks the Mobility type property. */
	ENGINE_API virtual bool IsPhysicsOwnerMovable() const override;
	
	/** Whether this body instance owner is simulating physics or not */
	ENGINE_API virtual bool IsPhysicsOwnerSimulatingPhysics() const override;
	
	/** Gets the body instance owner velocity. Since this is a UPrimitiveComponent we will also check component velocity. */
	ENGINE_API virtual FVector GetPhysicsOwnerVelocity() const override;
	
	/** Returns the attachment root of the body instance owner if it exists */
	ENGINE_API virtual UObject* GetPhysicsOwnerAttachmentRoot() const override;
	
	/** Whether this body instance is world geometry or not */
	ENGINE_API virtual bool IsPhysicsObjectWorldGeometry() const override;
	
	ENGINE_API virtual FTransform GetPhysicsOwnerSocketTransform(FName InSocketName) const override;
	
	/** Checks for a socket on the body instance owner */
	ENGINE_API virtual bool DoesSocketExistOnPhysicsOwner(FName InSocketName) const override;
	
	//~ End IPhysicsBodyInstanceOwner Interface.

	/** Wheter the component is relevant for first person */
	inline bool IsFirstPersonRelevant() const { return FirstPersonPrimitiveType != EFirstPersonPrimitiveType::None; }

	/** Thread-safe accessor for this component's SimpleStreamableAssetManager (SSAM) handle.
	 *  Returns the default-invalid handle if the component was never registered (SSAM disabled,
	 *  or ClearPrimitiveHandle already ran). */
	FSSAMPrimitiveHandle GetSSAMPrimitiveHandle() const { return SSAMPrimitiveHandle; }
private:
	friend class FSimpleStreamableAssetManager;
	/** SSAM handle for this component. Aliases the proxy's SSAMPrimitiveHandle when both are
	 *  live. Same atomicity reasoning as the proxy-side field -- see PrimitiveSceneProxy.h. */
	FSSAMAtomicPrimitiveHandle SSAMPrimitiveHandle;
protected:
...
inline bool UPrimitiveComponent::ComponentOverlapMulti(TArray<struct FOverlapResult>& OutOverlaps, const class UWorld* InWorld, const FVector& Pos, const FQuat& Rot, ECollisionChannel TestChannel, const struct FComponentQueryParams& Params, const struct FCollisionObjectQueryParams& ObjectQueryParams) const
{
	return ComponentOverlapMultiImpl(OutOverlaps, InWorld, Pos, Rot, TestChannel, Params, ObjectQueryParams);
}

inline bool UPrimitiveComponent::ComponentOverlapMulti(TArray<struct FOverlapResult>& OutOverlaps, const class UWorld* InWorld, const FVector& Pos, const FRotator& Rot, ECollisionChannel TestChannel, const struct FComponentQueryParams& Params, const struct FCollisionObjectQueryParams& ObjectQueryParams) const
{
	return ComponentOverlapMultiImpl(OutOverlaps, InWorld, Pos, Rot.Quaternion(), TestChannel, Params, ObjectQueryParams);
}

inline bool UPrimitiveComponent::ComponentOverlapComponent(class UPrimitiveComponent* PrimComp, const FVector Pos, const FQuat& Rot, const FCollisionQueryParams& Params)
{
	return ComponentOverlapComponentImpl(PrimComp, Pos, Rot, Params);
}

inline bool UPrimitiveComponent::ComponentOverlapComponent(class UPrimitiveComponent* PrimComp, const FVector Pos, const FRotator Rot, const FCollisionQueryParams& Params)
{
	return ComponentOverlapComponentImpl(PrimComp, Pos, Rot.Quaternion(), Params);
}

inline bool UPrimitiveComponent::ComponentOverlapComponentWithResult(const class UPrimitiveComponent* const PrimComp, const FVector& Pos, const FQuat& Rot, const FCollisionQueryParams& Params, TArray<FOverlapResult>& OutOverlap) const
{
	return ComponentOverlapComponentWithResultImpl(PrimComp, Pos, Rot, Params, OutOverlap);
}

inline bool UPrimitiveComponent::ComponentOverlapComponentWithResult(const class UPrimitiveComponent* const PrimComp, const FVector& Pos, const FRotator& Rot, const FCollisionQueryParams& Params, TArray<FOverlapResult>& OutOverlap) const
{
	return ComponentOverlapComponentWithResultImpl(PrimComp, Pos, Rot.Quaternion(), Params, OutOverlap);
}

inline const TArray<FOverlapInfo>& UPrimitiveComponent::GetOverlapInfos() const
{
	return OverlappingComponents;
}

inline bool UPrimitiveComponent::K2_IsCollisionEnabled() const
{
	return IsCollisionEnabled();
}

inline bool UPrimitiveComponent::K2_IsQueryCollisionEnabled() const
{
	return IsQueryCollisionEnabled();
}

inline bool UPrimitiveComponent::K2_IsPhysicsCollisionEnabled() const
{
	return IsPhysicsCollisionEnabled();
}

inline bool UPrimitiveComponent::GetGenerateOverlapEvents() const
{
	return bGenerateOverlapEvents;
}

inline bool UPrimitiveComponent::ShouldCheckOverlapFlagToQueueOverlaps(const UPrimitiveComponent& ThisComponent) const
{
