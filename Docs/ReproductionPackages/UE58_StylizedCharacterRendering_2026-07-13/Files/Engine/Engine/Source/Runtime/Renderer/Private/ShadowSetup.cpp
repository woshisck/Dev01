	...
	bool bInTranslucentShadow)
{
	...
	bCastShadowOnOtherObjectsOnly = !bInPreShadow && InParentSceneInfo->Proxy->CastsShadowOnOtherObjectsOnly();
...
void FProjectedShadowInfo::SetupMeshDrawCommandsForProjectionStenciling(FSceneRenderer& Renderer, FInstanceCullingManager& InstanceCullingManager)
{
	...
	const bool bMaskSubjectOutOfProjection = bCastShadowOnOtherObjectsOnly || bMobileModulatedShadowsAllowSelfShadow;
	if (bPreShadow || bSelfShadowOnly || bMaskSubjectOutOfProjection)
	{
		...
		for (int32 ViewIndex = 0; ViewIndex < Renderer.Views.Num(); ViewIndex++)
		{
			...
			if (View.ShouldRenderView())
			{
				...
				else if (bCastShadowOnOtherObjectsOnly)
				{
					// Preserve unrelated scene stencil bits while removing the subject from the projected-shadow mask.
					DrawRenderState.SetDepthStencilState(
						TStaticDepthStencilState<
						false, CF_DepthNearOrEqual,
						true, CF_Always, SO_Keep, SO_Keep, SO_Replace,
				...
				if ((bSelfShadowOnly || bCastShadowOnOtherObjectsOnly) && !bPreShadow && !bMobileModulatedShadowsAllowSelfShadow)
				{
...
struct FGatherShadowPrimitivesPacket
{
	...
	void AnyThreadFinalize(FDynamicShadowsTaskData& TaskData)
	{
		...
		for (int32 ShadowIndex = 0; ShadowIndex < ViewDependentWholeSceneShadowSubjectPrimitives.Num(); ShadowIndex++)
		{
			...
			{
				FCachedShadowMapData* CachedShadowMapData = const_cast<FCachedShadowMapData*>(TaskData.Scene->GetExtension<FCSMSceneExtension>().GetCachedShadowMapData(ProjectedShadowInfo->GetLightSceneInfo().Id, ProjectedShadowInfo->CascadeSettings.ShadowSplitIndex));
