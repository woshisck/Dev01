...
void ACelesPointLight::ConstructionUpdate()
{
	...
	if (SourcePointLight)
	{
		...
		SourcePointLight->StylizedCharacterLightMode = CharacterLightMode;
		SourcePointLight->StylizedCharacterShadowStrength = FMath::Clamp(CharacterShadowStrength, 0.0f, 1.0f);
		SourcePointLight->StylizedCharacterRimPower = FMath::Max(CharacterRimPower, 0.25f);
		SourcePointLight->StylizedCharacterRimIntensity = FMath::Max(CharacterRimIntensity, 0.0f);
		SourcePointLight->MarkRenderStateDirty();
	}
}
