...
|---|---|---|---|
| 主循环：波次刷怪 + 触发条件 + 兜底池 (LOOP-001) | "刷怪 → 清场 → 选符文 → 进门"核心循环 | ✅ 编译通过 | ✅ DA_Room / DA_Campaign 已配 |
...
| LENode_WaitForLootSelected (FEAT-031) | 等玩家选符文再继续 | ✅ 编译通过 | — |
| 三选一奖励基础（拾取物驱动 + CommonUI 隔离）(COMBAT-003a / FIX-022 / 23) | 多拾取物独立触发，避免重弹 | ✅ 编译通过 | ⚙ HB_PlayerMain 赋 LootSelectionWidgetClass |
| Loot 8 方向最佳落点算法 (FEAT-025 副) | 落到玩家可见 + 无碰撞位置 | ✅ 编译通过 | — |
| 关卡结束揭幕特效（时间膨胀 + 渐黑 + 圆形揭幕）(MAP-002 / FEAT-023) | 关卡完成仪式感过场 | ✅ 编译通过 | ⚙ WBP_LevelEndReveal + DA_LevelEndEffect 待配 |
| 开场镜头标记 (MAP-003 / FEAT-024) | 切到标记视角 + 平滑回玩家 | ✅ 编译通过 | ⚙ 关卡放 Actor + LevelFlow 调 TriggerIntro |
...
- **设计文档**：[LevelSystem_ConfigGuide](../../01_长期系统文档/系统/Level/LevelSystem_ConfigGuide.md) · [LevelSystem_ProgrammerDoc](../../01_长期系统文档/系统/Level/LevelSystem_ProgrammerDoc.md)
- **验收方式**：
...
- **设计文档**：[BuffPool_ConfigGuide](../../01_长期系统文档/系统/Level/BuffPool_ConfigGuide.md)
...
- **设计文档**：[Portal_ConfigGuide](../../01_长期系统文档/系统/Level/Portal_ConfigGuide.md)
...
- **设计文档**：[Portal_ConfigGuide](../../01_长期系统文档/系统/Level/Portal_ConfigGuide.md) · [WBP_PortalPreview_Layout](../../01_长期系统文档/系统/Level/WBP_PortalPreview_Layout.md) · [WBP_PortalDirection_Layout](../../00_入口与规范/缺失引用记录.md)
- **验收方式**：
...
- **设计文档**：[CrossLevelState_Technical](../../01_长期系统文档/系统/Level/CrossLevelState_Technical.md)
- **验收方式**：
...
- **设计文档**：[EditorSetup_LevelFlow_Tutorial_WeaponSpawner](../../99_归档/TODO/EditorSetup_LevelFlow_Tutorial_WeaponSpawner.md)
...
| ID | 内容 | 文档 |
|---|---|---|
| LEVEL-006 WBP（A）| 创建 `WBP_PortalPreview`（父类 `UPortalPreviewWidget`）按 Layout 规格搭建 | [WBP_PortalPreview_Layout](../../01_长期系统文档/系统/Level/WBP_PortalPreview_Layout.md) |
| LEVEL-006 WBP（B）| 创建 `WBP_PortalDirection`（父类 `UPortalDirectionWidget`，根 RootCanvas + ArrowTexture 字段）| [WBP_PortalDirection_Layout](../../00_入口与规范/缺失引用记录.md) |
| LEVEL-006 HUD | `BP_YogHUD` Details 填 `PortalPreviewClass` / `PortalDirectionClass` | 同上 |
| LEVEL-END-CONFIG | 创建 `WBP_LevelEndReveal`（全屏 Image 命名 RevealImage）+ `DA_LevelEndEffect` + 配置 BP_YogHUD | [TASKS](../../05_项目管理与进度/PM/TASKS.md) |
